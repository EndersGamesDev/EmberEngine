...
Players depend on `index.html` and `games.json` to discover one wasm bundle per selected game or lab, while each live page depends on `deploy/deploy-pages.sh` to assemble its matching generated bindings and wasm binary.
