...
function loop(now) {
  ...
  if (guard) {
    ...
    $('guard-label').textContent = guard.brokenLeft > 0 ? 'Guard broken' : guard.impactLeft > 0 ? 'Strike blocked' : guard.ready ? 'Guard ready' : guard.amount > 0 ? 'Raising / lowering guard' : 'Guard down';
    const blockCost = state.enemy?.blockCost ?? 28;
    const readyCue = state.enemy?.unblockable ? 'Crushing slam — dodge sideways' : state.stamina < blockCost ? `Low stamina — this strike needs ${blockCost} to block` : 'Face the weapon · dodge heavy slams';
    $('guard-cue').textContent = guard.brokenLeft > 0 ? 'Recover — dodge or step away' : guard.ready ? readyCue : combat?.active ? 'Guard waits for committed swings' : `Hold ${state.pad ? 'L2' : coarse ? 'Guard' : 'right mouse / F'} to guard`;
  }
  ...
  $('shortcuts').textContent = state.pad ? 'R1 strike · R2 heavy · × then R2 jump heavy · Hold L2 guard · Options menu' : document.pointerLockElement ? 'LMB strike · R / middle click heavy · Space then heavy · F / right mouse guard' : 'Click to look · WASD move · LMB strike · R heavy · F guard';
