...
function loop(now) {
  ...
  if (state.event !== lastEvent) { lastEvent = state.event; lastMessageAt = now; $('message').textContent = state.message; sound('event'); }
  ...
  if (state.stage === 5 && !ended) { ended = true; setPause(true); $('complete').hidden = false; $('hud').hidden = true; $('menu-button').hidden = true; $('play-again').focus(); }
}
requestAnimationFrame(loop);
