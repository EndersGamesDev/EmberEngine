...
use ember_game_arena_v11::proto::{C2S, S2C};
use ember_game_arena_v11::{
...
#[test]
fn every_v11_wire_message_has_a_canonical_json_fixture() {
...
fn legacy_lobby_create_join_and_refusal_match_the_deployed_wire() {
    ...
    assert_eq!(requested_version, 11);
    ...
    assert_eq!(game_key.game_version, 11);
    ...
    let refusal = ArenaLegacyDecoder::version_refusal(10, &[11]);
...
fn timestamped_transcript_produces_authoritative_checkpoints() {
    ...
    let InnerFrame::Text(joined_text) = joined_frame else {
        panic!("Arena v11 emitted a binary join frame");
    };
