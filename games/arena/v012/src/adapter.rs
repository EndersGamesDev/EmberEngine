//! Evergreen host adapters around the frozen Arena v12 contract.
...
impl ArenaCodec {
    /// Constructs the stateless v12 codec.
    #[must_use]
...
/// Factory for authoritative Arena protocol-12 sessions.
#[derive(Clone, Copy, Debug, Default)]
pub struct ArenaFactory;

impl ArenaFactory {
    /// Constructs the stateless v12 session factory.
    #[must_use]
    pub const fn new() -> Self {
        Self
    }
    ...
    fn create(
            ...
            || creation.game_key.game_version != u32::from(PROTO_VERSION)
        {
            return Err(FactoryError::InvalidConfiguration(
                "Arena v12 factory received a different game key".to_string(),
            ));
        }
        if !creation.configured_rules.is_empty() {
            return Err(FactoryError::InvalidConfiguration(
                "Arena v12 has no configurable rules".to_string(),
            ));
        }
            ...
            C2S::Input {
                ...
                let jump = jump
                    || self
                        .inputs
                        .get(&player_id)
                        .is_some_and(|record| record.input.jump);
                let melee = melee
                    || self
                        .inputs
                        .get(&player_id)
                        .is_some_and(|record| record.input.melee);
                self.inputs.insert(
                    player_id,
                    InputRecord {
                        input: PlayerIn {
            ...
            let state = S2C::State {
                ...
                players: self
                    ...
                    .map(|player| PState {
                        ...
                        ack_age_ticks: self.inputs.get(&player.id).map_or(0, |record| {
                            bounded_tick_age(self.sim.tick, record.received_tick)
                        }),
                    })
                    .collect(),
                bullets: self
...
impl ArenaLegacyDecoder {
    ...
    /// Builds the deployed protocol mismatch refusal for the hosted v12 set.
    #[must_use]
