//! Evergreen host adapters around the frozen Arena v10 contract.
...
impl ArenaCodec {
    /// Constructs the stateless v10 codec.
    #[must_use]
...
impl ArenaFactory {
    /// Constructs the stateless v10 session factory.
    #[must_use]
    pub const fn new() -> Self {
        Self
    }
    ...
    fn create(
            ...
            || creation.game_key.game_version != u32::from(PROTO_VERSION)
        {
            return Err(FactoryError::InvalidConfiguration(
                "Arena v10 factory received a different game key".to_string(),
            ));
        }
        if !creation.configured_rules.is_empty() {
            return Err(FactoryError::InvalidConfiguration(
                "Arena v10 has no configurable rules".to_string(),
            ));
        }
...
impl ArenaLegacyDecoder {
    ...
    /// Builds the deployed protocol mismatch refusal for the hosted v10 set.
    #[must_use]
