...
use ember_game_arena_v10::proto::{C2S, S2C};
use ember_game_arena_v10::{
...
#[test]
fn every_v10_wire_message_has_a_canonical_json_fixture() {
...
fn legacy_lobby_create_join_and_refusal_match_the_deployed_wire() {
    ...
    assert_eq!(requested_version, 10);
    ...
    assert_eq!(game_key.game_version, 10);
    ...
    let refusal = ArenaLegacyDecoder::version_refusal(9, &[10]);
...
fn timestamped_transcript_produces_authoritative_checkpoints() {
    ...
    let InnerFrame::Text(joined_text) = joined_frame else {
        panic!("Arena v10 emitted a binary join frame");
    };
