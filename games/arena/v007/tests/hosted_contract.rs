...
use ember_game_arena_v7::proto::{C2S, S2C};
use ember_game_arena_v7::{
...
#[test]
fn every_v7_wire_message_has_a_canonical_json_fixture() {
...
fn legacy_lobby_create_join_and_refusal_match_the_deployed_wire() {
    ...
    assert_eq!(game_key.game_version, 7);
    ...
    let refusal = ArenaLegacyDecoder::version_refusal(6, &[7]);
...
fn timestamped_transcript_produces_authoritative_checkpoints() {
    ...
    let InnerFrame::Text(joined_text) = joined_frame else {
        panic!("Arena v7 emitted a binary join frame");
    };
