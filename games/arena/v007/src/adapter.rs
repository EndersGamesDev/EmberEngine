//! Evergreen host adapters around the frozen Arena v7 contract.
...
/// Exact JSON text-frame codec for Arena protocol 7.
#[derive(Clone, Copy, Debug, Default)]
pub struct ArenaCodec;

impl ArenaCodec {
    /// Constructs the stateless v7 codec.
    #[must_use]
...
/// Factory for authoritative Arena protocol-7 sessions.
#[derive(Clone, Copy, Debug, Default)]
pub struct ArenaFactory;

impl ArenaFactory {
    /// Constructs the stateless v7 session factory.
    #[must_use]
    pub const fn new() -> Self {
        Self
    }
    ...
    fn create(
            ...
            || creation.game_key.game_version != u32::from(PROTO_VERSION)
        {
            return Err(FactoryError::InvalidConfiguration(
                "Arena v7 factory received a different game key".to_string(),
            ));
        }
        if !creation.configured_rules.is_empty() {
            return Err(FactoryError::InvalidConfiguration(
                "Arena v7 has no configurable rules".to_string(),
            ));
        }
...
impl ArenaLegacyDecoder {
    ...
    /// Projects exact Arena-7 entries into the deployed legacy list tag and fields.
    #[must_use]
    ...
    /// Builds the deployed protocol mismatch refusal for the hosted v7 set.
    #[must_use]
