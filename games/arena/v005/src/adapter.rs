//! Evergreen host adapters around the frozen Arena v5 contract.
...
/// Exact JSON text-frame codec for Arena protocol 5.
#[derive(Clone, Copy, Debug, Default)]
pub struct ArenaCodec;

impl ArenaCodec {
    /// Constructs the stateless v5 codec.
    #[must_use]
...
/// Factory for authoritative Arena protocol-5 sessions.
#[derive(Clone, Copy, Debug, Default)]
pub struct ArenaFactory;

impl ArenaFactory {
    /// Constructs the stateless v5 session factory.
    #[must_use]
    pub const fn new() -> Self {
        Self
    }
    ...
    fn create(
            ...
            || creation.game_key.game_version != u32::from(PROTO_VERSION)
        {
            return Err(FactoryError::InvalidConfiguration(
                "Arena v5 factory received a different game key".to_string(),
            ));
        }
        if !creation.configured_rules.is_empty() {
            return Err(FactoryError::InvalidConfiguration(
                "Arena v5 has no configurable rules".to_string(),
            ));
        }
...
impl ArenaSession {
    ...
    fn run_tick(&mut self, update: &mut SessionUpdate) {
        ...
        self.sim.step(&|player_id| {
            inputs
                .get(&player_id)
                .map_or_else(PlayerIn::default, |record| record.input)
        });
