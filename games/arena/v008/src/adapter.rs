//! Evergreen host adapters around the frozen Arena v8 contract.
...
/// Exact JSON text-frame codec for Arena protocol 8.
#[derive(Clone, Copy, Debug, Default)]
pub struct ArenaCodec;

impl ArenaCodec {
    /// Constructs the stateless v8 codec.
    #[must_use]
...
/// Factory for authoritative Arena protocol-8 sessions.
#[derive(Clone, Copy, Debug, Default)]
pub struct ArenaFactory;

impl ArenaFactory {
    /// Constructs the stateless v8 session factory.
    #[must_use]
    pub const fn new() -> Self {
        Self
    }
    ...
    fn create(
            ...
            || creation.game_key.game_version != u32::from(PROTO_VERSION)
        {
            return Err(FactoryError::InvalidConfiguration(
                "Arena v8 factory received a different game key".to_string(),
            ));
        }
        if !creation.configured_rules.is_empty() {
            return Err(FactoryError::InvalidConfiguration(
                "Arena v8 has no configurable rules".to_string(),
            ));
        }
                ...
                self.inputs.insert(
                    ...
                    InputRecord {
                        input: PlayerIn {
                            ...
                            reload,
                            jump,
                            // Protocol v8 has shield simulation but no shield input field yet.
                            shield: false,
                            delay_ticks: 0,
                        },
                        sequence: seq,
...
impl ArenaLegacyDecoder {
    ...
    /// Projects exact Arena-8 entries into the deployed legacy list tag and fields.
    #[must_use]
    ...
    /// Builds the deployed protocol mismatch refusal for the hosted v8 set.
    #[must_use]
