...
/// Weapon-pad respawn duration in seconds.
pub const PAD_RESPAWN_SECS: f32 = 15.0;
/// Weapon-pad pickup radius.
...
pub struct WeaponStats {
    ...
    /// Magazine capacity.
    pub mag: u8,
    /// Damage dealt by each round.
    pub damage: u8,
}

/// Returns the frozen statistics for a weapon level.
#[must_use]
...
/// Hit points restored on spawn.
pub const MAX_HP: u8 = 3;
/// Delay before a defeated player respawns.
pub const RESPAWN_SECS: f32 = 2.5;
/// Maximum players admitted to one Arena session.
...
/// Maximum height of the jumpable cover class.
pub const CRATE_MAX_H: f32 = 1.5;
/// Minimum height of the hard-cover class.
...
// These integer-to-float casts are part of the frozen deterministic formula.
...
// This cast is part of the frozen deterministic spawn formula.
#[allow(clippy::cast_precision_loss)]
...
/// One seeded weapon-upgrade pad and its cooldown.
#[derive(Clone, Debug)]
pub struct Pad {
    /// XZ centre of the pickup area.
    pub pos: [f32; 2],
...
/// Held player intents and the bounded rewind delay applied to new rounds.
// These independent input flags are shared simulation API and cannot be consolidated.
...
pub struct PlayerSt {
    ...
    pub alive: bool,
    /// Whether the crouched stance is active.
    pub crouch: bool,
...
/// One authoritative projectile.
#[derive(Clone, Copy, Debug)]
pub struct Bullet {
    /// Horizontal XZ position.
    pub pos: [f32; 2],
    /// Horizontal XZ velocity.
    pub vel: [f32; 2],
    ...
    pub ttl: f32,
    /// Credited player identifier.
    pub owner: u8,
    /// Damage dealt on connection.
    pub dmg: u8,
...
/// Deterministic authoritative Arena simulation.
pub struct Sim {
    /// Frozen seeded obstacles.
    pub obstacles: Vec<Obstacle>,
    /// Weapon-upgrade pads.
    pub pads: Vec<Pad>,
    /// Players in simulation order.
...
impl Sim {
    /// Constructs the exact seeded v8 arena state.
    #[must_use]
    ...
    /// Removes a player, their rounds, and their rewind history.
    ...
    fn rewound(&self, id: u8, delay: u16) -> Option<([f32; 2], f32, bool, bool)> {
        ...
        let idx = self.history.len().checked_sub(1 + usize::from(delay))?;
