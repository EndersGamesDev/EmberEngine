...
## Arena Shooter (v13: Trench City, v14: fullscreen, v15: the revolver, v16: one operator, v17: scutum and Murasama, v18: Freight Yard, v19: modes, v20: realism)
...
**v19 is the match update: free for all, team deathmatch and king of the hill.** Until now the arena was one endless free-for-all; v19 gives the lobby a `GameMode`, chosen at creation beside the map and carried by name on the wire exactly as the map is (`CreateLobby.mode`, `LobbyInfo.mode`, `GameJoined.mode`), which the shared sim resolves into rules: who a round may hit, how a point is earned, when the round ends. **Free for all** is today's game with an end, first to `FFA_FRAG_LIMIT` (20) frags. **Team deathmatch** is blue (0) against red (1): a joiner lands on the smaller team (ties by id parity), each team spawns on its own side of the map through `Level::spawns_for`, a teammate is neither hit nor pierced by a friendly round, spared by a friendly rocket's splash and untouched by a friendly blade, and the first team to `TDM_FRAG_LIMIT` (30) wins. **King of the hill** is the mode the maps were already asking for: both have a raised centre that the king block hangs over, Freight Yard's loading dock and Trench City's statue plinth, and that footprint is the `Hill` on the `Level`; a player standing on it alone earns a point every `HILL_TICK_SECS`, two or more earn nothing (`HILL_CONTESTED`), and the first to `HILL_LIMIT` (60) is king, with frags kept in a separate `frags` counter so the scoreboard still shows them. Every mode is a round: at the limit the sim records the winner in `round_over`, the server sends `S2C::RoundOver` once, and after `ROUND_PAUSE_SECS` (10, counted down on the wire as `State.round_pause`) every score, frag, death and team total resets, everyone respawns with the sidearm, every bullet is cleared and every `?` block re-armed. All of it is stepped in the sim, so the server and any replay agree on the second a round ends. The client draws team colours in place of id colours, the hill as four bars in the holder's colour with an orange pulse when contested, and a status line per mode; the page gains the mode selector and a mode pill on every lobby row. Full design in `docs/plans/arena-v19-modes.md`. **Protocol 16**: every new field is `serde(default)` and decodes on a v15 peer, but a v15 client in a team game shoots a teammate and watches the round vanish with no hit and no kill, and in king of the hill watches scores climb with nobody dying; that is the `CLAUDE.md` test, so the frozen v18 page goes list-only against a v16 host exactly as at every bump. Map and mode both travel as strings, so the next mode is additive.

**v21 is boots: you can hear players move, and crouching hides you.** A footstep fires on each foot plant of the walk animation rather than on a timer, so the cadence is the legs you can see and a sprinter's steps come faster on their own; a sprint lands a heavier, louder cue than a walk, not the same one turned up. A crouching player makes no sound at all, at any speed, on every peer that could have heard them: that is what crouching buys. Steps are panned and delayed by distance through `Audio::play_spatial` like every other remote cue and fade out well before a gunshot would, and your own are played quieter than a stranger's so they never mask the boots you need to hear. Nothing new goes on the wire for it: crouch, position and feet height are already there, and a walk is told from a sprint by the measured speed against the stances the sim defines, so **the protocol stays 17** and a v20 page still joins a v21 host.

**v20 is the realism pass: ballistics, tracers, impacts and sound.** Until now a round flew at 34 to 60 m/s because the sweep tested cover at the tick's end point and sampled the head band in at most 32 steps; a round at 900 m/s crosses 15 m in one tick and would have tunnelled through a 0.4 m wall and skipped a head. v20 makes both tests exact on the segment (`segment_box_entry` finds where a segment enters a box, the world pass takes the smallest entry over every box, the floor and the arena wall, and the head band is an analytic overlap of the round's height with the band over the segment), sorts a tick's bodies by contact so the nearest one takes the round, and sets every speed to the real muzzle velocity: 280 m/s for the sidearm's .45, 380 for the Vityaz, 715 for the AK-47, 880 for the M4, 450 for the revolver and 900 for the sniper, with the real gravity on the last two, and the RPG-7 at 120 m/s with a sustainer that adds 360 m/s² to 300 over the first half second. A round now rarely lives long enough to appear in a state, so the sim records a **shot event** the tick a round ends (`Sim.shots`, forwarded as `S2C::Shot`): owner, weapon, the segment's start and end, what stopped it (expiry, cover, body, shield, floor, wall), which cover kind, which victim and the surface's normal; a reflected round ends one event at the plate and starts another from it, a pierced round chains one per body. The client draws from those events instead of from states: a tracer that is the round itself (a body of revolution at its real calibre, drawn at five times its size, in a copper jacket) travelling at the weapon's speed with a round streak behind it that fades in 120 ms (the aim dot is gone; the page's crosshair is the crosshair, and the native client draws a hairline "+"), a muzzle flash that is a star of five spikes along the bore (not a cube) with the smoke of the shot behind it (round puffs, held back by the flash's own 35 to 60 ms and started a little down the bore, so a shot reads as light first and smoke second where it used to read as four grey boxes over an unseen star), a brass casing from an own gun, and an impact by material (sparks on metal, dust on stone and the floor, splinters on wood, a puff on sand, the flash on a body) with a near-black bullet hole kept 20 s on the surface, at most 96, sized by the calibre that made it: three calibres across (a 9 mm leaves 27 mm, a .454 Casull 35 mm) and half a metre for a rocket's blast. The sound is rebuilt as a small deterministic synthesis kit (noise, sines, envelopes, one-pole and biquad filters, a comb tail, a soft clip): every gun has a layered gunshot (mechanism, blast, body, tail) in near, mid and far variants, and every remote cue goes through `Audio::play_spatial` with a delay of distance over 343 m/s and a pan from the shooter's bearing; a round that passes within 3 m of the eye above the speed of sound plays a supersonic crack with no delay; impacts, a ricochet on metal, a casing on the cobbles and a reload per gun complete the set. Recorded samples are the next tier: `crates/arena/assets/sfx/` is the slot, empty in v20 and documented in `docs/asset-pipeline.md`. What this renderer cannot do is said plainly in the plan and the backlog: no additive blending, so every effect is opaque geometry, and one texture per mesh, so a mark carries no picture. Full design in `docs/plans/arena-v20-realism.md`. **Protocol 17**: a v16 client draws rounds from `BState` alone, so against a v17 server it would see almost nothing fly while the server's rounds cross the map in three ticks, and `S2C::Shot` is a variant an old peer drops; that is the `CLAUDE.md` test, so the frozen v19 page goes list-only against a v17 host exactly as at every bump.

**Play: <https://endersgamesdev.github.io/EmberEngine/>** — pick Arena Shooter. First-person drop-in shooter for up to 8 players in three modes (free for all, team deathmatch, king of the hill), with real ballistics, tracers, impacts and spatial sound since v20: WASD + mouse look, Space jump, Shift sprint, C crouch, RMB aims down the sights, R reload, Q raises a shield that reflects the round back at the shooter, E is a melee that goes through a raised shield, headshots kill outright, and the guns hang in `?` blocks you bonk from below. A standard gamepad works beside the keyboard, with rumble on Chromium.
