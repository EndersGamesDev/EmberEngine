...
| 3 | ~~Create release **v22** on main~~ | barza | done 22:2x | tag `v22` at d85a40f, "v22 - skies and weather" — https://github.com/EndersGamesDev/EmberEngine/releases/tag/v22 |
...
- The durable host-deploy worktree is `C:\Users\end\dev\ember-environment` (codex's, own cargo target); the `ember-arena-host` wrapper points there per codex's request (barza seq 54) — do not re-point it without posting to the barza board first. `C:\Users\end\dev\ember-host` stays a clean main worktree for this loop's commits (it carries a git-ignored `target` junction to the shared `C:\Users\end\dev\ember\target` for fast local builds; `deploy-pages.sh` assumes a local `target/`, so the junction is what makes a shared-target build resolve its wasm paths). Wrapper backups: `~/.ember/arena-local/ember-arena-host.cmd.bak-barza` (original, lane tree) and `.bak-v22` (the durable codex version, currently installed).
- Idle `heartbeat: …` commits on `main` were quiet markers rather than malfunctions. Their Actions workflow is retired; `marketing/heartbeat.local.log` remains this machine's record, and `marketing/heartbeat.log` is the closed public record.
...
- 2026-09-04 22:2x: correction — codex's own deployment (barza seq 54/55) had already shipped host and Pages to r1265/d85a40f (Pages commit 3dc4514; verified public 22:2x). My pages-deploy attempt was aborted, its one-off task deleted, and the wrapper restored to codex's durable worktree per their request. Release v22 created (task 3 done). Top open tasks: wiki seed (4) and the Show HN paste (5, owner).
