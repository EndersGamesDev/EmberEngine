...
| 6 | Paste r/rust, then r/webassembly | Ender | open | r/rust the same day as HN (different framing, the drafts differ), r/webassembly the next day |
| 7 | Paste r/gamedev with the clip | Ender | open | after task 5; r/indiegaming the same week |
| 8 | First triage pass: answer every issue within a day | barza (next session) | recurring | `gh issue list` on the repo; the bug template already asks for the protocol number and the host chip |
| 9 | Weekly: re-measure the KPIs, append `LOG.md` | barza (next session) | recurring | `gh repo view ... --json stargazerCount,forkCount,watchers,issues` |
| 10 | Deep post: "how we did the address book" from `docs/hosts.md` | barza | queued | after the first week of HN comments — use the questions asked there as the outline |
...
- The working tree on `lane/arena-v18` had uncommitted changes (README, arena crates, the v20 page, the backlog) when the marketing session started, and the deploy scripts refuse to run against a dirty tree — so if task 4 (a full `deploy-pages.sh`) is ever wanted, it needs a clean checkout. `engine.html` is already on gh-pages directly; the deploy line only future-proofs it.
...
- The release `v20` tags main's HEAD as of 2026-09-04 (46ed42b). The next release follows the next arena version, notes from the README section.
