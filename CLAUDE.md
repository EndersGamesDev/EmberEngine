...
Every game in this repo runs on ember: Rust against the layering above, compiled to one wasm bundle, drawn by the renderer whose constraints are listed below, and listed in the launcher (`web/games.json`) only once it does. A live page ships that bundle plus one shared loader (`crates/ember-loader` and the tracked `web/loader.ts.j2` template, emitted as root `loader.js`), built once by the deploy and shipped once for every page, which finds the server while the bundle downloads and reports the download, the compile and the engine's start; a game that brings its own loading code brings a second answer to a question that has one. A game that brings its own engine, its own runtime or its own build toolchain is not merged and is not put in the launcher, however finished it is: two engines in one repo means two sets of constraints, two deploy paths and two things to keep alive.

The `feature/verschaetz-dich` branch stays unmerged and unlisted as a standalone Next.js app whose five commits replace the whole repository root. It remains on `origin` as an independent branch; bringing the game into this repository would first require a port onto Ember, while the existing application fits a separate project.
...
Outdoor sun, shadows, moving clouds and alpha particles are shared engine features, enabled through `Frame.environment`; existing games keep their default lighting. Weather stays presentation-only. API, budgets and verification: **`docs/environment-weather.md`**.
