...
The live Four Kings page now boots a compiler-emitted `main.js` from a one-line stamped module import instead of carrying its behavior inline. Its template consumes generated Kings client/server unions, loader events, host-book data and wasm-bindgen declarations; handles every server variant under an exhaustive `never` branch; and narrows hostile socket, storage, wasm and HUD values locally. The assembler places and stamps the emitted module beside the page, while a rendered DOM/WebSocket smoke and the Pages fixture pin the no-inline and publication paths (`f93ba07`, `6029a92`).

Host selection, the shared loader and the Pages host gate now live with their three independent Node suites as pedantic TypeScript legacy templates. `ember-webgen` prepends their generated boundary imports, stages line maps and maps compiler diagnostics back to template coordinates; the assembler publishes only the emitted root `hosts.js` and `loader.js`, while Pages rebuilds the unshipped emitted host gate from its checkout under Node 24.20.0. The only conversion-added runtime guards are `eventFrom`, `loaderRulesFrom`, `gameGlueFrom` and `hostsModuleFrom`; no other previously accepted input is narrowed. The source-AST gate rejects unchecked escape hatches across every rendered production input (`7d5b4692`, `45e0a3ff`, `1e289ef6`, `9ab9cb91`).

End Game v12's six browser modules and four independent tool suites are pedantic TypeScript legacy templates compiled against generated loader and wasm-bindgen declarations; a fifth Node suite substitutes a controlled promise at emitted `main.js`'s computed import boundary, validates the loader namespace hand-off and drives a real HUD tick against DOM classes parsed from the tracked page. The assembler still publishes all six modules at their existing paths, now takes them from compiler output after live-source copying and verifies the stamped `main.js` plus five verbatim siblings against the emitted tree, while the TypeScript suite proves that the six data-listed modules are the complete literal static, re-exported and dynamic sibling-import closure. Computed module specifiers are outside that closure check. Before this conversion the live-directory copy already shipped every module, so the study's claimed four-file omission was not present in the assembled page (`9477b00c`, `1fc2c6eb`, `df46e9c4`, `789f16da`).

Every compiler-described 64-bit script-boundary field now declares whether it requires exact integer identity or is precise within a stated engineering bound; generated declarations render those choices as `bigint` or `number`, schemas and the manifest retain the rationale, and both the derive and generator reject an undeclared wide integer without changing the existing JSON transport. The loader now treats an advertised resource length above JavaScript's safe-integer range as unknown, omitting percentage and ETA instead of carrying a rounded total (pending).

The typed-web generator foundation pins Node 24.20.0, npm 11.19.0, TypeScript 5.9.3 and the Node 24 declarations; stages wasm-bindgen declarations outside the Pages tree; renders compiler-owned boundary declarations, exhaustive consumers and JSON schemas from native Rust registrations; and makes the release assembler reject generated contracts that fail the pedantic TypeScript projects. Generated files remain untracked and unshipped until a later phase converts a page or hand-written script to consume them (`0fc21b6a`, `a9464c1b`, `264273a3`, `240554ab`).

The Pages assembler now derives every root and live-page source copy from tracked runtime files, validates relative imports, workers and runtime assets against paths this assembly placed, retains frozen Arena v0 byte-for-byte from the publication seed and reads wasm inputs from Cargo's configured target directory so shared-output builds need no workspace symlink; the assembled End Game v12 page was loaded through to the running dungeon on the build pod's served tree (`0b1f8c2f`, `dd1b19f3`, `18da3229`).
...
Integration now lands on `develop` through pull requests whose required CI checks include the complete workspace and deployment policy suites; required approvals start at zero because CI is the merge gate and review is recorded in the pull-request body. Tracked active ruleset payloads bind those literal CI jobs to `develop`, separate immutable `main` and release-tag integrity from narrow creation and update bypasses, and block all other tags. Every `develop` push runs the complete CI workflow, including documentation-only merges, so the release gate can select the successful push run at the exact tagged SHA rather than relying on a pull request's synthetic merge SHA. Signed, annotated release tags select green `develop` commits, and the queued release workflow validates their shape, series, signature, ancestry, every dedicated manifest version, exact accepted changelog entry and the newest successful completed `.github/workflows/ci.yml` run with exactly the three successful required jobs before building the whole Pages site as one release archive, attaching it to an unpublished draft, fast-forwarding the tagged commit onto `main` and only then publishing the release; `queue: max` retains later tag runs for sequential execution, Ember tags additionally prove every shared crate inherits the checked workspace version, a failed promotion deletes its draft, and single-tag publication fails closed rather than deriving public notes from an annotation fallback. Draft creation explicitly suppresses latest-release selection, while post-promotion publication recomputes the tag's prerelease and latest metadata before making the release public. Pages waits for the release workflow to succeed and deploys that published archive at `main` HEAD; its tag lookup uses the same coarse numeric release namespace as the repository rulesets, and secondary host and design descriptions name this workflow-success trigger. Hosts follow only the fetched remote `main`; a missing remote release branch fails before the deployed marker or live servers change, and the timer preserves that failure status. Host test remotes reproduce CI's detached source shape before publishing the tested commit as a real `main` branch, so every Git version exercises the branch-shaped deployment contract. Workflow policy tests inspect parsed triggers, job keys and release dependencies, exercise the runtime tag regex against accepted and rejected tags, and prove quoted-job and permissive-regex fixtures fail even on the normalized shell fallback. The written contract now records End Game's actual `12.0.0` baseline, the historical retag tool's standing safety rule, prospective signature enforcement, the single present armored release key and its signing-subkey fingerprint, the second-key prerequisite, the external credential boundary, and ruleset activation before the first governed pull request. Legacy Pages publishers are preparation-only, trusted signing keys live in `deploy/keys`, and the hourly repository heartbeat is retired because pull requests and CI now record integration health; every repository description now treats its workflow and committed log as retired or closed rather than promising new hourly markers (`ed152c7c`, `2f3e3423`, `b76f9177`, `a8032fd6`, `86e4954d`, `0fce9805`, `6e9de077`, `7841b85e`, `0c2e54c3`, `936f9557`, `47f06884`, `832c612d`, `99cb9dd8`, `f52142ae`, `e21cf621`, `7ad7f820`, `c6971bcf`, `7ac533a9`, `62d057f0`, `e3c22108`, `4bb23da5`, `9ee338ed`, `ce764420`, `87acb1eb`, `b1cb82c5`, `573734e2`, `38001427`, `fce0c508`, `9438e0d7`, `2c6e2a89`, `76e5ac31`, `2524477e`, `5d28367c`, `6cc6260c`, `682fc471`, `1a189e46`, `b05530e7`, `01aaa5c1`, `4607565c`, `9466dd56`, `3d22eec5`).
...
The Fire Racer page now serves its own type. Its stylesheet opened with an `@import` of `fonts.googleapis.com`, which made it the one page on this site that asks another operator for part of itself on every load, against a deploy that otherwise assembles one tree and ships it. Seven `@font-face` rules load woff2 files placed beside the page, keeping the `font-display` and `unicode-range` the imported sheet declared so nothing about the rendering changes but the origin. Seven, not the nine the import fetched: the stylesheet selects Barlow at 400, 500, 600 and 700 and Barlow Condensed at 600, 700 and 800, and never selects the Condensed 500 and 900 that every player was downloading. The faces are Google's latin-subset builds of upstream Barlow, 156,980 bytes, deployed with the 4,377-byte SIL Open Font License 1.1 the fonts are released under, because that licence requires its notice to travel with them — 161,357 bytes added to the published tree. `deploy/deploy-pages.sh` places the directory and `deploy/tests/test-pages.sh` pins it in both its fixture and its expected tree, since a face the assembly leaves behind is a silent fallback to system type rather than an error anyone sees. `.gitattributes` marks the faces binary, so no Windows checkout can rewrite a font into something that will not load, and exempts the licence from the whitespace check, because one line of upstream's text ends in a space and a licence is reproduced rather than reformatted (`76cc7fce`).

The page's static title read "Fire Racer V2 · GT Circuit", which matched no other page; Arena's reads "Killshot · Ember" and League's "UltimateLegue · Ember". It now reads "Fire Racer · Ember". That tag is visible only until the bundle loads, after which `race.js` sets the same "name · Version N · Ember" title the other four live pages set, so the static tag was the only part out of line and the only part changed. The header is unchanged: the `V2` badge is the directory slot that `docs/versioning.md:31` keeps stable, and the `#release-version` label beside it is the version surface `docs/versioning.md:63-66` asks every page to derive from its package version, which every live page carries (`4a3fd09e`).

The fire series advances to 1.0.2. A version identifies shipped source and each merge advances the patch grade of every series it changes, and this merge changes what the Fire release ships even though no crate source moved: a player served 1.0.1 and a player served this receive different bytes. `fire`, `fire-core` and `fire-server` move together, `Cargo.lock` mirrors those three stanzas, and the launcher's live v2 slot follows, because `deploy/deploy-pages.sh` binds that entry to the package version and cannot be moved on its own. `cargo build --locked` proves the manifests and the lock agree (`99240988`).

Verification: all six shell suites pass at each of the three commits, including the changelog suite at 538/538 with the published branch present, and the one cargo gate the version bump needs reports exit 0 in 7.7 s. The page itself was proven outside this lane, by driving the assembled tree in a browser: the tab reads "Fire Racer · Ember", all seven woff2 files are served from the site with 200, no request reaches `googleapis` or `gstatic`, and the page renders in Barlow.
...
The live Fire Racer race and garage modules are checked legacy TypeScript templates compiled against generated Fire protocol, loader and wasm-bindgen declarations. Socket ownership stays in wasm; hostile HUD JSON, lobby data, loader events, storage and imported namespaces narrow locally from `unknown`. An emitted Node smoke imports both entry points under DOM doubles and dispatches every generated server-message variant through an exhaustive check without reading either 64-bit field. Assembly copies the emitted modules by name, stamps the race module only in the Pages tree and verifies stamped or exact compiler bytes while leaving the emitted tree unchanged (`d1c679a9`, `24dec0cf`, `f8778f58`, `a89dc0cf`, `74fb87c3`).
...
Publication now proves a reachable host per live server game instead of assuming one. Every page picks its host by exact protocol equality, so a release that moves a page's protocol while the published hosts still speak the old one produces a healthy site, a healthy release and a game nobody can enter; on 2026-09-11 three of the four live games were in that state for a day. `deploy/check-hosts.mjs` asks the question by doing what a browser does, in the order `chooseHost` does it — load the book, fetch its bound mirrors, merge, probe every host that runs the game, and only then apply the protocol filter, so a live Welcome outranks a book that has gone stale — and `.github/workflows/pages.yml` runs it against the extracted release asset between extraction and deployment, failing the deploy when any live server game has no host it can join; the policy test pins the step's position and that it can fail the job, since `continue-on-error` would leave a gate-shaped log line (`fa62fe63`, `6f51e38c`). It reads the bound-mirror rule from `web/hosts.js` rather than copying it (`056f4951`), and the launcher-version test it sits beside no longer asserts a hand-maintained list of release numbers that had been failing since Julibrot 1.2.0 (`9322f99e`). The gate can trust the catalog because `deploy/deploy-pages.sh` now refuses a live `proto` that differs from its game's protocol crate, one that no crate defines, and one on a lab, deriving both sides from the tree so a fifth server game cannot be gated by nothing (`860209da`). Mirror bindings moved to a tracked `web/mirrors.json` merged into the assembled book, which also drops the stale `hosts[]` entry of every bound name — without that the book's own entry outranks the mirror and the binding changes nothing — and `deploy/republish-host.sh --per-host` writes each host's mirror as its own file on the `hosts/book` branch, refusing the one name that would write the address book itself (`20c41f38`); that namespace is recorded in the branch table, the ruleset prose and the tracked request body together so the tree matches the live ruleset once the owner step adds it (`73557cba`). `deploy/ship-host.sh` takes `EMBER_SHIP_REF` to build and stamp a named ref instead of the published commit, validating it as a ref name before it reaches the builder as shell, resolving it once there and checking out that resolution because an ambiguous ref makes two lookups disagree silently, and comparing the stamp against it whether or not a ref was named; `check` resolves the same ref on the same freshly fetched builder, the window in which the host runs ahead of the site is printed with both commits named rather than discovered by a player, and `docs/branching.md` places that step before the tag push and proves the assembled release tree rather than the already-published site (`19e77a87`, `c2e25de3`). Tracked JavaScript and JSON are pinned to LF, which the release archive's byte comparison depends on and no rule covered (`ccced801`). A silent host no longer makes the release-tree check report a game hostless on one socket: a host that stays silent is asked again, three attempts at most with pauses of 1 s and 2 s before the second and third, because the Julibrot 1.3.0 pre-tag release-tree check saw every Four Kings host time out inside a single window and every one answered on a rerun 1.3 s later, so a blip shorter than a probe reported that a site whose hosts were working had none; the pauses outlast that blip because a retry inside the window that failed would only sample it twice, a host that answers on another protocol is taken at its word the first time, and the reported line names the attempt that carried a host or, where none did, what each attempt reported (`d9ed49e`).
Every live page now loads its bundle through one shared loader, with server discovery starting beside the download from the catalog's preliminary protocol and an empty or disproved ranking repeated after the bundle supplies the authoritative protocol. With working loader rules the retry runs in the background, is exposed as `hostSettled` and reports accepted host events after ready, so practice is not held behind a second probe window; a degraded load has no event channel and therefore waits for the retry before returning it. Arena and Fire Racer keep their chips, protocol and launch state on the latest result, Four Kings keeps its protocol line and launch state current, and League re-enables its online buttons when the late ranking arrives. Download progress starts from an unencoded `Content-Length` without waiting for the catalog, upgrades to the decoded bundle size the deploy stamps into the served catalog if it arrives while the download is open, bounds that lookup at four seconds, abandons any total the decoded body outruns, counts a buffered compile retry and cancels the replaced stream; engine-init errors retain the init phase, eager background rejections are observed, and browser events omit fields that do not apply through one shared camel-case key table (`633f42af`, `8c85007b`). The deploy builds and ships the loader once, stamps one or more imports in exactly six distinct live game pages, checks loader resolution across every live page including labs, names any missing path and refuses a mismatched count (`633bf191`). Arena takes the shared path (`7bc56355`); Four Kings joins the shared host rule while Fire Racer and League keep their current host state live (`4df497f6`); End Game and what is this? use the same download and start reporting without discovery (`61b90509`). The affected series advance together so those shipped bytes have new release identities (`b7154607`).
...
The Linux host tooling now runs League as a fourth managed game rather than leaving it to be started by hand: `deploy/host.sh` carries it through the same table the other three go through, on `EMBER_LEAGUE_PORT` (7783), with `league-server` and `league-probe` joining the prebuilt product set and `league_proto` joining the stamp a node with no checkout answers from, and `deploy/ship-host.sh` builds, stages and probes both so a shipper and the host it ships to agree on what a complete directory is. `deploy/host-tick.sh` enumerates the fourth game too: a dead league server was already caught through `update`'s own loop, but a dead league tunnel was repaired by nothing and the host went on republishing an address that had stopped answering. League is started with `--name` like Kings, its bind address is positional, and its probe is built from the `wsprobe` example its own crate declares and renamed to the per-game name; the loopback, public and `check` probes take distinct lobby names because the public probe retries one name for up to two minutes and a run killed mid-lobby would otherwise refuse the next attempt. The host suites cover the fourth game the way they cover Kings, including the real loopback run that builds and probes every server, the prebuilt stamp-field refusal that gates the upgrade path and a timer case that fails if the tick stops enumerating a game, and `docs/hosts.md` records the four-game host beside the 2026-09-11 hand recovery that made the gap visible (`14b37860`, `120007a6`, `9f7b7a9b`, `8a428059`, `97104139`).
