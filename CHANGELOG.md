...
Julibrot now renders its first production WGSL template while creating the shade pipeline, deriving the real palette layout, enum values and binding slots from the CPU-side Rust items, validating the result with Naga and pinning the unchanged shader bytes outside those rendered declarations (`a5d81b2a`).
