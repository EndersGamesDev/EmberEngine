...
This crate owns the load's rules and nothing else: the phase machine, the download arithmetic, the stall test and the one line each event carries. It holds no socket, opens no request and touches no DOM. `web/loader.ts.j2` is the checked browser input and output — discovery through `web/hosts.ts.j2`, the counting fetch, the compile, and the call into the game's own wasm-bindgen glue — and its emitted module drives this machine with the clock as it goes.
...
Every call carries the page's own clock instead of reading one. That is what lets the module emitted from `web/loader.ts.j2` start the fetch and discovery in the page's first tick and replay the drive calls afterwards, in order and with their real timestamps, once this module has instantiated: the loader's own arrival never delays the work it reports on.
