...
`wasm.rs` is the only file that knows about a browser: it is the `cfg(target_arch = "wasm32")` surface `web/loader.js` drives, and it converts an event into a plain object. Everything else builds and is tested natively.
