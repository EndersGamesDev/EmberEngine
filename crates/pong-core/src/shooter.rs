...
/// Melee reach from the attacker's centre, before the target's own radius is added.
///
...
mod tests {
    ...
    #[test]
    // The first assertion compares two constants on purpose: it is the
    // balance guard itself, meant to fail the suite the moment HEAD_H drifts
    // above the level-fire line, and clippy's objection to constant
    // assertions does not apply to a guard whose whole job is to be one.
