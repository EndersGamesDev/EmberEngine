...
pub struct PageFacts<'a> {
    ...
    pub reference_verification: Option<&'static str>,
    pub consumed_word_error_ulps: Option<u32>,
    pub reference_precision_escalations: Option<u32>,
    pub owner_epoch: u64,
...
impl<'a> PageFacts<'a> {
    ...
    pub fn snapshot(app: &'a App) -> Self {
        ...
        Self {
            ...
            glitch_pixel_count: present.glitch_pixel_count,
            ...
            scene_fence_callback_observation_wait_ms: present
                .last_scene
...
fn newest_measurement(
    ...
    match (scene, warp) {
        (Some(scene), Some(warp)) => {
            Some(if scene.completion_sequence >= warp.completion_sequence {
                scene
            } else {
                warp
            })
