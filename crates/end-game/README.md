...
Version 1.0.0, a single-player dark fantasy dungeon built on Ember. The chapter begins in a prison cell and ends after the player retrieves the greatsword and breaks the far gate's chain. Rules and material physics live in `end-game-core`; this client owns scene construction, input translation, the camera and presentation.

Build the browser game with `powershell -File tools/end-game/build.ps1` from the repository root, then serve `web/` and open `games/end-game/v1/`. Run the native client with `cargo run -p end-game --bin end-game-app`. Builds run at Idle priority through the helper. Native player launches take focus; unattended captures use `EMBER_CAPTURE_PATH` and disable activation and input capture.
