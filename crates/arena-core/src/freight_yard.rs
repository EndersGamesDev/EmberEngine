...
mod tests {
    ...
    /// The frozen v17 Trench City, written once from `Level::trench_city()`
    /// with `serde_json::to_string_pretty` and checked in, so that the v18
    /// changes to the sim's types (`Cover::Loot`, the map module) can be
    /// shown to leave the map they were not meant to touch exactly alone.
    const TRENCH_CITY_V17: &str = include_str!("../tests/fixtures/trench-city-v17.json");
    ...
    fn a_v13_json_level_still_decodes_without_loot() {
        // The frozen v17 map: every kind it names predates `Cover::Loot`,
        // and a sim built from it arms no block.
        assert!(!TRENCH_CITY_V17.contains("Loot"));
        let old: Level = serde_json::from_str(TRENCH_CITY_V17).unwrap();
    ...
    // 15
    #[test]
    fn trench_city_is_unchanged_by_v18() {
        let frozen: Level = serde_json::from_str(TRENCH_CITY_V17).unwrap();
