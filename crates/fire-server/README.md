...
`fire-server` is the authoritative protocol-1 race host: connection threads own WebSockets and bounded outbound queues, while one hub thread owns every lobby and advances `fire-core::Race` at 60 Hz.

Fire clients send control intents rather than positions, and the server broadcasts authoritative car, lap, roster, and host state used for client reconciliation.
