...
//! the kings/fire `NetChan`, over `league_core::proto`.
//!
...
//! with rustls so `league-app ws://...` and `wss://...` both work.
//!
...
//! Neither may depend on the game loop — the loop runs on
//! `requestAnimationFrame`, and a hidden tab gets no frames. Duplicating
//! this with kings and fire is a known backlog line (lift to a shared crate).

use std::collections::VecDeque;
use std::time::Duration;

use league_core::proto::{C2S, CLIENT_PING_SECS, PROTO_VERSION, S2C};
...
fn sendable(msg: &C2S) -> bool {
    if matches!(msg, C2S::Hello { .. }) {
        tracing::warn!("league net: Hello is sent by the socket itself; ignoring the game's");
        return false;
    }
    true
}
...
mod imp {
    ...
    impl Net {
        /// Open the socket. It greets the server with `handle` as soon as
        /// it is open and pings on a timer from then on; the game sends
        /// nothing until `Welcome` arrives.
        ///
        /// # Errors
        ///
        ...
        pub fn connect(url: &str, handle: &str) -> Result<Self, String> {
            ...
            let on_open = Closure::<dyn FnMut(web_sys::Event)>::new(move |_| {
                ...
                if ws_open.send_with_str(&hello).is_err() {
                    tracing::warn!("league net: Hello failed to send on open");
                }
            });
            ...
            // The keepalive runs on a JS interval, NOT the frame loop, so
            // it keeps going in a hidden tab. It sends only while open.
            ...
            if let Some(win) = web_sys::window() {
                ...
                let cb = Closure::<dyn FnMut()>::new(move || {
                    ...
                    if ws_ping.send_with_str(&ping_text(n)).is_err() {
                        tracing::warn!("league net: Ping failed to send");
                    }
                });
                ...
                match win.set_interval_with_callback_and_timeout_and_arguments_0(
                    ...
                    Err(_) => {
                        tracing::warn!("league net: no keepalive timer; the seat will time out")
                    }
                }
                keepalive = Some(cb);
            }
        ...
        pub fn send(&self, msg: &C2S) {
            ...
            {
                tracing::warn!("league net: send failed, the socket is not open");
            }
        }
...
mod imp {
    ...
    impl Net {
        /// Connect in a background reader thread; the thread owns the one
        /// Hello and the keepalive, so the game loop can stop and the seat
        /// survives.
        ///
        /// # Errors
        ///
                    ...
                    loop {
                        ...
                        match ws.read() {
                            Ok(Message::Text(t)) => match serde_json::from_str::<S2C>(&t) {
                                ...
                                Err(e) => {
                                    tracing::warn!("league net: undecodable frame ({e}): {t}");
                                }
                            },
                            Ok(Message::Close(_)) => {
                            ...
                            Err(tungstenite::Error::Io(e))
                                if league_core::proto::is_transient_read(&e) => {}
                            Err(e) => {
                                tracing::warn!("league net: reader thread exiting: {e}");
                                set_status(&st, Status::Closed(e.to_string()));
                                return;
                            }
                        }
