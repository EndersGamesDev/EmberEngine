...
pub fn print_text_without_trailing_newline<F>(t: &str, f: &mut F, state: &State<'_>) -> fmt::Result
where
    F: fmt::Write,
{
...
pub fn list_item_padding_of(l: Option<u64>) -> Cow<'static, str> {
    l.map_or_else(
        || "  ".into(),
        |n| {
            format!("{n}. ")
...
pub fn close_link<F>(uri: &str, title: &str, f: &mut F, link_type: LinkType) -> fmt::Result
where
    F: fmt::Write,
{
    let needs_brackets = {
...
pub struct EscapeLinkLabel<'a>(pub &'a str);
...
pub fn escape_special_characters<'a>(
    t: &'a str,
    state: &State<'a>,
    options: &Options<'a>,
) -> Cow<'a, str> {
    ...
    let ends_with_special = (state.next_is_link_like && t.ends_with('!'))
        || (state.current_heading.is_some() && t.ends_with('#'));
    ...
    if first_special || ends_with_special || table_contains_pipe {
        ...
        for (i, c) in t.char_indices() {
            if (i == 0 && first_special)
                || (i == t.len() - 1 && ends_with_special)
                || (c == '|' && table_contains_pipe)
            {
                s.push('\\');
            }
            s.push(c);
...
pub fn max_consecutive_chars(text: &str, search: char) -> usize {
...
mod max_consecutive_chars {
    ...
    /// ´test:unit:happens-in-the-entire-string´
    #[test]
...
#[derive(Debug)]
pub struct Repeated<T>(pub T, pub usize);

impl<T: fmt::Display> fmt::Display for Repeated<T> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let Self(content, count) = self;
