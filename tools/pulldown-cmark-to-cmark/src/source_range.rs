use super::{Borrow, Error, Event, Options, Range, State, cmark_resume_one_event, fmt};

/// Serialize a stream of [pulldown-cmark-Events][Event] while preserving the escape characters in
/// `source`.
///
/// Each input [`Event`] is accompanied by an optional [`Range`] that maps it back to `source`.
///
/// Unlike [`cmark_resume_with_options`](super::cmark_resume_with_options), which always escapes
/// Markdown special characters like `#` or `[`, this function only escapes a special character
/// if it is escaped in `source`.
///
/// 1. **source**
...
/// # Errors
///
/// Returns an error if writing to the formatter fails or if the [`Event`] stream iterated over by
/// `event_and_ranges` cannot be produced by deserializing valid Markdown. Each failure mode
/// corresponds to one of [`Error`]'s variants.
...
/// produced by deserializing valid Markdown.
