...
const root=process.cwd(),[url,version,commit,name='dusky-osprey']=process.argv.slice(2),started=Date.now();
...
console.log(JSON.stringify({commit:text(wt,'rev-parse','HEAD'),url,elapsedSeconds:(Date.now()-started)/1000,worktree:wt}));
