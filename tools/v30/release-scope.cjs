...
const ENTRY = 'games/arena/v30';
const PREVIOUS = 'games/arena/v29';
const PROTO = 23;
...
function launcher(previous) {
  ...
  assert.equal(previous.split(from).length, 2, 'Expected exactly one v29 launcher fallback; review concurrent launcher changes');
  assert(!previous.includes(to), 'v30 launcher fallback already exists');
...
function catalog(previous, source) {
  ...
  assert.equal(before[0].path, `${PREVIOUS}/`, 'Expected v29 currently live');
  assert.equal(before[0].proto, 22, 'Expected v29 protocol22');
  ...
  assert.equal(after[0].v, 'v30');
  assert.equal(after[0].path, `${ENTRY}/`);
  assert.equal(after[0].proto, PROTO);
  assert(!oldArena.versions.some(version => version.path === `${ENTRY}/`), 'v30 already exists; never overwrite an archived release');
  assert.equal(newArena.versions.filter(version => version.path === `${ENTRY}/`).length, 1, 'Duplicate v30 entry');
  assert.deepEqual(newArena.versions.filter(version => version.path !== `${ENTRY}/`),
    ...
    'Archived Arena metadata must be unchanged except retiring v29');
