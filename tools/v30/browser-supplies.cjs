// Real Killshot v30 WASM + four authored rendering-only supply viewpoints.
// Supply availability and roster snapshots are synthetic; NO pickup/gameplay proof.
...
const output = path.resolve(process.env.EMBER_QA_OUTPUT || 'target/killshot-supplies');
const started = Date.now();
const map = 'harbor', arenaHalf = 48, proto = 23;
// Authored capture fixtures, not geometry or spawn-allocation verification.
// Gameplay belongs to the separately gated real-server network tests.
const spawns = [[-37, 42], [-37, -42], [-13, 42], [-13, -42], [11, 42], [11, -42], [35, 42], [35, -42]];
// These are the exact Level::harbor order/coordinates, not generated props.
const supplySpawns = [
  { pos: [-36, -12], kind: 'Health' }, { pos: [-36, 12], kind: 'Health' },
  { pos: [36, -12], kind: 'Health' }, { pos: [36, 12], kind: 'Health' },
  { pos: [-8, -18], kind: 'Ammo' }, { pos: [8, 18], kind: 'Ammo' },
  { pos: [8, -18], kind: 'Ammo' }, { pos: [-8, 18], kind: 'Ammo' },
];
const levelSource = fs.readFileSync(path.resolve('crates/arena-core/src/harbor.rs'), 'utf8');
const measuredSpawns = [...levelSource.matchAll(/SupplySpawn\s*\{\s*pos:\s*\[(-?[\d.]+),\s*(-?[\d.]+)\],\s*kind:\s*SupplyKind::(Health|Ammo)/g)]
  .map(match => ({ pos: [Number(match[1]), Number(match[2])], kind: match[3] }));
assert.deepEqual(measuredSpawns, supplySpawns, 'Harbor supply slots changed; re-author the fixture against the real level');
const viewpoints = [
  { name: 'health-west-close', supply: 0, eye: [-36.9, 1.45, -14.2], target: [-36, 0.32, -12] },
  { name: 'health-quay-context', supply: 3, eye: [34.5, 1.45, 15.0], target: [36, 0.32, 12] },
  { name: 'ammo-south-close', supply: 4, eye: [-9.1, 1.45, -20.2], target: [-8, 0.32, -18] },
  { name: 'ammo-north-context', supply: 5, eye: [6.2, 1.45, 21.0], target: [8, 0.32, 18] },
];
...
const result = { purpose: 'Real v30/proto23 WASM rendering-only supply fixtures. Eight supply flags are injected true in exact Level order; no collection, health/ammo replenishment, collision, multiplayer or performance proof',
  webRoot, output, map, arenaHalf, expectedProtocol: proto, fixtureSpawns: spawns, supplySpawns, syntheticAvailability: Array(8).fill(true),
...
const html = '<!doctype html><meta charset="utf-8"><title>Killshot supply rendering fixture</title>'
  ...
  + '<div id="ember-root"><span id="loading">Loading real Killshot supply geometry</span></div><div id="status"></div><div id="scoreboard"></div>';
function state() {
  const players = spawns.map(([x, z], id) => ({ id, x, z, y: 0, vy: 0,
    ...
    reloading: false, reload_remaining: 0, inventory: Array.from({ length: 9 }, (_, slot) => ({ weapon: slot === id % 7 ? slot + 1 : 0, ammo: slot === id % 7 ? [6,30,30,30,6,5,1][slot] : 0, reserve: slot === id % 7 ? 60 : 0 })), ads_fraction: 0, spread: 0.026, recoil_bloom: 0, deaths: 0,
  ...
  return { t: 'state', tick: ++tick, players, bullets: [], pads: [], loot: [], supplies: supplySpawns.map(() => true), team_score: [0, 0], hill: 255, round_pause: 0 };
}
...
async function main() {
  ...
  await page.routeWebSocket('**', ws => {
    if (!ws.url().endsWith('/supplies-fixture')) { result.errors.push('Blocked unexpected WebSocket'); ws.close(); return; }
    socket = ws;
    ws.onMessage(data => {
      ...
      if (m.t === 'hello') {
        ...
        ws.send(JSON.stringify({ t: 'game_joined', id: 0, seed: 12, arena_half: arenaHalf, map, mode: 'ffa', loadout: 'classic', starting_weapon: 1,
  ...
  await page.evaluate(async origin => {
    ...
    arena.start_online(JSON.stringify({ url: origin.replace('http:', 'ws:') + '/supplies-fixture',
      action: 'create', lobby: 'supplies-fixture', handle: 'harbor-camera', map: 'harbor', mode: 'ffa', loadout: 'classic', starting_weapon: 1 }));
  }, origin);
  ...
  assert.equal(result.protocol, proto, 'Build the current protocol23 WASM, not the previous release');
  ...
  for (const view of views) {
    ...
    assert.deepEqual(detail.packet.supplies, Array(8).fill(true), 'Supply availability must match all eight real Level slots');
    assert.deepEqual([view.target[0], view.target[2]], supplySpawns[view.supply].pos, 'View does not target its real supply coordinate');
  ...
  await gallery.setContent('<style>body{margin:0;background:#20252b;color:white;font:16px sans-serif}main{display:grid;grid-template-columns:1fr 1fr;gap:8px;padding:8px}figure{margin:0}img{display:block;width:100%}figcaption{padding:6px}</style>'
    + `<h3>${shadowDiagnostic ? 'DIAGNOSTIC ' + shadowDiagnostic + ' · ' : ''}Killshot v30 supplies · real WASM / synthetic availability rendering-only</h3><main>`
  ...
  await gallery.screenshot({ path: path.join(output, 'sheet-supplies.png'), fullPage: true });
  if (shadowDiagnostic) {
