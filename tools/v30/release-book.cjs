...
async function readyHost(book, expected, options = {}) {
  ...
  const probe = options.probe || (url => probeHost(url, { proto: 0, timeoutMs: 12000, handle: 'v30-release-readonly' }));
  ...
  const results = await Promise.all(candidates.map(async host => {
    try {
      ...
      assert(result.ok && welcome?.proto === 23, 'Host does not serve protocol23');
  ...
  assert(selected, 'No reachable protocol23 host built from the exact release revision; publish/verify the approved server first');
  return selected;
}

function arenaBook(book, selected, cacheVersion = String(Math.floor(Date.now() / 1000))) {
  const next = { ...book, ws: selected.host.ws, proto: 23, v: cacheVersion };
