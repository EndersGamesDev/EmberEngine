// Actual v30 page + WASM + an owned, private loopback arena-server (protocol 23).
// Run from the repository root AFTER building and wasm-bindgen:
// EMBER_QA_PLAYWRIGHT=<absolute module> node tools/v30/browser-killshot.cjs
...
const output = path.resolve(process.env.EMBER_QA_OUTPUT || path.join(root, 'target', 'killshot-browser'));
const webPort = 8088, gamePort = 7788, proto = 23;
const origin = `http://127.0.0.1:${webPort}`, gameUrl = `ws://127.0.0.1:${gamePort}`;
const entry = `${origin}/games/arena/v30/index.html`;
...
async function startServices() {
  ...
  const versionPkg = path.join(webRoot, 'games', 'arena', 'v30', 'pkg');
  ...
  server = http.createServer((request, response) => {
    ...
    const packagePath = '/games/arena/v30/pkg/';
...
async function joinPrivateMatch() {
  ...
  check(details.welcome.proto === proto && details.gl && details.joined, 'actual WASM joins owned protocol23 server and draws WebGL2', details);
...
async function remappedPlay(baseLook) {
  ...
  'actual authoritative snapshots carry finite protocol23 parkour state');
...
async function createLoadoutMatch(weapon, loadout = 'custom') {
  const credentials = { lobby: `v30-${weapon}-${crypto.randomUUID().slice(0, 8)}`, password: crypto.randomUUID() };
  await page.reload();
  await page.waitForFunction(() => window.__qa.welcome, null, { timeout: 60000 });
  ...
  check(state.inventory.length === 9 && state.inventory[0].weapon === 1
    && state.inventory[expected - 1].weapon === expected && state.inventory[7].weapon === 0 && state.inventory[8].weapon === 0,
