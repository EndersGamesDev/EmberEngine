...
if (!url || !/^wss?:\/\//.test(url)) throw new Error('Usage: node tools/v30/network-killshot.cjs ws[s]://host');
const proto = 23;
const started = Date.now();
const output = path.resolve(process.env.EMBER_QA_OUTPUT || 'target/killshot-network');
...
class Peer {
  constructor(handle, version = proto) {
    ...
    this.seq = 0; this.states = []; this.observed = []; this.phase = null; this.closed = false;
...
async function main() {
  ...
  const old = new Peer('harbor-old-version', 22); old.expectedError = true;
