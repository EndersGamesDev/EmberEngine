...
const book = () => ({ proto: 22, ws: 'wss://old.example', v: 'old', fire_proto: 2,
  fire_ws: 'wss://fire.example', kings_ws: 'wss://kings.example', unknown: { keep: true },
  hosts: [{ name: 'dusky-osprey', ws: 'wss://old.example', proto: 22, fire_ws: 'wss://fire.example', fire_proto: 2 }],
  mirrors: [{ name: 'knecht24', url: 'https://mirror.example/host.json' }] });
const mirror = { name: 'knecht24', ws: 'wss://new.example', proto: 23, version: 'r1490', commit: 'aaaaaaa',
  fire_ws: 'wss://mirror-fire.example', kings_ws: 'wss://mirror-kings.example' };
const options = (overrides = {}) => ({ fetchEntry: async () => mirror,
  probe: async url => url === mirror.ws ? { ok: true, welcome: { host: mirror.name, proto: 23, commit: 'aaaaaaa', version: 'r1490' } } : { ok: false },
...
test('mirror-only transition changes only legacy Arena keys and never masks future mirror rotation', async () => {
  ...
  assert.deepEqual(next, { ...before, proto: 23, ws: mirror.ws, v: 'new' });
  ...
  const second = await readyHost(next, expected, options({ fetchEntry: async () => rotated,
    probe: async url => ({ ok: url === rotated.ws, welcome: { host: mirror.name, proto: 23, version: 'r1490', commit: 'aaaaaaa' } }) }));
...
test('live Welcome outranks stale listed protocol and preserves same-host peer fields', async () => {
  const original = book(); original.hosts.push({ ...mirror, proto: 22, commit: 'bbbbbbb', extra: 'keep' });
...
test('bound mirror cannot override a listed host of the same name', async () => {
  const original = book(); original.hosts.push({ name: mirror.name, ws: 'wss://stale.example', proto: 22 });
...
test('wrong live identity, protocol, build version, short/fake/ambiguous commit are refused', async () => {
  for (const patch of [{ host: 'wrong-host' }, { proto: 22 }, { version: 'r1489' }, { commit: 'a' }, { commit: 'not-a-sha' }]) {
    await assert.rejects(readyHost(book(), expected, options({ probe: async () => ({ ok: true,
      welcome: { host: mirror.name, proto: 23, version: 'r1490', commit: 'aaaaaaa', ...patch } }) })), /No reachable/);
  }
