# Server-only owner for the approved Killshot v30 update; no tunnel management.
[CmdletBinding()]
param([Parameter(Mandatory=$true)][ValidatePattern('^[A-Fa-f0-9]{64}$')][string]$ExpectedSha256)
Set-StrictMode -Version Latest
$ErrorActionPreference = 'Stop'
$binary = 'C:/Users/end/dev/ember-environment/target/release/arena-server.exe'
$directory = 'C:/Users/end/dev/ember-environment'
$logs = 'C:/Users/end/.ember/arena-local/v30-server-only'
$child = $null
$result = 1
try {
    ...
    $stem = 'arena-v30-{0}-{1}' -f [DateTime]::UtcNow.ToString('yyyyMMddTHHmmssfffffffZ'),$PID
    ...
    Write-Output ('Killshot v30 PID={0}; SHA256={1}' -f $child.Id,$actual)
    $child.WaitForExit()
