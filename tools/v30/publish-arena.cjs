...
async function main() {
  ...
  assert(/PROTO_VERSION: u16 = 23\b/.test(String(read('crates/arena-core/src/proto.rs'))), 'Source protocol must be23');
  ...
  const temporary = fs.mkdtempSync(path.join(os.tmpdir(), 'ember-v30-pages-'));
  ...
  const frozen = ['games/fire', 'games/kings', 'games/what-is-this', 'labs', 'games/arena/v29'];
