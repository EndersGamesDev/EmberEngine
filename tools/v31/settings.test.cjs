 ...
 * Run: node --experimental-vm-modules tools/v31/settings.test.cjs
 */
'use strict';
...
const pageRoot = path.resolve(__dirname, '../../web/games/arena/v31');
const html = fs.readFileSync(path.join(pageRoot, 'index.html'), 'utf8');
const settings = require(path.join(pageRoot, 'settings.js'));
const previousSettings = require(path.resolve(pageRoot, '../v30/settings.js'));
if (!vm.SourceTextModule) {
  throw new Error('Run with node --experimental-vm-modules tools/v31/settings.test.cjs');
}
...
check(() => {
  assert.equal(settings.STORAGE_KEY, previousSettings.STORAGE_KEY, 'v30 and v31 use the same unchanged preferences schema');
  assert.deepEqual(settings.defaults(), previousSettings.defaults());
  const customized = previousSettings.rebind({ ...previousSettings.defaults(), sensitivity: 1.45 }, 'slot8', 'KeyZ');
  assert.deepEqual(settings.validateConfig(customized), customized, 'v30 slot8 remapping and sensitivity are preserved exactly');
});
...
const snapshot = {
  ...
  slots: [1, 2, 3, 4, 5, 6, 7, 8, 0], selected_slot: 3,
};
...
check(() => assert.deepEqual(settings.WEAPONS.map(weapon => weapon.name), ['Empty', 'Sidearm', 'Vityaz', 'AK-47', 'M4', 'Revolver', 'Sniper', 'RPG-7', 'Breach-12']));
check(() => {
  const shotgun = settings.normalizeHud({ ...snapshot, weapon: 8, magazine: 5, reserve: 24, selected_slot: 8, reload_remaining: 1.4, reload_duration: 2.8 });
  assert.equal(shotgun.weapon, 8); assert.equal(shotgun.selected_slot, 8);
  assert.equal(shotgun.magazine, 5); assert.equal(shotgun.reserve, 24); assert.equal(shotgun.reload_progress, 50);
  assert.equal(settings.normalizeHud({ ...snapshot, weapon: 9, slots: [1, 0, 0, 0, 0, 0, 0, 8, 9], selected_slot: 9 }).selected_slot, 0);
});
...
for (let weapon = 1; weapon <= 8; weapon++) {
  check(() => {
...
check(() => assert.match(html, /<option value="8">Breach-12 · Shotgun<\/option>/));
check(() => assert(!/<option value="9">/.test(html)));
