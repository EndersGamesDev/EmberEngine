...
try {
    ...
    node --check web/games/end-game/v2/main.js
    ...
    wasm-bindgen --target web --no-typescript --out-dir web/games/end-game/v2/pkg (Join-Path $artifacts 'wasm32-unknown-unknown/release/end_game.wasm')
    if ($LASTEXITCODE -ne 0) { throw 'End Game WASM bindings failed' }
    New-Item -ItemType Directory -Force -Path web/pkg | Out-Null
    Copy-Item -LiteralPath web/games/end-game/v2/pkg/end_game.js,web/games/end-game/v2/pkg/end_game_bg.wasm -Destination web/pkg
    Write-Output ('WASM bytes: ' + (Get-Item web/games/end-game/v2/pkg/end_game_bg.wasm).Length)
} finally {
    Pop-Location
