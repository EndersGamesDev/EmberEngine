...
try {
    ...
    foreach ($name in @('warden-movement.wav','warden-unlocking.wav','warden-sword.wav','warden-death.wav')) {
        Copy-Item -LiteralPath (Join-Path 'assets/end-game/v6' $name) -Destination (Join-Path 'web/games/end-game/v8' $name)
    }
    cargo test --locked -p end-game-core -p end-game --lib
    if ($LASTEXITCODE -ne 0) { throw 'Dungeon simulation tests failed' }
    node --test tools/end-game/quality.test.mjs tools/end-game/dialogue.test.mjs tools/end-game/guard.test.mjs
    if ($LASTEXITCODE -ne 0) { throw 'Resolution or dialogue tests failed' }
    node --check web/games/end-game/v8/main.js
    if ($LASTEXITCODE -ne 0) { throw 'Game shell syntax failed' }
    node --check web/games/end-game/v8/dialogue.js
    ...
    wasm-bindgen --target web --no-typescript --out-dir web/games/end-game/v8/pkg (Join-Path $artifacts 'wasm32-unknown-unknown/release/end_game.wasm')
    if ($LASTEXITCODE -ne 0) { throw 'End Game WASM bindings failed' }
    New-Item -ItemType Directory -Force -Path web/pkg | Out-Null
    Copy-Item -LiteralPath web/games/end-game/v8/pkg/end_game.js,web/games/end-game/v8/pkg/end_game_bg.wasm -Destination web/pkg
    Write-Output ('WASM bytes: ' + (Get-Item web/games/end-game/v8/pkg/end_game_bg.wasm).Length)
} finally {
    Pop-Location
