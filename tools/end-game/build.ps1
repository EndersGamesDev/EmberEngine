...
try {
    ...
    foreach ($name in @('warden-movement.wav','warden-unlocking.wav','warden-sword.wav','warden-death.wav')) {
        Copy-Item -LiteralPath (Join-Path 'assets/end-game/v6' $name) -Destination (Join-Path 'web/games/end-game/v11' $name)
    }
    foreach ($name in @('voice-lines.js','boss-intro.wav','boss-phase2.wav','boss-defeat.wav','escape-clue.wav','escape-ending.wav','castle-ambience.wav')) {
        Copy-Item -LiteralPath (Join-Path 'assets/end-game/v10' $name) -Destination (Join-Path 'web/games/end-game/v11' $name)
    }
    ...
    wasm-bindgen --target web --no-typescript --out-dir web/games/end-game/v11/pkg (Join-Path $artifacts 'wasm32-unknown-unknown/release/end_game.wasm')
    if ($LASTEXITCODE -ne 0) { throw 'End Game WASM bindings failed' }
    New-Item -ItemType Directory -Force -Path web/pkg | Out-Null
    Copy-Item -LiteralPath web/games/end-game/v11/pkg/end_game.js,web/games/end-game/v11/pkg/end_game_bg.wasm -Destination web/pkg
    Write-Output ('WASM bytes: ' + (Get-Item web/games/end-game/v11/pkg/end_game_bg.wasm).Length)
} finally {
    Pop-Location
