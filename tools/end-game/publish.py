...
def assemble(source, destination, commit):
    ...
    (page / 'version.json').write_text(json.dumps({'game': 'end-game', 'version': '5.0.0', 'source': commit, 'files': manifest}, indent=2) + '\n', encoding='utf-8', newline='\n')
...
def main():
    ...
    stage = Path(tempfile.mkdtemp(prefix='end-game-v5-pages-')) / 'pages'
    ...
    if args.publish:
        git('commit', '-m', f'feat(end-game): publish the wardens knife from {commit[:12]}', cwd=stage)
