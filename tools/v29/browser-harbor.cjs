...
const output = path.resolve(process.env.EMBER_QA_OUTPUT || 'target/parkour-harbor');
const started = Date.now();
const map = 'harbor', arenaHalf = 48, proto = 22;
...
const viewpoints = [
  { name: 'service-houses', eye: [-11, 12, -31], target: [-26, 2.8, -16] },
...
async function main() {
  ...
  assert.equal(result.protocol, proto, 'Build the current protocol22 WASM, not the previous release');
