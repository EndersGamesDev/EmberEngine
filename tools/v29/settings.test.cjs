 ...
 * Run: node --experimental-vm-modules tools/v29/settings.test.cjs
 */
'use strict';
...
const pageRoot = path.resolve(__dirname, '../../web/games/arena/v29');
const html = fs.readFileSync(path.join(pageRoot, 'index.html'), 'utf8');
const settings = require(path.join(pageRoot, 'settings.js'));
if (!vm.SourceTextModule) {
  throw new Error('Run with node --experimental-vm-modules tools/v29/settings.test.cjs');
}
