...
if (!url || !/^wss?:\/\//.test(url)) throw new Error('Usage: node tools/v29/network-parkour.cjs ws[s]://host');
const proto = 22;
const started = Date.now();
const output = path.resolve(process.env.EMBER_QA_OUTPUT || 'target/parkour-network');
...
async function main() {
  ...
  const old = new Peer('harbor-old-version', 21); old.expectedError = true;
