// Actual v29 page + WASM + an owned, private loopback arena-server (protocol 22).
// Run from the repository root AFTER building and wasm-bindgen:
// EMBER_QA_PLAYWRIGHT=<absolute module> node tools/v29/browser-parkour.cjs
...
const output = path.resolve(process.env.EMBER_QA_OUTPUT || path.join(root, 'target', 'parkour-browser'));
const webPort = 8088, gamePort = 7788, proto = 22;
const origin = `http://127.0.0.1:${webPort}`, gameUrl = `ws://127.0.0.1:${gamePort}`;
const entry = `${origin}/games/arena/v29/index.html`;
...
async function startServices() {
  ...
  const versionPkg = path.join(webRoot, 'games', 'arena', 'v29', 'pkg');
  ...
  server = http.createServer((request, response) => {
    ...
    if (pathname === '/server.json') {
      ...
      response.end(JSON.stringify({ v: 'parkour-private-qa', hosts: [{ name: 'parkour-qa', ws: gameUrl, proto }], mirrors: [] })); return;
    }
    const packagePath = '/games/arena/v29/pkg/';
  ...
  game = spawn(executable, ['--bind', `127.0.0.1:${gamePort}`, '--name', 'parkour-qa'], {
...
function instrument({ gameUrl, proto }) {
  ...
  localStorage.setItem('ember-account', JSON.stringify({ handle: 'parkour-qa', id: 'private-parkour-qa' }));
  Object.defineProperty(navigator, 'gpu', { value: undefined, configurable: true });
  Object.defineProperty(navigator, 'getGamepads', { value: () => [], configurable: true });
  window.__qa = { inputs: [], states: 0, stateFrames: [], shots: 0, joined: null, welcome: null, draws: 0,
...
async function preferencesBeforeMatch() {
  ...
  await page.waitForFunction(() => window.__qa.welcome && document.querySelector('#host-chip').textContent.includes('parkour-qa'), null, { timeout: 60000 });
...
async function joinPrivateMatch() {
  ...
  check(details.welcome.proto === proto && details.gl && details.joined, 'actual WASM joins owned protocol22 server and draws WebGL2', details);
...
async function remappedPlay(baseLook) {
  ...
  'actual authoritative snapshots carry finite protocol22 parkour state');
