...
async function readyHost(book, expected, options = {}) {
  ...
  const probe = options.probe || (url => probeHost(url, { proto: 0, timeoutMs: 12000, handle: 'v29-release-readonly' }));
  ...
  const results = await Promise.all(candidates.map(async host => {
    try {
      ...
      assert(result.ok && welcome?.proto === 22, 'Host does not serve protocol22');
  ...
  assert(selected, 'No reachable protocol22 host built from the exact release revision; wait for automatic deployment');
  return selected;
}

function arenaBook(book, selected, cacheVersion = String(Math.floor(Date.now() / 1000))) {
  const next = { ...book, ws: selected.host.ws, proto: 22, v: cacheVersion };
