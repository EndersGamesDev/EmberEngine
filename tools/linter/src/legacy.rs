...
/// (´[EMBER-dec:migration:superseded-forms]´); a family's reach is its
...
/// (´[EMBER-conv:migration:burn-family]´). So the four letters here are a
...
/// tail (´[EMBER-conv:migration:burn-family]´). Three is the width every record
...
///
/// ´const:emberlinter:unprefixed-record-number-width´ (´[EMBER-alg:const:count]´)
/// ´const:emberlinter:unprefixed-record-number-width-count-3´
...
/// (´[EMBER-conv:migration:burn-one-recognizer]´), under the
/// decision that retires section-number identities and leaves numbering carrying
/// no identity anywhere (´[EMBER-dec:migration:superseded-forms]´). The mark is what
...
/// row.
///
/// ´const:emberlinter:division-locator-sign´ (´[EMBER-alg:const:codepoint]´)
/// ´const:emberlinter:division-locator-sign-codepoint-ua7´
...
impl LegacyRule {
    ...
    /// (´[EMBER-dec:migration:superseded-forms]´). A fifth rule would be a
    ...
    /// (´[EMBER-conv:migration:burn-family]´) reached zero, the retirement was
    ...
    ///
    /// ´const:emberlinter:migration-lint-policy´ (´[EMBER-alg:const:form]´)
    /// ´const:emberlinter:migration-lint-policy-form-x7cadc3ef´
...
/// (´[EMBER-conv:migration:burn-family]´). The typewriter quote is the whole of
...
/// occurrence.
///
/// ´const:emberlinter:division-title-quote´ (´[EMBER-alg:const:codepoint]´)
/// ´const:emberlinter:division-title-quote-codepoint-u22´
...
/// (´[EMBER-dec:migration:superseded-forms]´). The value is the numeral alphabet
...
///
/// ´const:emberlinter:locator-numeral-alphabet´ (´[EMBER-alg:const:form]´)
/// ´const:emberlinter:locator-numeral-alphabet-form-x2e7a533a´
