...
ssh -o BatchMode=yes "$REMOTE" \
    'toolbox run -c ember-build bash -lc "source ~/.cargo/env && cd ~/ember-src && cargo build --release -p arena-server"'
...
if [ -z "$MANAGED" ]; then
    ...
    ssh -o BatchMode=yes -f "$REMOTE" \
        "RUST_LOG=info nohup ~/ember-src/target/release/arena-server --bind $BIND >> ~/pong-server.log 2>&1 &"
    sleep 2
