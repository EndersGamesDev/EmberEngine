...
`develop.json` replaces ruleset `22736795` with `gh api --method PUT repos/EndersGamesDev/EmberEngine/rulesets/22736795 --input deploy/rulesets/develop.json`. `main-authorization.json` replaces ruleset `22736890` after `main` exists. The remaining four files create separate rulesets because integrity rules carry no bypass actor while authorization rules grant only the narrowly required creation or update path.
