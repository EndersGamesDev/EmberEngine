...
# `cargo` and `sleep` on PATH — so they are listed rather than globbed.
FILES="$(find "$DEPLOY" -name '*.sh' -print; ls "$HERE"/shims/ssh "$HERE"/shims/scp "$HERE"/shims/cargo "$HERE"/shims/sleep)"
