...
if [ -s "$STAMP" ] && grep -q 'league_proto' "$STAMP"; then
...
if "$PY" "$STAMP" "$TMP/pages-broken.json" 12 1 1 3 >/dev/null 2>"$TMP/pages-err"; then
...
if "$PY" "$STAMP" "$TMP/pages-list.json" 12 1 1 3 >/dev/null 2>&1; then
...
json.dump({"hosts": [{"name": "amber-otter", "ws": "wss://a.example", "proto": 12}],
          ...
          open(sys.argv[1], "w", encoding="utf-8"), indent=2)
EOF
"$PY" "$STAMP" "$TMP/pages-good.json" 12 1 1 3 >/dev/null
...
is "$(jget "$TMP/pages-good.json" 'd["kings_proto"]')" "1" "including the third game's, since main added it"
is "$(jget "$TMP/pages-good.json" 'd["league_proto"]')" "3" "including League's independent protocol"
is "$(ls "$TMP"/*.tmp 2>/dev/null | wc -l | tr -d ' ')" "0" "written through a temp file that is renamed away"
: > "$TMP/pages-empty.json"
"$PY" "$STAMP" "$TMP/pages-empty.json" 12 1 1 3 >/dev/null
