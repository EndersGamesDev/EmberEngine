...
The rejected first implementation is a full GPU scan of all `B` cells. It avoids CPU upload only after paying `B` classifications, needs capacity for the worst output or a count-and-prefix pass, and makes the weakest fallback run a different algorithm. CPU slicing also gives the proposed editor preview and collision visualizer, headless tests, and renderer the same inspectable 3D artifact.
...
A relative 4D rotation lies in the six-dimensional `SO(4)`. Its three generators wholly within the current hyperplane form the `SO(3)` stabilizer of `n` and only change the 3D camera; its other three generators rotate a basis direction with `n` and therefore change which world points satisfy the slice equation. The simulation increments a stable plane revision whenever canonical `n` or `h` changes: an unchanged revision permits slice reuse, while a changed revision re-queries the 4D BVH, rebuilds affected cut geometry, and renders a new `SceneFrame`. Mathematical tests still assert exact `n`/`h` preservation for the embedded `SO(3)` subgroup, but runtime cache validity never depends on approximate float equality.
...
Physics consumes the authoritative 4D solids and their 4D poses. The visible slice is neither a collider nor a broad-phase proxy: two bodies may collide outside the current hyperplane, and a body that has no visible cross-section may still carry momentum into it on a later tick. The current arena authority is already documented in code as a pure deterministic fixed-60-Hz simulation (`crates/pong-core/src/shooter.rs:1-5`); this design preserves that clock while replacing every spatial assumption underneath it.
...
Composition is componentwise with order preserved: applying `b` and then `a` yields `(a_L b_L, a_R b_R)`. A pinned self-dual/anti-self-dual basis maps the six plane rates to imaginary quaternions `(u_L,u_R)` by the defining infinitesimal action `Ωx = (u_L x - x u_R)/2`. For body-space rates the fixed-step update is `(q_L exp(dt u_L/2), q_R exp(dt u_R/2))`; for world-space rates it is `(exp(dt u_L/2) q_L, exp(dt u_R/2) q_R)`. The basis signs, multiplication side, and an explicit noncommuting test case are part of the math contract, because identity-only tests cannot catch reversed composition.
...
Physics is a research-sized implementation risk. GJK's dimension-independent core is established, but a robust 4D EPA, deterministic feature clipping, reduction of 0D-to-3D contact regions, three-dimensional friction, and stable stacks under six rotational freedoms remain research-sized implementation problems. More contacts and a ten-component body velocity enlarge islands and solver cost, while the strict scalar path needed for native/wasm bit equality may be materially slower than hardware-default floating point.
...
Three-dimensional simulation can be an exact invariant subspace of the 4D physics: constrain every position and linear velocity to `w = 0`, every force to zero `w` component, every orientation to the `SO(3)` subgroup that fixes `e_w`, and every angular bivector to `(xy,xz,yz)`. Under those constraints the extra state remains zero and the 4D equations reduce to 3D. That is the compatibility definition and the rollback oracle.

Three-dimensional render geometry is not valid by setting each triangle's `w` to zero. That produces a two-dimensional, zero-thickness set rather than the three-dimensional boundary of a 4D solid, and the chosen one-sided slice correctly makes coplanar cells disappear. A closed 3D solid `A` becomes the 4D slab `A × [-epsilon,+epsilon]`; its central slice is `A`, its boundary is a proper 3-manifold, and `epsilon` is authored physical thickness rather than a numerical hack. Open and nonmanifold assets require repair or an explicitly nonphysical render-only path.

The result is neither a whole-engine rewrite nor a field-width patch. The rasterizer, material shaders, `SceneFrame`, presenter, UI composition, latency histograms, and adaptive-control shell remain downstream 3D machinery; the world asset, pose, slice, collision, constraint, authoring, and wire layers are new or meaningfully replaced. Migration keeps the old authority available until the replacement proves its constrained equivalence.

|Stage|Change|What it buys|Primary risk|
|---|---|---|---|
|0. Mathematical fixtures|Land standalone property tests for Spin(4) composition, 3D-subgroup embedding, inertia energy, simplex orientation, slice sign masks, and pose-codec error; define the proposed cooked asset schema without changing runtime|Turns this document's equations into executable oracles before APIs harden|Synthetic fixtures can pass while real content remains unworkable|
|1. Extruded rendering|Cook a small set of closed current assets into finite `w` slabs, run the CPU slicer at fixed `w = 0`, and feed its triangles into the existing rasterizer contract|Validates representation, winding, attributes, caching, cell counts, and visual parity while current physics remains authoritative|Asset repair cost and slice throughput may fail before any 4D gameplay exists|
|2. State spine|Add proposed 4D pose/momentum state, slice stamps, level-4 identity, rollback snapshots, and a versioned pose codec, but enforce the 3D invariant and continue using current gameplay semantics|Creates the authority/render/wire seams and measures the 25-byte pose without changing outcomes|Dual representations can drift; every adapter needs round-trip tests and an expiry plan|
|3. Shadow physics|Run 4D broad phase, GJK/EPA, contacts, and constraints beside the current 3D result on the constrained corpus; record but do not apply differences|Finds collision, inertia, ordering, and determinism faults against an existing oracle|Double simulation cost limits scene size, and matching simple boxes can conceal general-contact failures|
|4. Constrained 4D authority|Make the 4D solver authoritative while the `w = 0` and `SO(3)` constraints remain; keep a release switch back to the prior 3D authority and require native/wasm replay equality|Proves rollback, networking, stacking, and performance in production-shaped 3D play before new degrees of freedom obscure regressions|Small numerical differences can change familiar gameplay even when the subspace is mathematically equivalent|
|5. Phase movement|Permit `w` velocity and hyperplane offset while holding the hyperplane orientation fixed; ship slabs, near-slice cues, phase collision tests, and the editor's `w` extent controls|Delivers the first irreducibly 4D mechanic with only one new player-controlled slice degree and a fixed basis|Topology changes, invisible contacts, and non-warpable phase latency may already fail comprehension or weak targets|
|6. Plane tilt and full rotation|Enable the three slice-tilt controls, full Spin(4) body orientation, arbitrary 4D primitives/sweeps, and dynamic slice LOD|Completes the 4D-first model rather than leaving a stack of parallel 3D worlds|This combines the hardest authoring, ATW, physics, determinism, and comprehension problems; it ships only after all four gates in §8 pass|
