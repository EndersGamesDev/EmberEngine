...
- The six transient-read consumers selected for this bite had diverged: complete implementations at `crates/fire-core/src/proto.rs`, `crates/kings-core/src/proto.rs`, and `crates/league-core/src/proto.rs`; inline Arena checks that recognized Windows error 997, `WouldBlock`, and `TimedOut` but omitted `Interrupted`; and the defective `ember-server` copy that recognized only `WouldBlock` and `TimedOut`.
