...
|Lint allows|364 repository-wide attributes: 30 removable, 246 justified, and 88 hiding defects|Remove the 30 stale attributes, then split or justify the remaining active suppressions|
...
Classification codes are `J` for justified, `R` for removable because the lint no longer fires, and `D` for a real maintainability or type-boundary defect hidden by the suppression. The measured count is J 246, R 30, D 88. The native forced-lint run emitted 2,267 source diagnostics, 85 generated-warning summaries, and three standalone manifest unused-dependency diagnostics. The seven-crate wasm forced-lint run emitted 1,269 source diagnostics across 14 generated-warning summaries. Matching each attribute's lint and lexical scope against both applicable-target outputs proves the R set; the J-versus-D decision for every reproduced lint then follows from the invariant or defect described below.
...
- `crates/league-core/src/lib.rs`: J 4, J 5, J 6, D 11, R 16.
...
- `tools/cli-common/src/lib.rs`: D 624, J 792; `tools/cli-common/src/publication.rs`: R 159; `tools/cli-common/src/tests/mod.rs`: R 113; `tools/cli-common/tests/public_api.rs`: R 38.
...
The 30 R attributes are: build-script output suppressions at `crates/arena-server/build.rs:11`, `crates/fire-server/build.rs:11`, and `crates/kings-server/build.rs:8`; crate or item lints at `crates/league/src/online_game.rs:104`, `crates/league-core/src/lib.rs:16`, `crates/ember-server/src/lib.rs:10`, `crates/ember-legacy/src/lib.rs:9`, `:29`, `crates/ember-client-net/src/lib.rs:8`, `crates/arena/src/online.rs:8496`, `props.rs:720`, `sound.rs:15`, `:25`, `:571`, `:1324`, `:1598`, `:1711`, and `viewarms.rs:133`; frozen-source lints at `games/what-is-this/v001/src/lib.rs:9`, `games/arena/v008/src/proto.rs:61`, `games/arena/v007/src/proto.rs:53`, `games/arena/v001/src/proto.rs:157`, and `tests/hosted_contract.rs:2`; and tool lints at `tools/linter/src/reference.rs:611`, `:633`, `:751`, `tools/linter/src/token.rs:244`, `tools/cli-common/src/publication.rs:159`, `src/tests/mod.rs:113`, and `tests/public_api.rs:38`. Neither forced run emitted the named lint in the attribute's scope. Oracle: remove these attributes in crate-sized commits and require a clean ordinary Clippy run plus the affected crate tests; frozen-source removals additionally retain the hosted-contract tests.
...
6. Removable allow wave: remove the 30 R attributes listed in section C in one crate-sized commit at a time; value medium-high, risk low; oracle each crate's ordinary Clippy and tests, with native and wasm checks where applicable.
