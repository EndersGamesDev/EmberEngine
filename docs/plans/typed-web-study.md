...
The End Game row initially appeared to expose a deploy defect because v12 `main.js` imports `dialogue.js`, `castle-audio.js`, `castle-ui.js` and `voice-lines.js`, totaling 313 lines, but the assembly's tracked live-directory copy already supplied those four modules and the ten audio files before the typed conversion. The typed pipeline now checks the emitted sibling-import closure explicitly and retains the tracked runtime-asset validation.
...
Every 64-bit integer is a field-level contract rather than a generator default. Fields whose identity is compared or returned unchanged declare `exact` and render as `bigint`; fields with a stated safe-integer engineering bound declare `precise` and render as `number` with that bound in the declaration. Both the derive and generator refuse an undeclared 64-bit field. Every declaration carries a one-line reason at the field explaining why identity matters or why its bound holds, and the commit that introduces or changes it states the justification in its body so review can check the records the derive cannot. This representation metadata does not alter JSON: an exact JSON number that can exceed 2^53 - 1 still needs a token-preserving parser, and a future string transport would be a separate wire change.
...
The `typed web` CI job sets up exact Node 24.20.0, runs `npm ci`, stages the wasm-bindgen declarations needed by converted games, runs the generator check, every TypeScript project, the four suites converted so far and the generated-tree manifest. Later phases add their remaining declarations and suites to the same data-driven staging and emitted runners as they convert them.

`deploy/tests/test-typescript.sh` pins the configs, rejects forbidden handwritten outputs and AST escape hatches, checks generated hashes and compile-fail fixtures and runs each emitted suite by explicit path. `test-pages.sh` proves the assembler copies only the four named emitted browser modules. `deploy/tests/run.sh` lists the TypeScript suite; callers select required execution with `EMBER_TYPED_WEB_REQUIRED=1`.

The lane-6 named-import gate cherry-pick recorded its original `git diff --check` measurement over `cc614f4a` through `bcf0577c` rather than only its immediate parent `4089821c`; its rebased copy is `24dec0cf`.
...
|2|Complete (`d1c679a9`, `24dec0cf`, `f8778f58`, `a89dc0cf`, `74fb87c3`): Fire current page and protocol|2 files/535 script lines measured at `cc614f4a` by `wc -l web/games/fire/v2/{race,garage}.js`; Fire protocol input 615 lines at `cc614f4a`|The smallest live game's templates compile against generated `C2S`, `S2C` and wasm declarations even though socket ownership stays in wasm|Fire core/client tests, generated union oracle, browser race/garage smoke, Pages fixture, size comparison|
|3|Complete (`f93ba07`, `6029a92`): Kings current page|1 HTML block/517 non-blank script lines; Kings protocol input 883 lines; template 961 non-blank lines, +444 before documentation exclusion|The emitted module's direct socket messages, DOM and wasm glue consume generated Kings, loader, host-book and wasm declarations; every server variant is exhaustive|Kings protocol/server tests, rendered Node/browser page smoke, no-inline check, Pages fixture, size comparison|
...
|6|Complete (`9477b00c`, `1fc2c6eb`, `df46e9c4`, `789f16da`): End Game v12's six modules and four tool tests are checked legacy TypeScript templates, with a fifth emitted-main boot suite|6 templates/1,268 lines from 821 source lines at `aa4d344e`; 5 suites/961 lines from 4/511, +447 production and +450 test lines|Audio, media, DOM and wasm glue use generated declarations; the exact six-module import closure is emitted and assembled after live-source copying; the earlier deploy already shipped all six through its tracked directory copy|Five rendered Node suites, End Game core/client tests, DOM-double loader hand-off, import closure, Pages emitted-byte fixture, size comparison|
...
Phase 3 moves 517 non-blank inline script lines into a 961-line non-blank template. After excluding documentation comments from both sides — 44 in the inline body and 24 in the template — the converted-code delta is 464 lines and other production work adds 245, for 709 of the 1,000-line budget. The stamped-module verifier and the mechanism that reads its emitted-module list arrived with phase 2 and are reused unchanged, while the list itself gains the Kings module as an emitted and a stamped entry; this phase adds only the generic page-bootstrap tuple that rewrites a page's own module cache key to the deploy stamp. The inline body was 24,408 raw and 7,795 gzip bytes; the final emitted size is recorded by the lane report. The Kings wasm remains byte-identical.

Phase 6 emitted sizes, measured on a real prebuilt Pages assembly against the `aa4d344e` sources they replace, are `main.js` 32,931 to 44,079 raw bytes and 9,946 to 11,209 gzip bytes, `dialogue.js` 6,693 to 7,672 raw and 2,153 to 1,955 gzip, `castle-audio.js` 1,956 to 2,458 raw and 758 to 751 gzip, `castle-ui.js` 2,076 to 2,190 raw and 852 to 835 gzip, `quality.js` 1,549 to 1,781 raw and 619 to 625 gzip, and `voice-lines.js` 1,027 to 1,061 raw and 475 to 455 gzip: 46,232 to 59,241 raw, a 13,009-byte or 28.1% increase, and 14,803 to 15,830 gzip, a 1,027-byte or 6.9% increase. The raw growth is the runtime narrowing the conversion added, concentrated in `main.js`, which decodes every game-state field the page reads; four of the six modules gzip smaller than their sources because the added guards are repetitive. The End Game wasm and its glue pass through the assembly unchanged at `8ae20a76ad5cfe9c693d5123ef6865869347ab659e1fea27b932a5d9882e59cf` and `01c036bfdd61936c7b4ace677218d5281f2cc991b6fb785f85fcc9ae842a2e5b`, so nothing moved into Rust and the wasm budget is untouched.
...
|Fire current|2 files/535 lines|Move only state duplicated outside the socket-owning wasm; keep garage and presentation DOM in templates if the wasm boundary already owns the protocol|
...
- Apparent live-copy omissions and the Arena v0 rewrite can invalidate migration measurements unless the module graph and frozen manifest are executable checks; inspection during phase 6 established that End Game's tracked live-directory copy already shipped its full module closure.
...
Wide integers use per-field `exact` or bounded `precise` declarations because identity and safely bounded magnitude are different engineering facts. Exact values are `bigint`, precise values are `number`, the generated manifest makes the choice auditable, and missing declarations fail generation. Each field records why identity matters or why its bound holds, and the declaration-changing commit repeats that justification for review. The current numeric JSON transport remains unchanged; only Arena's random game seed is presently produced across the full `u64` range and therefore leaves a concrete token-preserving or string-transport question open.
