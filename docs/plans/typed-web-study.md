...
|Registration derive on each boundary type|Recommended: the compiler expands it on the same enum item and it rejects unsupported Serde shapes; use Serde's attribute parser rather than a second casing table|Records wire direction, nullable `Option`, defaulted-deserialize omission and required serialized fields separately; trait bounds resolve registered nested types|The derive emits the new variant descriptor in the same compile and a variant-count test forces a new wire sample|Impossible when deploy consumes only this invocation's staging output; possible if generated files are checked in or copied from `web/`|
...
For JSON, `Option<X>` means `X | null` when serialized; only a field accepted with `serde(default)` is optional on deserialization, while a defaulted field emitted by Rust remains required, so the generator needs distinct input and output views instead of turning every option into `?`.

The loader branch needs one more ownership repair before generation can be exact: `Event` says non-applicable fields are absent, but `wasm.rs` writes every such property as `null`, converts snake-case Rust names to camel-case keys by hand and returns `JsValue`; either a derived transfer struct must own that lowering or generated wasm-bindgen custom sections must name the actual object shape.
...
|----:|----|----|--------------|---------------------|
|0|Generator foundation: registration derive, descriptors, declaration templates, catalog/book Rust models, wasm-bindgen declarations and configs, plus the wasm-bindgen flag, shim, copy and documentation path|4 protocol inputs/3,252 lines plus loader Rust 3/1,028 at `d19f70c9`; at most 1,200 new lines|One compiler-owned schema renders protocol, ABI and JSON artifacts without checked-in output, including League's intersected `t` and `a` discriminators|Rust unit/oracle tests, deterministic render hashes, real catalog schema validation, `.d.ts` Pages fixture, clean and deliberate-fail `tsc` fixtures|
