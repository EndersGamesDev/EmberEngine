...
|EC-02|Lock boundary|Exclusive ownership moved by transfer, never shared memory; ping-pong buffer pairs; a credit header on the returned buffer; producers shape themselves to it (forward pressure); the same-thread lowering is the cheapest case of one abstraction|PAID|`crates/labs/julibrot/worker`: nine message kinds, four buffers, credit accounting, same-thread twin with a mode-equivalence trace; browser transfer `requires visible replay`; browser transfer seen in the reference browser 2026-09-03 (WebWorker mode, two wasm instances, references applied per navigation)|
...
|EC-06|Kernels|Dialect v2: pure WGSL bodies over heap spans lowered to gather-only fragment passes, rendered into SCRATCH and copied into the heap|PAID|`docs/gpu-heap-lattice.md`; the direct-overlap refusal and the copy path proven by the output-path spike; the escape and scaled-perturbation kernels conformant against the math oracles; the escape and scaled-perturbation kernels seen on WebGL2 at zoom 14.6 with a 4096-step reference; the WebGL2 `ldexp` gap (GLSL ES 3.00) paid and guarded by a naga translation test|
...
|EC-08|Scene|One pass, one depth buffer, no mips, no blend, no culling; a retained plus an in-flight scene texture|PAID|`crates/labs/julibrot/present`; not yet driven by the app's frame loop; seen on screen 2026-09-03 through the app's frame loop, Preview to Final|
|EC-09|Re-projection|Cooperative asynchronous re-projection owns presentation: at every refresh the last completed scene is warped to the latest pose from the hot ring; reference-shift rebasing keeps deep pans smooth|PAID|`docs/atw-first-rendering.md` for the decision; the flat exact homography and the bounded tumbled approximation in `present`; not yet seen on screen; flat and tumbled warps seen on screen for both presets and a hybrid plane; reference-shift rebasing on deep pans not yet measured separately|
...
|---|---|---|
|EO-01|Finish the Julibrot app's frame loop (Phase 3), remove the duplicate `worker_main` placeholder, rebuild the bundle, replay the rapid-input and deep-zoom checklists in the reference browser — DONE 2026-09-03 (`feature/julibrot` f23fcc2)|worker's four integration seams (in flight)|
|EO-02|Visible replay of the Julibrot: Mandelbrot and Julia presets, a hybrid slice, a zoom past 42 digits under scaled perturbation, continuous zoom under re-projection, the worker's transfer facts — DONE for presets, tumbled, palettes, anchored zoom, perturbation at zoom 14.6 and cap changes; OPEN for a hybrid-slice replay and a zoom past 42 digits with structure (18 digits reached only inside the set)|EO-01|
|EO-03|Merge `feature/julibrot` to main after the owner's walk; fold the measured ledger into the five slice documents|EO-02|
|EO-04|The engine design document: the AST-to-DAG compiler, the two petgraph DAGs with double drains and swaps, the credit-based channel as the engine's producer API, the hot ring for camera and late-latched objects, the migration of the arena renderer from fixed mesh ids to handles, one tier at a time with a measured reason each — DESIGNED; evidence: `docs/engine-design.md`|EO-03; the Julibrot's evidence|
