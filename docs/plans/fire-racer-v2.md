...
## Publication receipt — 2026-09-05

Runtime source `86086a2` is pushed on `codex/fire-racer-v2`. Fire-only Pages commit `80cf00e` published the game in 7.45 seconds. Server address publication `fe3d112` advertises host `dusky-osprey`, Fire protocol 2, build `r1244`, commit `86086a2`; the host action took 79.8 seconds including tunnel propagation. Candidate, loopback and public protocol/commit probes passed. GitHub's Pages build and deployment jobs completed successfully for both publications. The stable play address is https://endersgamesdev.github.io/EmberEngine/games/fire/v2/.

The published website passed the isolated browser practice check in 9.5 seconds and the full public online create/join/vehicle/movement/boost/focus check in 7.7 seconds without browser exceptions. A first online attempt ran before the address-book deployment had completed; the test now waits for a successfully selected host instead of treating a discovery placeholder as readiness. Compared against pre-release Pages `e8a9e17`, every non-Fire catalog object, all other page files and the Arena/Kings server URLs and protocols were unchanged. The existing Arena process remained running. Fire's persistent executable, process ownership records and logs are under the workstation's `.ember/fire-local` directory; no scheduled startup was installed.
