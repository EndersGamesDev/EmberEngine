...
Trench City keeps its four pads, but a pad now rolls the same loot table as a block, so there is exactly one reward rule in the game.
...
`PlayerSt` gains `pub reserve: u8` (rounds outside the magazine; `RESERVE_INFINITE` for the sidearm). `PlayerIn` gains `pub ads: bool` (held, like `shield`). `add_player` and respawn set `weapon = SIDEARM, ammo = 8, reserve = RESERVE_INFINITE, reload_t = 0`.
...
| **Loot K** ("the king block") | −0.5..0.5 | −0.5..0.5 | **3.45** | 4.45 | From the floor a near-apex bonk (1.59 m of rise against the 1.6867 apex: a floaty, deliberate jump that leaves you hanging in the open); from the dock a 0.39 m hop. |
...
| Ammo | 10.8..11.6 | 16.4..17.2 | 0 | 0.55 | Q4 chain start, against Q2's north face. |
| Crate | 10.8..12.0 | 17.6..18.8 | 0 | 1.2 | Q4 chain step (gap 0.2 to Q4, 0.4 to the ammo); one hop from here also reaches Q2's roof. |
...
| **Loot L** ("train-zone block") | 6.0..7.0 | 9.6..10.6 | **2.30** | 3.30 | Off the C1 crossing exit. Walked under with 0.44 m of headroom; a floor jump hits it 0.05 s (three ticks) after the press: the Mario snap. |

Counts: centre 2 + spine-x 4×2 + spine-z 4×2 + quadrant 19×4 = 94 boxes (Trench City has 89); **7 loot blocks** (K, R×2, L×4); 0 pads. The block that one design hung in the backlot 5.5 m from a spawn is dropped: a block top is a perch (§5.1) and a perch over a spawn is the worst thing this map could produce.
...
**Why it plays differently from Trench City**: rings versus lanes. Trench City funnels eight players into four identical tunnel fights over pads. Freight Yard has an open, dangerous yard (long lanes for the AK and the sniper, the dock and the king block as the exposed prize), a close train zone of 6.6 m alleys and roofs (SMG, revolver, RPG lobs), protected backlots you leave through two gaps each, and a second storey with its own reward.
...
8. `the_yard_has_a_long_sightline_and_the_train_zone_does_not`.
9. `every_train_zone_block_is_bonked_from_the_floor_and_walked_under` — for each L: 90 ticks jumping under it reports `bonked == Some(i)` once; 60 ticks walking under it never does; `blocked` is false at `y 0` on its footprint.
10. `the_king_block_is_bonked_from_the_dock_and_from_the_floor`.
11. `the_roof_block_is_bonked_from_its_container_and_not_from_the_floor` — from C2's roof the clamp reports it; 200 ticks of jumping on the floor under it never do.
...
15. `trench_city_is_unchanged_by_v18` — `Level::trench_city()` equals a frozen JSON fixture at `crates/arena-core/tests/fixtures/trench-city-v17.json`; `a_seeded_level_is_exactly_the_arena_the_generator_made` and `a_seeded_level_still_plays_exactly_the_v12_arena` stay.
...
| Intent | Key / mouse | Pad (standard / XInput) |
|---|---|---|
...
| 6 Sniper | 0.070 / 0.35 / 0.05 | 0.06 / 1.5 across the full 0.9 s / 0 | 1.00, 0.20, 110 | 0.22 for 40 ms | 1.6 × 0.05 cyan-white: a line, not a streak; ADS draws a scope frame of four opaque black cubes 0.5 m from the eye at FOV 22 | `ShotSniper` crack sweep(0.04, 1800→400, sq 0.9, decay 60, noise 0.6) + boom sweep(0.22, 120→45, sq 0.5, decay 12, noise 0.35) |
...
| RPG-7 | `assets/rpg7/source/RPG7/RPG7.fbx` | 5 meshes at scale 0.01 (`RPG7` 15 248 faces, `rocket` 3 326, `hammer`, `sight`, `sight_adjust`), UV layer `UVChannel_1`, materials `RPG7` and `RPG7Rocket` | launcher 0.48 along **Y**, the rocket protruding to −Y (the muzzle), sight +Z; about half real size | `w_rpg7` = launcher + hammer + sights joined (picture 1024) and `w_rpg7_rocket` separate (512), both with the launcher's origin so the rocket draws in place at the unit transform; 0.95 m |
| Sniper | `assets/sniper/source/source/1.fbx` | `rifle` **83 430 faces with four material slots** (back, SCOPE, front, middle), `rifle.001` 1 514, `mag` 3 254, `bullet` 1 634 (a loose display cartridge: dropped) | 3.26 along **X**, muzzle +X, scope +Z; about 2.7 units per metre | join rifle + rifle.001 + mag; **bake an atlas**; decimate to ≈ 15 000 faces **after** the bake; `w_sniper` at 1.15 m |
| Revolver | `assets/revolver/revolver.obj` + `baked/M1.png, M2.png` (v15) | 20 named parts, merged to five by `tools/v15/build_viewmodel.py`'s `GROUPS` | v15's fit: 0.75 m, +X, origin on the grip | `tools/v15/build_viewmodel.py` gains an `if __name__ == "__main__"` guard and a `build_revolver()` that returns the five parts, their pivots and the muzzle, without changing its own output; `w_revolver_frame` (M1 at 1024), `w_revolver_receiver`, `w_revolver_cylinder`, `w_revolver_hammer`, `w_revolver_trigger` (512 each) |
...
6. `verify_glb()`: node AND mesh names equal the exact sorted list `[hand_sword, hands, rifle, shield, sword, w_ak47, w_revolver_cylinder, w_revolver_frame, w_revolver_hammer, w_revolver_receiver, w_revolver_trigger, w_rpg7, w_rpg7_rocket, w_sniper, w_vityaz]`; one primitive each with `TEXCOORD_0`, a material and a base-colour texture; every image `image/png`, re-decoded from the GLB's binary chunk with PIL to assert 8-bit and at most the intended size; `baseColorFactor` white; per weapon the front bound > 0 and the length within 2 % of its target; `w_rpg7_rocket`'s bounds inside `w_rpg7`'s x-range; the GLB size printed.

### 8.3 Cost, honestly

Texture VRAM is `w × h × 4 × 4/3` per mesh id. Today's viewmodel: 44.7 MB (hands 2048², four parts at 1024²). New: Vityaz 5.6, AK 5.6, revolver 5.6 + 4 × 1.4, sniper 5.6, RPG 5.6 + rocket 1.4, loot 1.4 → **+36 MB, ≈ 81 MB for the viewmodel set** (every weapon is registered because any remote player may hold any gun; `CLAUDE.md`'s 112 MB line is the reference point). Bundle: `viewmodel.glb` is 8.0 MB today; 1024 PNGs run 0.6–1.6 MB here and the new geometry (≈ 60 000 faces after decimation) about 3 MB, so the GLB grows to ≈ 15 MB on a wasm bundle measured at **31.0 MB** (`target/wasm32-unknown-unknown/release/arena.wasm`, 2026-09-02; the "~17 MB" in `docs/asset-pipeline.md` is stale and is corrected there). The measured number after `wasm-bindgen` goes in the commit; if the GLB passes 16 MB the Vityaz and sniper atlases drop to 768 first, never the revolver.
...
| **B map** | `crates/arena-core/src/freight_yard.rs`, `crates/arena-core/tests/fixtures/trench-city-v17.json`, the shared `level_helpers` test module | §4 and its fifteen invariants |
...
1. Unit: everything above green; the determinism test and the frozen Trench City fixture pass.
2. `verify_glb` output and the previews reviewed by eye; `every_table_weapon_has_a_node_or_a_fallback` green.
3. Native two-client capture (a local `arena-server` on 127.0.0.1:7778, two `arena-app` clients, `EMBER_CAM` on the observer and a click to focus the second client — the v17 lesson; `keybd_event` works, synthetic clicks do not, so the harness is rewritten under `tools/v18/` because nothing of it is committed today): each weapon first person and on the other client, a bonk with the pop, a rocket in flight and its blast on both clients, the sniper scope. Screenshots into `tools/v18/ingame-*.png`, paths in the commit.
4. Gamepad: no pad is attached to this workstation (gilrs listed none on 2026-09-03), so the rumble and input paths are verified by compile and by their unit tests only, and the commit says exactly that; a pad plugged in later exercises them without a code change.
...
7. Deploy is **not** part of v18's definition of done: the live host is this workstation on proto 13 and moving it (`deploy/deploy-arena-local.sh up` before `deploy/deploy-pages.sh`) is the user's separate decision. The page is staged on the branch and `games.json` carries v18 on `proto: 14`, exactly as v13 was staged before its server moved.
...
- **Gamepad platform quirks**: gilrs on Windows is XInput (four pads, no DirectInput force feedback; a DualShock without a driver sees nothing); browsers expose a pad only after a button press; Firefox and Safari have no actuator. All degrade to keyboard and mouse; the status line names the probe result; native Linux is untested and said so.
...
- **Deploying proto 14** freezes v13–v17 list-only and needs the local host rebuilt before the pages deploy; the specht ssh stall (memory) means this workstation is the only host that can move, and that is the user's call.
