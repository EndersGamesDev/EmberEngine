...
|4 → 5|presentation (identity today)|`crates/ember-shader/templates/engine-present.wgsl.jinja:30-31`|
...
Julibrot applies the same separation to navigation and presentation. The stored view is the authority: today it is the app's requested-control record plus the worker owner's bignum centre, with precision that may grow but never shrinks during navigation; the pending `ember-camera` adoption replaces that split record with a fixed-512-bit centre, integer exponent quanta and integer angles. Pointer edits author only that record, and F64/F32 mirrors are downstream samples that must never feed back. A rendered pose names the accepted reference it is expressed against with a nonzero `generation_applied`, whereas zero is the no-reference sentinel while a slice replacement awaits acceptance and `centre_revision` names the requested draft. Presentation may retain and reproject an older pose, but it must consume each accepted reference shift exactly once when that generation changes and convert the shift through the accepted pose's pixel scale before presenting it.
