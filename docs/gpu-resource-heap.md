...
Executor decisions dated 2026-09-05 retain square-padded DATA uploads as the default and expose an opt-in valid-row path whose production-region oracle reduces a 4,096-record reference from 1,048,576 uploaded bytes to 65,536; that path leaves the page remainder untouched, requires callers to respect `logical_len`, and needs the nonzero-prefill browser layout and readback proof before enablement. Selected dispatch headers are compared once per page range after exact pass-count validation, metadata tables pack through retained executor scratch, resource-word publication does not repack metadata, and one bounded set of SCRATCH views plus fixed attachment slots is reused across pages.
