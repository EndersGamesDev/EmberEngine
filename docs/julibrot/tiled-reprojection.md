...
Stage-0 decision — 2026-09-05 on `lane/jb-tilejob`: the wire-free replacement predicate folds stored residency and refinement into the single effective ladder Backdrop before Detail Preview before Detail Interactive before Detail Final. The representable Backdrop-plus-refinement combinations all mean the same Backdrop rung, density remains a delivered fact rather than a CPU replacement rank, and the later fragment-level ownership policy retains its separately specified density and error evidence.
