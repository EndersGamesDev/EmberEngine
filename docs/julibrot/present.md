...
The displayed fifth coordinate is `h₅ = h·(H+2)·0.5` for the height control `h ∈ [0,4]`, evaluated in that written order in WGSL and in both binary64 mirrors. The requested zoom describes the floor: `H=-2` lifts by exactly zero, while `H=+2` lifts by `2h`, exactly the former peak position. At `h=0` the shader takes its direct identity branch before the projection chain and the mirrors produce zero lift for every `H`, so the flat picture is bit-identical rather than merely close. Every intermediate control value remains a continuous morph; negative controls remain refused because they invert the relief.
...
Amplitude no longer gives a meaningful boundary scale: floor anchoring makes that scalar one at every pose while tilted cameras can still expose surface that a wider sampling map reaches. `scene_footprint` therefore rasterizes the mirrored mesh at apron one and at the candidate set `{1.25,1.5,2,3,5}`. If apron five gains nothing over one, the request stays one and the residual is honest horizon sky; otherwise it chooses the first candidate whose reduction in uncovered fraction is at least half the reduction achieved by five.
...
`scene_footprint` reports the raster-selected requested `scene_apron_scale`, the uncovered fraction after that requested span, and the share of its fixed 9-by-9-by-5 bounded record-domain census at the near clamp. `scene_backdrop_scale` and `scene_backdrop_extent` are absent until a backdrop is actually allocated and then report its applied scale and capacity-selected extent; `relief_clipped_fraction` publishes the census result.

Coverage is measured the way the picture is drawn, because the earlier boundary formula ceased to describe the anchored geometry and the still earlier ring measurement was vacuous exactly where the answer mattered. The mirror rasterizes the sampling mesh at every census height and candidate apron into a lattice whose samples run to `±1` inclusive, so frame corners and edges are tested points rather than cell centres. A triangle is drawn only when all three vertices project and not all three are held at the near clamp — the scene shader's own primitive rule — so a pose whose mesh falls apart reports more sky and never less. The candidate comparison always uses this same deterministic raster mirror; its sampled heights and 65-by-65 mesh make the result a stated measurement rather than a mathematical bound.

The clipping census keeps its whole fixed denominator, all 405 points. A census point beyond the projective horizon is not at the clamp and is not removed from the denominator; at the selected 1.25 apron the close row holds `252/405` points at the near limit.

|Owner row|Main uncovered|`scene_apron_scale` / applied backdrop|Post-request uncovered|Near-clipped census|Horizon residual|
|---------|-------------:|-------------------------------------:|---------------------:|------------------:|---------------:|
|Julia, `height_scale=2.165`, `d₅=8`|0|1 / none|0|0|0|
|Julia, `height_scale=4`, `d₅=8`|0|1 / none|0|81/405 = 0.2|0|
|Tilted Julia, `height_scale=4`, `d₅=d₄=2`|821/4225 ≈ 0.1943|1.25 / 1.25|655/4225 ≈ 0.1550|252/405 ≈ 0.6222|655/4225 ≈ 0.1550|

The close row's residual is the plane's own projective horizon, and reading it as a coverage failure would be a mistake. That pose uses `o₁₃=o₂₄=-1.3166537201715494`, `q₁₃=q₂₄=-0.2541426066233471`, yaw `0.960422302787256`, pitch `π`, and tumbles the plane far enough that a large part of the frame looks past its horizon. Through the same mirror its floor leaves `821/4225` uncovered at apron one, `655/4225` at apron 1.25, and `668/4225` at apron five; the relief scene produces the same three fractions, so the remaining sky is geometry rather than missing coverage.

Apron five improves the main result by `153/4225`, so the half-gain threshold is `76.5/4225`; the first candidate, 1.25, improves it by `166/4225` and is selected. Its slightly better lattice fraction than scale five is a measured raster-quantization effect, not a monotonic geometric claim. A dedicated page fact for the horizon share remains a backlog item so browser readers can distinguish this residual without consulting the pinned table.
