...
An amplitude-only edge estimate no longer describes coverage: floor anchoring leaves the floor on its chart while tilted cameras can still expose surface that a wider sampling map reaches. `scene_footprint` therefore rasterizes the mirrored mesh at apron one and at the candidate set `{1.25,1.5,2,3,5}` and compares exact uncovered-point counts on the interior 63-by-63 census. It finds the candidate with the largest gain over apron one; if that best gain is below one percent of the 3,969 admitted samples, meaning fewer than 40 points, the request stays one. Otherwise it chooses the smallest candidate that recovers at least half of the best gain. A best gain of zero identifies sky that no reviewed apron reaches, while a positive sub-threshold gain is reachable ground deliberately not worth a separate coarse scene.
...
One uncached footprint performs six rasterizations. At 960 by 540 that is 126,750 vertex projections, 245,760 triangle setups, and approximately 907,000 barycentric tests. The optimized native test on sokol measured ten close-row calls in `247.983 ms`, or `24.798 ms` per call. `ViewerController` therefore memoizes the result in a two-slot cache keyed by the bit patterns of the complete object, complete view, and extent; the once-or-twice backdrop preparation and the page-facts snapshot reuse their matching half- or full-extent entries instead of repeating this work.
...
|Tilted Julia, `height_scale=4`, `d₅=d₄=2`|611/3969 ≈ 0.1540|1.25 / 1.25|572/3969 ≈ 0.1441|252/405 ≈ 0.6222|
...
The close row's candidate uncovered counts are `1.25 → 572`, `1.5 → 573`, `2 → 571`, `3 → 569`, and `5 → 585`. Scale three is best and recovers 42 points from the main count of 611; scale 1.25 is the smallest candidate recovering at least half that gain, with 39 points, and is selected. The non-monotonic spread is raster quantization, not a geometric ordering by apron width. A dedicated page fact that classifies residual sky remains a backlog item so browser readers do not have to infer its cause from the coverage count.

At the floor-half 480-by-270 policy extent, all four shipped presets request no backdrop: the flat Mandelbrot and Julia rows return the structural zero, while Mandelbrot relief reports `7/3969` and Julia relief reports zero, with all five candidate counts zero in both cases. Mandelbrot relief's seven reachable gains and Julia relief's zero are below the 40-point admission floor, so frame-rim arithmetic no longer buys a coarse grid.

Backdrop policy runs at the floor-half extent selected in `app/src/frame/loop.rs`, while `PageFacts` measures at the device extent. With zero camera translation the forward homography's extent terms cancel under the exact power-of-two rescale between a Final and its half-Final, and the mirror pins bit-identical answers at 480 by 270, 960 by 540, and 1920 by 1080 for both shipped relief rows and the close owner row. That agreement is arithmetic, not a general extent-invariance theorem: the prior rim-inclusive census changed at the same-aspect 1024-by-576 extent, which is why the policy now excludes the frame tie rather than treating the power-of-two coincidence as geometry.
...
`ReliefRedraw` deliberately redraws only retained main-grid records through their source main map. A changed relief backdrop is re-requested before Preview rather than retained for redraw, so ground outside the main grid but inside that wider request remains honest sky until the coarse scene lands; this limitation is published here rather than hidden by stretching either layer.
