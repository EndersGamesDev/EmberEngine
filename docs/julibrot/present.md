...
The close row is the honest one and it is a partial win, not a clean one. At `d₅=2` with the slider at maximum, records with `height_scale·record_height ≥ d₅` lift to or past the eye and cannot be projected at all, so the backdrop at an apron of five still leaves about a sixth of the frame as sky. Measured through the same mirror at the same pose with the apron set to one, the main grid alone leaves `813/4225`, so the coverage layer closes about a fifth of the gap rather than all of it. That is worth having and it is not a rescue: the pose is extreme, and the fact now says so instead of claiming a perfect zero.
