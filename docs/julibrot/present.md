...
Refusing rather than clamping is what makes the bound honest. A vertex past the limit is behind the five-dimensional camera; the projective algebra still returns a point for it, its mirror image through the pole, and holding the denominator at the limit draws exactly that mirror image. Measured at a saved row with all ten camera angles nonzero and `h=3.565`, `d₅=d₄=8`, the held point is up to 148.8 px from anywhere the surface reaches, against `6.4e−7` px of round-trip error everywhere the limit is not passed. Whether a sample passes it depends on its own escape count, so a band of the frame alternated drawn and dropped columns with the dwell bands: at that row 79,356 of 518,400 samples (15.31%) pass the limit at some record height and 15,284 of those (2.95%) pass it only at some heights. A refused vertex takes its whole primitive with it, which is what the four-dimensional and observer poles have always done, and the binary64 coverage mirror applies the same rule.
