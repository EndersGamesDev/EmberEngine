...
`Presenter::set_main(&mut self,main:PresentMain)` is the infallible MAIN-drain endpoint: it records latest-wins state, applies a not-yet-consumed `reference_shift_px` to compatible retained and in-flight poses for the accepted revision, moves the last completed image to the held slot when slice, delivered `max_iter`, `precision_mode`, or strict MAIN generation starts a new partition, and marks incompatible in-flight work for dropping without allocating, submitting, waiting, or returning an error.
