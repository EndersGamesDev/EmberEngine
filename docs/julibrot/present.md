...
|Tilted Julia, `height_scale=4`, `d₅=d₄=2`|821/4225 ≈ 0.1943|1.25 / 1.25|655/4225 ≈ 0.1550|252/405 ≈ 0.6222|

The close row uses `o₁₃=o₂₄=-1.3166537201715494`, `q₁₃=q₂₄=-0.2541426066233471`, yaw `0.960422302787256`, and pitch `π`. Its floor mesh is the flat chart and reaches 3,404 of 4,225 lattice points at apron one. Each of the four lifted census meshes has projectable vertices but every one is held at the near clamp, so every lifted triangle is all-clamped and dropped; that measured mechanism, rather than equality between two derived fractions, is why the relief coverage is exactly the floor coverage at this row.

The close row's candidate uncovered counts are `1.25 → 655`, `1.5 → 656`, `2 → 654`, `3 → 652`, and `5 → 668`. Scale three is best and recovers 169 points from the main count of 821; scale 1.25 is the smallest candidate recovering at least half that gain, with 166 points, and is selected. The non-monotonic spread is raster quantization, not a geometric ordering by apron width. A dedicated page fact that classifies residual sky remains a backlog item so browser readers do not have to infer its cause from the coverage count.

At the floor-half 480-by-270 policy extent, the shipped flat Mandelbrot and Julia presets request no backdrop. The shipped Mandelbrot-relief and Julia-relief presets have main uncovered counts 97 and 47 respectively, all five candidate counts are zero, and both therefore request apron 1.25 under the 43-point minimum; these are deterministic lattice-policy facts, not browser pixel counts.
