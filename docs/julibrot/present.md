...
The scene pass draws the screen-grid height field into an LDR `Rgba8Unorm` target over a sky clear distinct from the palette's exterior colour at zero smooth iterations. At height zero every interior main-mesh vertex is its own screen position by construction and the outermost ring is drawn on the frame boundary, so the mesh tiles the frame exactly and the clear is never seen. A `width`-sample grid covers `width` pixels, so a mesh spanning only their centres falls half a pixel short at both ends: the rasterizer's fill rule counts the low boundary as covered and the high boundary as outside, which left the last column and row painted with the pass's clear and showing a colour no record carries. The ring is a drawn rule and not a sampling one, so the plane point each record carries is unchanged and every pixel still resolves to its own record. At positive height the floor remains on that chart, so a face-on frame has no contracted edge; the optional backdrop is reserved for raster-measured holes in tilted views. A lifted vertex past the five-dimensional near limit, a later four-dimensional pole, a near-edge-on plane, the far side of the horizon, or ground reached by neither grid all remain honest sky.

Which samples are placed by that projection is decided by the record, and the decision differs from the shading. A sample that never escaped is in the set: its record height is the chart floor `−2`, so its lift is zero and it is projected exactly like an escaped sample that left the disc on its first iteration, which has the same height. The two are one point of one surface and no camera may separate them; only the fragment colour differs, `interior_rgba` against the palette's exterior. A horizon record is the opposite case. It marks a pixel whose screen-to-plane denominator was not positive, so the plane reaches no point there and there is no vertex to place: the sample is refused and its primitives are not drawn. The pass clears to the same exterior colour the horizon carries, so the picture is unchanged wherever the projection is the identity. Placing it flat instead put a slab at clip depth zero, the nearest depth the pass can write, which under a turned camera covered its whole grid footprint and won every depth test against relief that was really in front of it; at a saved row with all ten camera angles nonzero and `h=3.565`, `d₅=d₄=8`, 17,556 of 518,400 records (3.39%) carry the horizon status, a wedge in the lower left of the frame bounded by the plane's own straight horizon line. The mesh has four more: a record is sampled at its pixel centre, while `grid_screen` places the outermost ring of vertices on the frame boundary instead, so 17,560 of the 518,400 mesh vertices fall on the far side of that line. Both counts are pinned in `crates/labs/julibrot/present/tests/steep_frame.rs`.
...
Newly exposed source coordinates outside the retained texture show `clear_rgba` and set the exposure latch; the next completed scene fills them over the exterior sky. An exposed accepted warp continues sampling the sharper retained source wherever it has coverage instead of promoting a draft over the whole surface; this lane deliberately keeps the uncovered region temporarily clear rather than adding a second texture sample and bind group. For image warps, `warp_exposed_fraction` reports the share of the same fixed 9-by-9 destination lattice that the actual uploaded rows put out of source while excluding points the shader paints as horizon sky; for `ReliefRedraw`, it reports the binary64 mesh-coverage mirror's remaining clear share using the resident backdrop apron when available and the main apron otherwise. The plan carries the exact `(scene_id,texture_index)` it was solved against, and draw clears if the retained source differs, preventing a scene promoted between HOT write and frame from sampling a different texture with stale rows.
...
`ReliefRedraw` always redraws the retained main-grid records through their source main map and additionally draws the current validated coarse backdrop when its records remain resident. The same backdrop-first, disjoint-depth ownership as an ordinary scene prevents the coarse grid from displacing fine content. If the backdrop is absent or stale, the main-only coverage fraction remains explicit and ground outside the retained mesh stays honest clear until a coarse or new main scene lands.
...
`PresentFacts` publishes retained and pending identities, the optional held-frame partition and held scene identity, delivered state, precision provenance, view and reference displacement, scene/warp measurements, image-warp and relief-redraw counts, texture reallocations, exposure/fill state and fraction, chart residual, measured maximum and p95 error, and status. `record_warp_plan` atomically replaces the planner facts and clears any unvalidated redraw coverage; only the submit-time relief coverage recorder may then fill that fraction after record and backdrop validation. Every planned geometric source has a measured maximum, while a held-only or pre-solve incompatible source has no fabricated error.
