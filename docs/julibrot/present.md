...
What the refusals do to the picture is a separate measurement, and a smaller one than the sample counts suggest. At that row the two refusals together change 5,442 of 518,400 pixels, about one in a hundred, none of them above canvas row 350: a refused sample was already painted the palette's exterior colour, and the pass clears to that same colour, so the change is the loss of the fragment lighting term over ground below the plane's horizon rather than a change of shape. `crates/labs/julibrot/present/tests/steep_frame.rs` renders that frame from the records through the mesh, the depth buffer and the fragment shading, and reproduces the browser's own readback statistics; a claim about the picture is made there rather than inferred from a census of samples.

The streaks such a row shows are the height control, not the limit. Rendered natively at this row's pose and records, no sampled column of the frame flips between background and relief six or more times while `h ≤ 0.2`; twenty to forty do at `h = 1`, and the count is monotone up to the row's own 3.565. One iteration of escape count moves a vertex 0.19 to 3.99 px at that amplitude, so two adjacent samples six iterations apart stand about twelve pixels apart and the surface between them is a wall.
