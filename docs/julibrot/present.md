...
The redraw reuses the scene pass rather than adding a shader path. The scene vertex stage already reads records from the RGBA32F heap through the descriptor lattice and takes the destination lift and camera from HOT, so redraw binds the retained extent, plane, source map and record span through the existing `SceneUniform`, then uses the destination HOT write: `group(0)` heap data, descriptors and directory unchanged, `group(1)` the same scene and HOT bindings. The same WGSL entry points and indexed mesh are specialized into a second render-pipeline object only because the browser surface format differs from the `Rgba8Unorm` retained-image target; there is no new bind-group layout, shader variant, uniform byte, texture sample, or device feature beyond WebGL2 and `EXT_color_buffer_float`. Because Preview, Interactive, and Final overwrite one Final-capacity span, app alternates two such spans by ladder round: the accepted Final keeps its own records while the other span is refined, and a promoted Final exchanges their roles. At 960 by 540 the extra reservation is one eight-page DATA span, 8,388,608 bytes in the already allocated heap, plus three 64-page header sets, 49,152 bytes at the WebGL 256-byte alignment; no third main span is live.

The plan kind selects the draw. `ReliefRedraw` means the corpus was measured above the homography ceiling and the pose pair is in the exact family, so it keeps the retained scene identity, counts one warp-class redraw, clears the target to the same distinct `clear_rgba` used by the image warp, and draws the retained main-grid record mesh; an exterior record is shaded with the palette exterior colour, so a disocclusion remains visibly distinct. Backdrop records are not retained by this redraw: app requests the current backdrop before Preview, but the redraw may therefore show sky in newly exposed relief ground until that backdrop lands. Uniform construction reads the retained frame's own span, dimensions, source map, and level and refuses only if that span's dimensions no longer equal the frame it belongs to; the live ladder extent is irrelevant, while a real surface resize still fails the planner's equal-lattice proof before drawing. `ClearOnly` also covers every over-ceiling pair outside the exact family and every corpus that cannot be measured. `HoldStale` draws a retained source with identity rows, reports no exposure, and is not an accepted warp for draft-skipping; manual mode may keep it indefinitely while waiting for Update scene, whereas auto mode permits it only through one ladder round while a replacement is being resolved, then returns to honest clear if no Final arrived. The oracle asserts the split directly: `observer distance five h1`, `observer height 0 to 1`, `height 0 → 2.165 across the apron`, and `observer height 1 to half` select `ReliefRedraw` and prove sample by sample against independently computed fresh main scenes that every covered destination pixel agrees with no uncertain comparison; neutral-camera yaw, pitch and `d₄` agree within the homography bound, while their over-ceiling non-neutral-camera cross terms clear.
...
Screen-aligned sampling inverts the scene map at height zero, which makes main-grid vertex `(i,j)` land on pixel `(i,j)` but gives the map no knowledge of the height that record will carry. On the preset Julia relief plane a floor record at `H=-2` contracts radially by `boundary_scale=d₅/(d₅+2·height_scale)`, so the first owner row (`height_scale=2.165`, `d₅=8`) reaches only `0.648824` of each frame axis and leaves `57.9%` of the surface without sampled ground.

The second owner row makes a uniform main-grid apron untenable: `height_scale=4`, `d₅=d₄=2`, `ρ₁₃=ρ₂₄=−1.3166537201715494`, `q₁₃=q₂₄=−0.2541426066233471`, `q₁₂=q₃₅=0`, yaw `0.960422302787256`, pitch `π`, origin `c=(−0.671,0.131)`, and `zoom_log2=3.92` give `boundary_scale=0.2` and request a fivefold span, while records with `H≥0.5` reach or cross the five-dimensional eye before clipping. Clamping an apron at two would still cover only forty percent of each axis, and applying five to the main map would spend fivefold coarser resolution on the entire picture.

The taken design leaves every main map at `apron_scale=1.0` and allocates one separate coarse backdrop only when the footprint requests more than one. The backdrop uses the same plane, centre, accepted reference orbit, precision mode, height records, and presented camera as the main grid, but samples the unclamped span `scene_footprint.apron_scale=1/boundary_scale`; it is a real escape scene rather than a flat floor or a stretched edge.
...
The close row's residual is the plane's own projective horizon, and reading it as a coverage failure would be a mistake. That pose tumbles the plane far enough that a large part of the frame looks past its horizon: those screen points fail the sampling map outright, no height and no apron recovers them, and honest sky is the correct picture there. Measured through the same mirror at the same pose with the height set to zero — a flat chart, no relief at all — the frame is still `821/4225` uncovered at apron one and `668/4225` at apron five. The relief scene with its backdrop leaves `650/4225`, which is below the flat chart's own floor at the same apron, because the lift brings slightly more surface into view than the discarded clipped rim costs.

So the backdrop closes essentially the whole recoverable gap at this row. The `813/4225` main-alone baseline was itself already almost all horizon sky, which is the only reason the two published numbers sit so close together; the coverage layer is not the thing falling short. What remains is geometry, and the honest way to publish it would be a horizon share beside the uncovered share so the two can be read together — a backlog item, since the fact surface reaches from math through present and app to the page contract.

Both numbers are the drawn rule evaluated at a stated resolution, not bounds. Sampling five heights understates what the full domain reaches; a 65-by-65 mirror mesh coarser than the drawn one overstates it, because near the projective horizon one long chord cuts across the curved image and covers ground a fine mesh leaves as sky. A point scatter of the same mesh, which counts only where vertices land and never fills a triangle, reports far more sky than either. The published share is therefore comparable against itself — main against backdrop, pose against pose — and is not a claim about the exact pixel count of the delivered frame.
...
`ReliefRedraw` deliberately redraws only retained main-grid records through their source main map. A changed relief backdrop is re-requested before Preview rather than retained for redraw, so newly exposed relief ground may show honest sky until that coarse request lands; this limitation is published here rather than hidden by stretching either layer.
