...
Phase C closes the P2-P4 sequencing defect without changing `MainState` or the worker/kernel protocol. The already-published `generation_applied` is the accepted-reference identity: compatible zero-shift acceptances and new-orbit acceptances both advance it, whereas `centre_revision` remains the requested-draft identity. `Presenter::set_main` therefore rebases only when `generation_applied` changes, and `SceneLedger` records that generation on retained and pending poses so replaying a MAIN record cannot apply the shift twice.

The audit also exposed a scale conversion hidden by the missed-rebase defect: `reference_shift_px` is measured at the accepted view's zoom and extent, while a retained pose may have been sampled at another zoom or extent. Presentation now converts that shift through the latest accepted HOT pose before changing each retained offset; using the retained pose as both sides would force the ratio to one and would move an otherwise correctly anchored zoom. The Phase-A trace invokes the same conversion and asserts every post-input frame is within the named 0.001-pixel tolerance of settlement. The exact-camera violations in N1-N7 and the F64/F32 pose boundary remain open under the `ember-camera` backlog item.
