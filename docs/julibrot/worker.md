...
For pool capacity `M`, every buffer has `capacity_bytes = max(644,64+8M)`: 32 header bytes, room for up to `M` orbit records or the request body, a 16-byte orbit-verification tail, and a 16-byte immutable pool trailer. The browser app initializes `M = 4,096`, the maximum cap exposed by the current page, so every page cap change reuses the four stable 32,832-byte buffers and arms no drain; the total transport pool is 131,328 bytes. This makes PF-R8's return-path clearing and scanning cost always-on: a default cap-512 request uses a 32,832-byte buffer instead of the former 4,160-byte buffer, about 7.9 times as many bytes per returned request whether or not the cap control moves. The reusable worker channel also accepts any request cap at or below its current pool capacity and grows but never shrinks: a burst above an initially smaller capacity retains the largest requested capacity, coalesces into at most one four-slot drain, delivers queued arrivals for stale disposition, replaces the pool after reconciliation or the four-second return deadline, restarts the producer once, increments `allocation_events` once, and re-encodes only the newest request. A direct or scripted cap above 4,096 is not refused: it triggers that growth path and preserves correctness, but forfeits the page's stable no-drain guarantee.
...
~~After a trigger, the worker remains disarmed while work is in flight and coalesces newer edits; it re-arms after an applied reference when `d_px ≤ grid_width/8` and `|zoom_log2−reference_zoom_log2| ≤ 1`, otherwise it immediately retains only the newest pending request, giving the thresholds explicit hysteresis.~~ — SUPERSEDED: a compatible accepted lease now bypasses the old zoom-band re-arm rule, while an incompatible edit retains only the newest pending request.

An accepted deep reference is leased to one MAIN generation rather than one bit-exact zoom value. The lease remains current only while the plane identity, centre revision, precision policy, delivered precision, and MAIN generation match, and renewal requires exactly the requested iteration-cap record count. Exact full-cap equality is deliberately conservative: a shorter orbit cannot prove that every requested perturbation step can read its reference sample, while `crates/labs/julibrot/kernels/src/gpu.rs:501-508` binds the reference length to the span logical length and refuses a span longer than the requested cap, so a lower-cap edit regenerates into the stable transport pool without draining it. An escaping orbit stops before the cap and therefore cannot renew under the frozen wire, which carries no escape flag; a freshly accepted short orbit may render in its source generation once so the existing census can select a longer-lived reference, but it cannot renew into a later generation. Zoom is excluded from lease identity: `crates/labs/julibrot/kernels/src/gpu.rs:476-522` validates reference resource, generation, length, and precision without a zoom field, while `crates/labs/julibrot/kernels/src/perturb.wgsl:207-216` derives the scaled offset and exponent from each dispatch uniform. A one-ULP zoom nudge at the same exact centre and cap therefore renews MAIN identity without requesting or uploading another full-length orbit; a changed centre, cap, plane, precision policy, or required precision ends the lease before dispatch. Zoom-in is self-limiting because required precision can exceed the delivered bits; zoom-out is bounded only by the shallow switch at `zoom_log2 = 14`. That zoom-out direction can degrade perturbation quality, but shader normalization and glitch detection flag the affected samples instead of silently asserting a false set classification.
...
The sokol worker measurement uses the zero-centre `measured_orbit` fixture at zoom 100 and width 960, takes the median of seven orbit samples, then repeats fixed-buffer packing 256 times. The earlier paired 16-byte baseline versus one-orbit 8-byte path measured: cap 512, time-to-first orbit 1,416→324 us, payload 8,192→4,096 bytes, pack mean 19,681→6,076 ns, admission price 1,416→324 us and depleted wait 5,664→1,296 us; cap 4,096, time 6,740→2,572 us, payload 65,536→32,768 bytes, pack mean 94,821→47,981 ns, price 6,740→2,572 us and wait 26,960→10,288 us.

On sokol on 2026-09-03, `compute::tests::measures_first_orbit_transfer_and_admission_at_worker_caps` measured the unmodified `cee30ec` current arm at 583 us for cap 512 and 4,664 us for cap 4,096; after checking the deadline every eight entries it measured 313 us and 2,471 us, reductions of 46.3% and 47.0%. The measured per-entry cost before batching was about 1.14 us at both caps, so eight entries between observations imply about 9.2 us of deadline overshoot, far below one 16.7 ms frame, while the independent 64-entry ceiling remains unchanged.

In the same 2026-09-03 runs, current-arm packing fell from 11,886 to 6,682 ns at cap 512 and from 101,494 to 52,695 ns at cap 4,096 after orbit response and credit headers stopped scrubbing producer-owned unread capacity; `wire::tests::orbit_decode_ignores_poisoned_tail_and_credit_retains_payload` poisons that capacity and proves the decoder exposes only the declared record prefix and facts.

PF-R3 is selectable by ignored test name rather than by a cargo feature, keeping production behavior invariant: run `cargo test -p ember-julibrot-worker measures_paired_orbit_16_byte_baseline_at_worker_caps -- --ignored --nocapture` for the paired-orbit/16-byte-record baseline and `cargo test -p ember-julibrot-worker measures_first_orbit_transfer_and_admission_at_worker_caps -- --ignored --nocapture` for the one-orbit/8-byte current arm. On sokol on 2026-09-03, the baseline-to-current results were: cap 512, first orbit 623→313 us, payload 8,192→4,096 bytes, pack mean 12,347→6,682 ns, admission price 623→313 us, and depleted wait 2,492→1,252 us; cap 4,096, first orbit 5,015→2,471 us, payload 65,536→32,768 bytes, pack mean 92,331→52,695 ns, admission price 5,015→2,471 us, and depleted wait 20,060→9,884 us.
...
`OwnerEndpoint::shutdown()` closes an already reconciled same-thread channel or immediately returns `BufferStarved`; the browser owner sends `Shutdown`, stops accepting requests, and drives event-based reconciliation for at most the same four-second buffer-return deadline as resize. A complete close terminates the producer and releases all owner-held pool buffers; an expired close also terminates the producer, invalidates late leases, releases every buffer still reachable by the owner, records terminal completion, and returns typed `BufferStarved` naming the first missing pool and slot rather than hanging.
...
`OwnerCore` holds its at-most-two arrivals in an allocation-free fixed ring whose full insertion is a typed refusal rather than an overwrite, so taking the next response and stale-draining shutdown work preserve FIFO order without moving the second slot; the ignored `measures_two_slot_arrival_drain_wall_before_after` native harness measured 20,000,000 drains on osprey on 2026-09-04 at 74,485 us for the former shifting `Vec` and 43,507 us for the fixed ring, a 41.6% reduction.
...
The wasm producer validates the request header, trailer, declared limb count, and available prefix length through the transferred `Uint8Array`, then performs one `subarray(0,used)` bulk copy of only the declared 116-plus-limbs prefix into a compact codec buffer rather than walking or copying the unused tail; the ignored native codec-equivalent harness measured 32,832→628 copied bytes and 15,739→590 us of native memcpy wall across 20,000 canonical cap-4,096 copies on osprey on 2026-09-04. The native harness pins only codec-equivalent bytes and memcpy cost; only visible browser replay can measure `Uint8Array::copy_to`, sender detachment, JavaScript engine allocation behavior, and actual browser wall time.
...
`WorkerFacts::new(mode)` creates the startup snapshot with `allocation_events = 1` for the four-buffer allocation, and that count increments only after a capacity-growing replacement; lower cap requests reuse the pool. `OwnerEndpoint::facts()` and `ProducerEndpoint::facts()` return the same coherent snapshot without inventing unavailable browser observations.
...
App retains each accepted response's `reference_verification`, `max_consumed_word_error_ulps`, and `precision_escalations` beside its lease. The facts snapshot publishes verification as `Deferred` or `Stable`, publishes the orthogonal escalation count and consumed-word error when present, and separately names the last completed scene's precision policy; PictureFast Preview remains deferred and performs no new verification work. A true on-glass precision policy requires the presentation schedule's presented-scene identity and is not inferred from completion.

The per-level timing ledger publishes worker generation, outbound request transfer, request-to-response callback observation, acceptance, and reference upload from monotonic marks available on the main thread. Credit wait remains unavailable because the frozen wire carries no producer-side admission timestamps, and scene and warp completion walls are named callback observations rather than GPU walls; no residual or fence observation is relabelled as GPU execution. Legacy timing keys remain in JSON as `null`, so the overlay renders them as unavailable instead of dropping their rows.
...
The stable-pool tests drive 512 to 64 to 4,096 to 512 through the in-process port with work in flight, proving lower caps transfer through unchanged buffers, only the increase allocates and restarts, `pending_request_depth` returns to zero, an unreturned lease cannot hold the growth drain past the return deadline, and both endpoint modes agree field by field. A separate ten-change burst from capacity 64 reaches cap 4,096 and returns to 512 while proving at most one drain, one replacement allocation, one restart, newest-request delivery, stale-arrival credit return, and no `BufferStarved` refusal.
