...
`OwnerCore` holds its at-most-two arrivals in an allocation-free fixed ring whose full insertion is a typed refusal rather than an overwrite, so taking the next response and stale-draining shutdown work preserve FIFO order without moving the second slot; the ignored `measures_two_slot_arrival_drain_wall_before_after` native harness measured 20,000,000 drains on osprey on 2026-09-04 at 78,564 us for the former shifting `Vec` and 42,691 us for the fixed ring, a 45.7% reduction.
