...
An accepted deep reference is leased to one MAIN generation rather than one bit-exact zoom value. The lease remains current only while the plane identity, centre revision, precision policy, and MAIN generation match, and while the accepted orbit contains at least the requested iteration-cap records; this full-cap length rule is deliberately conservative because a shorter orbit cannot prove that every requested perturbation step can read its reference sample. Zoom is excluded from lease identity: each dispatch derives its own scaled pixel offset and exponent from the current `zoom_log2`, so a finite zoom nudge inside the existing centre-trigger hysteresis neither requests nor uploads another orbit. A cap increase beyond the accepted record count, a centre move past the trigger, or any plane, precision-policy, or MAIN-generation change ends the lease before refinement dispatch.
