...
`OwnerCore` holds its at-most-two arrivals in an allocation-free fixed ring, so taking the next response and stale-draining shutdown work preserve FIFO order without moving the second slot; the ignored `measures_two_slot_arrival_drain_wall_before_after` native harness measured 20,000,000 drains on osprey on 2026-09-04 at 74,703 us for the former shifting `Vec` and 41,131 us for the fixed ring, a 44.9% reduction.
...
The wasm producer validates the request header, trailer, declared limb count, and zero-owned unused tail through the transferred `Uint8Array`, then copies only the declared 116-plus-limbs prefix into a compact codec buffer rather than copying the complete orbit-capacity allocation; the ignored native codec-equivalent harness measured 32,832→628 copied bytes and 15,104→597 us across 20,000 canonical cap-4,096 copies on osprey on 2026-09-04. Native tests pin the codec bytes and copy bound, while only visible browser replay can prove sender detachment, JavaScript engine allocation behavior, and actual browser wall time.
