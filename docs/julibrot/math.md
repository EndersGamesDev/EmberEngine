...
`ViewControls` is `{camera:[f64;10],camera_translation:[f64;5],camera_yaw,camera_pitch,height_scale,distance_five,distance_four}`, twenty scalars with neutral rotations and translation zero, neutral height zero, and both neutral distances eight. Validity bounds `height_scale` to the page's closed `[0,4]` range, excluding both the rejected receding-pit sign and magnitudes at which the binary32 shader's written height operation could overflow. The page limits each translation to `[−8,8]` chart units: the neutral chart spans four units, so this range crosses it completely and can retreat past either perspective pole.
...
The scalar `Homography.apron_scale` is presentation-and-sampling metadata rather than a matrix coefficient. Every ordinary main map returned by `screen_to_plane` has the exact binary64 value one; app forms a backdrop map by copying that same map and replacing only the scalar with the raster-selected footprint request, leaving `rows`, `inverse`, `condition_number`, centre and projection aspect bit-identical.
...
`scene_footprint` mirrors the scene vertex chain in binary64 over the bounded record-height domain `[-2,2]`, applying `height_scale * (record_height + 2) * 0.5` in the same left-to-right operation order as WGSL. It rasterizes the 65-by-65 mirrored mesh onto a 65-by-65 frame lattice at apron one and at `{1.25,1.5,2,3,5}`, excludes the outermost lattice ring, and finds the candidate with the smallest uncovered count among the remaining 3,969 samples. It requests the smallest candidate that both recovers at least half that best gain and itself recovers at least 40 samples; if no candidate qualifies, it buys no backdrop. It publishes that requested `apron_scale`, the interior uncovered fraction after the request, and a fixed 9-by-9-by-5 clipping census; at zero height it returns the exact constant `{apron_scale:1,uncovered_fraction:0,relief_clipped_fraction:0}`.

The uncovered fraction is measured by rasterizing the mirrored 65-by-65 sampling mesh at each of the five census heights, through the backdrop's apron, into a 65-by-65 lattice closed on `[-1,1]`, then counting only the interior 63-by-63 samples. The excluded outer ring lies exactly on the frame boundary, where the last bit of a vertex and the triangle fill rule can turn a boundary tie into scattered apparent holes; those samples do not measure reachable ground and cannot justify a coarse grid. A triangle covers nothing unless all three of its vertices project and at least one is not held at the near clamp, which is the scene shader's own primitive rule; a mesh that falls apart therefore reports more sky and can never improve the answer by failing. The clipping census counts all 405 sampled points in its denominator, projectable or not, so a pose that projects almost nothing cannot publish a small clipped share.
...
The binary64 stage-0 oracle implements [the rendered-view coordinate model](tiled-reprojection.md#rendered-view-coordinate-model) without changing kernels or presentation. `SourceDepthRecord` mirrors `S1=(a_F,b_F,zeta_F,validity)`: `(a_F,b_F)` locate the retained visible point in the source-local plane, `zeta_F` is positive linear observer distance after the two perspective divides, and validity distinguishes a selected surface from a source hole. `source_depth_record` is deliberately a flat-chart fixture generator: it derives source-local coordinates by inverting the neutral-height source map and refuses a nonzero-height pose, whose post-visibility record must instead come from raster-selected GPU `S1`. `reconstruct_source_sample` evaluates `U_F` and refuses unless source pixel and depth reproduce within `1e-9`; its depth lane is an independent receipt only for externally produced records such as GPU-written `S1`, because a fixture record's depth is computed by the same projection. `project_reconstructed_sample` evaluates the requested `Pi_T` chain through `Q_T`, translation, the guarded five-to-four and four-to-three perspectives, observer yaw and pitch, viewport, linear depth, and pre-quantized raster depth.

The canonical four-point keeps the source plane origin and is made target-local by subtracting the requested origin before projection, so plane-preserving parameterizations share a point while an out-of-plane move remains an invalidation question. Native oracles pin flat and noncanonical lifted self reprojection, external-depth mismatch, invalid source validity, edge-on and pole refusal, a tight near-affine one-pixel interpolation bound, and a genuinely curved one-pixel cell below the 1.0 px admission ceiling. These are stage-0 policy and math foundations only; the shipped retained image remains LDR and depth-free until tiled-reprojection stage 1.
...
|`SceneFootprint`|`{apron_scale:f64,uncovered_fraction:f64,relief_clipped_fraction:f64}`; raster-selected backdrop request and post-request pose facts|present, app|
