...
At `θ₁=θ₂=−π/2` the seed is exactly `(e₁,e₂)`, which is why the Julia row keeps the orientation the lab has always drawn, and at `θ₁=θ₂=+π/2` it is the same plane with both basis directions reversed; for every angle strictly between zero and either quarter turn each transformed seed vector has a nonzero component in both `span(e₁,e₂)` and `span(e₃,e₄)`, which is the required hybrid-plane oracle and is now the ordinary case rather than an excursion between two named planes.
...
The perturbation kernel is therefore selected when `zoom_log2≥14`, a displayed POLICY rather than a device wall; below 14 the shallow kernel runs, and a reference orbit is maintained at every depth so crossing the switch does not change centre, generation, or navigation semantics.
