...
`scene_footprint` mirrors the scene vertex chain in binary64 over the bounded record-height domain `[-2,2]`, applying `height_scale * (record_height + 2) * 0.5` in the same left-to-right operation order as WGSL. It rasterizes the 65-by-65 mirrored mesh onto a 65-by-65 frame lattice at apron one and at `{1.25,1.5,2,3,5}`, excludes the outermost lattice ring, finds the candidate with the smallest uncovered count among the remaining 3,969 samples, and buys no backdrop unless that best candidate recovers at least 40 samples. Once admitted, it requests the smallest candidate recovering at least half the best gain. It publishes that requested `apron_scale`, the interior uncovered fraction after the request, and a fixed 9-by-9-by-5 clipping census; at zero height it returns the exact constant `{apron_scale:1,uncovered_fraction:0,relief_clipped_fraction:0}`.
