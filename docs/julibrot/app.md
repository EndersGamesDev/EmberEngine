...
App then drains HOT, constructs `O`, builds the main `F`/`M` at the requested extent from `O`, `Q`, camera translation and observer controls, converts a singular map to `PoseMap::EdgeOn`, builds `PresentHot` and the math-owned main `Pose`, selects `WarpValidation`, and calls `Presenter::write_hot`. The two-slot bit-exact map cache remains keyed by the complete view plus extent; a backdrop obtains the appropriate cached main map for its own extent and copies it while changing only `apron_scale`, so main-cache semantics and the HOT zoom-mismatch refusal are unchanged. A separate two-slot footprint cache is keyed by the bit patterns of object, complete view, and extent, retaining the half-extent backdrop decision and full-extent page fact without rerunning either six-rasterization census on an unchanged stamp.
