...
When a reference is accepted, present re-expresses each retained or pending pose from the pose at which that scene was sampled. Generation change alone no longer clears; object-angle change or an out-of-plane origin delta is slice-incompatible, while an in-plane origin delta is exact pan.
...
Scene policy is `auto` by default and preserves that scheduling unchanged. In `manual`, a material control change still drains and writes HOT and requests exactly the bounded warp or clear needed to show the new pose, and MAIN changes still request their reference orbit, but the Preview-to-Final ladder remains paused until `update-scene` restarts it for the current pose; the initial page scene remains automatic, and returning to `auto` with an update pending restarts Preview immediately. `scene_update_pending` is a fact, not an outstanding-work term: once the requested warp or clear is presented, manual mode may leave it true while `app_needs_refresh` becomes false, so the checkbox cannot latch the cooperative loop.

A draft never displaces a better picture. After HOT planning, an accepted, non-exposed warp from a compatible higher-level retained scene sends the schedule directly to Final, skipping Preview in PictureFast and Preview plus Interactive in Deterministic; the first scene and any refused, exposed, slice-incompatible, or MAIN-incompatible warp keep the ordinary Preview-first ladder. The same rule applies after the manual update button starts a ladder, and `draft_skipped_count` plus `last_draft_skip_reason` report the decision without becoming never-hang work terms.
...
`WarpPlan` carries rows, validity, `AnchorHomography|ClearOnly`, chart residual, measured max/p95 error, and the source scene and texture identities it was solved against.
