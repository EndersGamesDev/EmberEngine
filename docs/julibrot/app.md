...
Worker owns the bignum centre, transfer protocol, credit, cancellation, owner records and drains; math defines `ViewControls` and present re-exports it, while present owns palettes, `HotUniform`, the layer-local `SceneUniform` buffers, its two retained value textures, one presentation-value target, the hot ring, scene and warp fences, backdrop/main composition, warp planning, the sole shade pass, and present facts.
...
App’s ordinary schedule begins at Preview after a compatible selection or extent change, asks kernels for the next level under the selected precision mode, may skip a level when newer work makes it irrelevant, and requests at most one scene submission while present’s sole target is in flight; `SceneBusy` leaves the same next level pending. Main admission proves that two equal Final-capacity spans and six immutable header sets fit, then allocates the spans atomically, so 1,920 by 1,080 and 2,560 by 1,440 both select divisor two instead of accepting a Final extent whose second grid would fail. When the rasterized footprint has a reviewed apron that itself recovers at least 40 of the 3,969 interior lattice samples and at least half of the best measured gain, app first requests the smallest such apron as one separate backdrop through its own Final-level plan at floor-half main dimensions, no more than one quarter of the main Final records, because missing reachable ground has priority over a detail level; frame-rim fill-rule ties and a face-on positive relief request none. The temporary backdrop-only presentation has Preview rank, so its completion cannot be mistaken for a retained main Final and the main ladder’s Preview remains next.

Coverage first buys the backdrop its own delivery, not the whole gesture. The backdrop is requested against the view stamp and a drag moves that stamp every frame, so a schedule that simply prefers a stale backdrop dispatches a new one before the ladder ever runs and presents the entire drag at the coarse sampling. The two alternate instead: one backdrop, then one main level, then the next backdrop, and a turn is yielded while a main scene is in flight because switching the presented layer replaces present’s selection and drops the scene in flight. A settled coverage layer hands its claim back, so the first delivery in the next pose family is wide again.

A kernel level is due when sampling state or the composed neutral-height map changes, when an edge-on stamp becomes mapped, or when refinement remains pending. A raster-requested backdrop is due ahead of that level until its pose-equivalent coarse grid is ready, subject to the alternation above; it shares the main reference and precision mode but owns its extent, capacity-selected plan, record grid, and map. Both admitted main spans remain resident while that single backdrop is planned, so the backdrop honestly capacity-degrades against the descriptors left after the retained-record guarantee rather than evicting either main span. When coverage falls back to apron one, or optional backdrop planning or allocation degrades to a main-only frame, app releases any former backdrop through the shared grid-free path and present forgets its records and completed image before their span can be reused. Its dispatch sits behind the same outstanding-reference, pending-navigation and zoom guards as the main dispatch, so a stale orbit never drives the wide layer; and the Final-only status census counts the main grid alone, so a Final main publishes the same glitch count with a backdrop attached as without one. A scene is due when presentation changes, when a warp exposes source-free pixels, or when a compatible kernel level completes; a control edit that leaves both the slice and `F` unchanged to `1e−12` does not restart the ladder.
...
The same census names one reference candidate per completed level, and app uses it to choose the reference point. The reference is not required to be the navigation centre: the centre is where the view is, while the reference is only the orbit perturbation is expanded around, and a reference whose own orbit ends before a level's cap turns every record that outlives it into a glitch. On a completed level whose cap is strictly longer than the accepted orbit, app requests a new orbit at the candidate's pixel through `ViewerController::request_reference_for_pixel`, which moves the orbit point without moving the navigation centre and leaves the resulting displacement in HOT's `centre_from_reference_px`.

Four rules bound what that exchange can cost and what it can do. The completed level's cap must strictly outlast the accepted orbit, because only then can that grid's top-ranked record name a longer one at all, and the request stops the moment the reference outlasts the level. At most one request is outstanding per accepted orbit length: a ladder whose Interactive and Final caps both outlast one short reference would otherwise spend two of the bounded slots buying a single correction, since the second request only supersedes the first. At most four requests are made per accepted navigation, so a grid whose glitches have another cause cannot drive an unbounded chase, and a refused request simply leaves the Final delivered with its glitches counted. And because the top rank is a heuristic rather than a certificate — a record can report no escape within the cap while its exact orbit is far shorter — an arriving sampled reference that does not lengthen the accepted orbit is discarded before it is uploaded; the exchange is bounded and keeps the best, not monotone.

An accepted correction resumes the refinement ladder at the level whose census named the candidate rather than restarting it from Preview. A reference exchange replaces the orbit the same view is expanded around and changes no pixel's meaning, so the levels below the one already delivered would only repaint that meaning at lower resolution. The page publishes what the correction costs as `sampled_reference_requests`, `sampled_reference_rounds`, and `sampled_reference_discards`.

A discarded correction resumes the same ladder rather than ending the round. The request is a navigation: its delta is zero and the view does not move, but the owner's exact-edit path still spends one requested generation and one centre revision staging it, so a discard that only returns leaves the accepted lease naming the state before the correction and the scene gate reads the reference as belonging to an older selection for as long as the page is open. The discard therefore hands the orbit already held to that navigation, carrying the source generation with the rest, and resumes at the level whose census asked. The owner answers a navigation only through the submission that named it, so the arrival is processed while that submission is still in flight and finished afterwards: a submission finished first is a navigation nothing can be handed to, whatever the discard path then tries to do. The successor a finished submission releases is taken later in the same turn, so nothing waits for the change of order. Renewing a short orbit across a change of view still requires the conservative full-cap length; this is not a change of view, which is what the source generation records. The orbit stays as short as it escaped, so the levels drawn from it carry reference-exhausted records where a pixel outruns the reference and exact escape counts everywhere else, and `sampled_reference_refusal` names why the correction bought nothing while that verdict stands.

A correction has three ends, and each is published. The orbit is taken and the ladder resumes; or the gesture moved on while the correction was in flight, in which case the owner answers only its newest navigation and refuses, and the reference that newer navigation brings is the pending replacement work a hold is written for; or the orbit cannot be handed over and no newer navigation exists, in which case there is nothing to wait for. Only the third clears the hold: the ladder stands down, `scene_refusal_reason` says why, and the stale-view rule does not re-arm the level, because a due level with nothing in flight is exactly the claim that has just been shown false. New scene input retires the verdict.

At a centre outside the set this is the ordinary case rather than a corner: the reference orbit escapes after a handful of iterations, every level's cap outlasts it, and the census candidate is another point of the same exterior whose orbit is no longer, so the correction is asked for and discarded on every accepted navigation.
...
App then drains HOT, constructs `O`, builds the main `F`/`M` at the requested extent from `O`, `Q`, camera translation and observer controls, converts a singular map to `PoseMap::EdgeOn`, builds `PresentHot` and the math-owned main `Pose`, selects `WarpValidation`, and calls `Presenter::write_hot`. The three-slot bit-exact map cache remains keyed by the complete view plus extent; a backdrop obtains the appropriate cached main map for its own extent and copies it while changing only `apron_scale`, so main-cache semantics and the HOT zoom-mismatch refusal are unchanged. A separate three-slot footprint cache is keyed by the bit patterns of object, complete view, and extent, retaining requested and delivered backdrop decisions plus the full-device-extent page fact without rerunning any six-rasterization census on an unchanged stamp; both caches insert FIFO and promote a hit to the newest slot.
...
If mapped kernel work is due, app passes that level's main `M` to `encode_shallow` or `encode_perturbation`, then gives present the same sampled map. Before Preview when `scene_footprint` requests an apron greater than one, the same dispatch path instead runs the backdrop's Final level with `zoom_log2−log₂(apron_scale)` and its copied map; deterministic mode uses the same accepted reference and precision as main. Edge-on skips kernel encoding and installs an all-sky scene. `SceneBusy` leaves the work pending and allocates no third scene texture.
...
A target click stores a four-dimensional plane point obtained through the main presented-camera map and changes no projection. `crosshair_plane_px`, `pan_px`, and `zoom_about_crosshair` never use the coarse backdrop map: a plain drag maps its endpoint difference through the main `M` and pans, a Shift-box release maps its centre and applies `Δq=log₂(min(W/w_box,H/h_box))`, and a scale move applies the requested `zoom_log2` delta about the stored target. The crosshair's screen projection likewise uses the presented camera and main map; the worker then atomically updates the bignum centre and displacement for edits that move or scale the chart.
...
If source identity, cap, precision, slice, residual, solve, or one-pixel error bound fails, `source_valid=0` clears until the next scene. Horizon is exterior, MapUncertain remains sampled in PictureFast, and an accepted reference shift rebases each retained pose exactly once from the accepted HOT coordinate frame before that pose can be sampled again.
...
The next completed scene atomically promotes its value texture, pose, grid, generation, measured fence and level; later due turns submit Interactive then Final, while every intervening surface frame reprojects the newest compatible completed values and applies the current palette.
...
Scene policy is `auto` by default and preserves that scheduling unchanged. In either scene mode, a refused or unavailable redraw with a compatible retained picture becomes `HoldStale` only when the presenter's independent flat-map proof says that source covers the requested destination; the last accepted best remains visible, unmoved and explicitly stale, regardless of scheduler state. The hold has no round, elapsed-time, or hold-count expiry and ends when a usable warp or completed requested scene replaces it, or when the coverage proof no longer holds. In `manual`, MAIN changes still request their reference orbit, but the Preview-to-Final ladder remains paused until `update-scene` restarts it for the current pose; the initial page scene remains automatic, and returning to `auto` with an update pending restarts Preview immediately. `scene_update_pending` is a fact, not an outstanding-work term: once the requested warp or hold is presented, manual mode may leave it true while `app_needs_refresh` becomes false, so the checkbox cannot latch the cooperative loop.

A draft never displaces a better picture. After HOT planning, any accepted warp from a compatible higher-level retained scene sends the main schedule directly to Final, skipping Preview in PictureFast and Preview plus Interactive in Deterministic; `ReliefRedraw` covers an over-ceiling or corpus-refused plane-preserving move when its conservative hole-plus-residual-disagreement estimate is at most 8%. Each accepted Final keeps the DATA span and dimensions it was sampled from, and the next ladder uses one alternate Final-capacity span, so Preview or Interactive cannot erase the records the redraw is presenting; promotion exchanges the two roles. The presenter may redraw records from the grid currently named as main while that grid is idle; immediately before a later kernel dispatch overwrites that span, app drops its retained-record binding but keeps the completed texture available for an honest image hold. The redraw retains and draws the complete main record mesh and never draws the coarse backdrop, so ground the source lattice does not cover stays at the distinct clear colour until the requested main scene lands. An exposed accepted image warp likewise retains the sharp source over its covered region. The first scene and any refused, slice-incompatible, MAIN-incompatible, or over-8% relief plan keep the ordinary Preview-first ladder. The same rule applies after the manual update button starts a ladder, while manual mode without an update presents and holds the redraw; that presentation reports `ReliefRedraw`, because the plan is accepted rather than a refused `HoldStale`. `draft_skipped_count` counts omitted levels, and `last_draft_skip_reason` distinguishes covering from exposed acceptance without becoming a never-hang work term.
...
A preset is a row of control values and nothing else, and it is no longer a thing of its own: the built-in rows and the user's saved rows are one list, offered on each side of the view. `app_preset(id)` expands a built-in row into the complete stored shape, including zero depth and an exact centre at its origin, and the page reads them in order until the app runs out of them, so a row added there appears on both sides with no edit to the loader. Choosing a row on a side applies it and makes it that side's view, which is what the load buttons did on their own. A save asks for a name, refuses an empty or already-taken one with a visible message, captures the current row, adds it to both sides' lists and selects it on the side it was saved from; the named rows and the two selections persist in `localStorage` through the same wrapped reader and writer the boxes use. Built-in rows are never deletable and a saved row deleted on one side leaves both. Each box and the sampling group carry a copy control that puts the row on the clipboard as its exact JSON on one line and the box's own summary on the next, the boxes copying what they hold and `copy-current` copying the live control state, so a broken picture is reported as the row that produced it rather than as a description of it; the row is stringified whole and never assembled field by field, and a clipboard that is absent or refuses falls back to the same text selected in a read-only textarea. A row is written into the controls field by field, by name — the control that carries a field is the one whose id is the field's name with underscores as dashes, with `height_scale` and `zoom_log2` the two exceptions — and then crosses `app_apply_saved_view` once as a complete transaction; the individual controls keep their direct setters, while row loading and morphing cannot replay those setters into incompatible intermediate states. The rows are Mandelbrot, Julia at `c₀=(−0.8,0.156)`, and a relief row for each; the relief rows differ only in `h=1`, `θᵥ=(0.6,0.97)`, and observer `(0.349,0.262)`, which is the orientation the retired fixed mount had, now a row of numbers a user can leave.
...
JavaScript glue, wasm export, worker bootstrap, and wire protocol use `JULIBROT_ABI_VERSION=3`, while the checked-in loader query remains pinned at `v=1` in URLs `./pkg/ember_lab_julibrot.js?v=1`, `./pkg/ember_lab_julibrot_bg.wasm?v=1`, and `./worker.js?v=1`; `main.js` publishes that worker URL as `globalThis.JULIBROT_WORKER_URL` before wasm startup, the Rust browser owner reads it for initial construction and restarts with the checked-in URL only as a fallback, and every orbit-pool transfer remains withheld until the object handshake accepts ABI three, while any disagreement becomes typed `VersionSkew` before orbit work.

The loader query version is deployment cache policy rather than the wire ABI and remains pinned to `v=1` by the page contract; `deploy-pages.sh` rewrites every `?v=1` token in the assembled `index.html`, `main.js`, and `worker.js` copies to the same deploy stamp written as `server.json`'s `v`, refuses a source copy without the pinned token, and refuses any assembled copy that still carries it, while protocol changes continue to bump `JULIBROT_ABI_VERSION`.
...
One slider `t` runs from 0 to 1 and is labelled A ↔ B. It is disabled until both boxes hold a view. Moving it asks math for `lerp_view` at that `t`, writes every field of the interpolated row into its control element, and sends the whole row through `app_apply_saved_view` once. Rust compares every field bit-exactly with the requested view, stages only differences, and releases at most one navigation decision after the row is complete; releasing `t` anywhere leaves a live row a user can carry on moving by hand.
...
`JulibrotKernels::plan_grid_pair` requires two equal Final-capacity spans and six immutable header sets to fit the live executor, and `allocate_grid_pair` obtains both spans through the heap executor's atomic paired allocation before it publishes either kernels-owned grid; `allocate_grid` remains the single-span path for a backdrop.
...
|---:|-----|----|-------|
|0|`generation_applied`|`u32`|Latest accepted selection when nonzero; zero is the no-reference slice-replacement sentinel|
...
`PaletteRecord` is `repr(C,align(16)) {map:[f32;4],interior_rgba:[f32;4],clear_rgba:[f32;4]}`, exactly 48 bytes; Classic is `{map:[64,0,0.78,1],interior:[0.005,0.005,0.008,1],clear:[0.015,0.018,0.025,1]}`, Ember is `{map:[48,0.02,0.88,1],interior:[0.01,0,0,1],clear:[0.015,0.008,0.005,1]}`, and Ice is `{map:[80,0.55,0.72,1],interior:[0,0.005,0.01,1],clear:[0.005,0.01,0.015,1]}`. It is uploaded only for the presentation shade pass.

`PresentHot` carries epoch, plane, six object angles, origin, zoom, twenty view scalars, the level map, and centre displacement.

`PresentBackdrop` is the CPU-only record `{grid:EscapeGrid,iteration_cap:u32,plane:Plane,map:PoseMap}`, and `PresentMain` is `{epoch:u64,state:MainState,grid:EscapeGrid,object:ObjectAngles,plane:Plane,map:PoseMap,backdrop:Option<PresentBackdrop>}`; present applies the worker state's reference shift once when `generation_applied` advances, using the last accepted HOT pose to convert that shift into each retained or pending pose's coordinates. `centre_revision` remains the draft and scene-compatibility revision and is not an accepted-reference clock.
...
`HotUniform` is exactly 288 bytes: ten camera sine/cosine pairs at 0–64, camera translation at 80/96, observer at 112, view scale at 128, warp rows at 144–176, screen-map rows at 192–224, zeroed reserved value-pipeline lanes at 240/256, and flags at 272.
...
`WarpPlan` carries rows, validity, `AnchorHomography|ClearOnly|HoldStale|ReliefRedraw`, chart residual, measured image max/p95 error, the source scene and texture identities, the destination pose and planned exposed fraction for a relief redraw, and an optional `WarpRefusalReason`: `PoseMismatch`, `ObjectSamples`, `Matrix`, `ChartResidual{px}`, `ErrorCeiling{max_px,p95_px}`, `ErrorCorpus{refused_samples}`, `ReliefExposure{predicted_fraction,limit}`, or `EdgeOn`. `warp_refusal_reason` serializes that reason as one string and is null for an accepted warp; an accepted relief redraw also publishes null `warp_max_error_px` and `warp_p95_error_px`, because the rejected image homography's error is not the mesh algorithm's error. `warp_kind=HoldStale` is the page-visible fact that distinguishes a coverage-qualified refused or unavailable redraw from an accepted moving warp. A hold draws the last completed picture unmoved, so the view that reaches the canvas is the one already there and not the one being asked for: the loop stamps the requested view as presented only for a warp that draws for it, and `presented_view_stale` therefore stays true under a hold of another view's picture. That reading makes the hold auditable: it is defensible exactly while the retained source remains compatible and its flat inverse map covers the requested destination, independently of scheduler or manual state.

Both readings come from the warp plan's source, which decides what that comparison can say. A hold names the retained frame, and the completed reading names the same frame, so the two stay equal through a hold of any length and the rule keeps working exactly as it did before the stamp changed. A clear names no source, so a completed scene beside a blank canvas reads as the one-present gap and the loop waits: that is the state every page load passes through, where the opening warps clear and the first scene completes a turn before its own warp runs. `completed_scene_id` survives a clear — the clear is a presentation decision, and only the retained records leaving the heap retires the fact — so the picture remains available for a later compatible reprojection. A clear replaces a hold when the retained source is absent or incompatible, its map is invalid or edge-on, or the destination falls outside the source coverage; that transition is independent of outstanding scheduler work.

Manual refinement is exempt from the completed-versus-presented comparison that starts automatic replacement work. Coverage qualification itself is unchanged: a manual page holds only when the retained source is compatible and covers the destination. The rule starts no work there; it records that the pose moved, and the page's own update decides when a scene is drawn. Withholding the record would lose the pending-update flag the page shows without saving any repaint, so a manual page marks every stale turn as it did before the rule existed, and what the observation then does is unchanged in both modes.
...
`PresentFacts` is `{completed_scene_id:Option<u64>,in_flight_scene_id:Option<u64>,source_generation:Option<u32>,precision_mode:&'static str,delivered_width:u32,delivered_height:u32,delivered_level:Option<RefinementLevel>,iteration_cap:Option<u32>,palette:PaletteId,view:ViewControls,last_scene:Option<SubmissionMeasurement>,last_warp:Option<SubmissionMeasurement>,reprojected_per_scene:Option<u32>,relief_redraw_count:u64,warp_hold_count:u64,refreshes_without_scene:u64,texture_reallocations:u32,warp_exposed:bool,warp_exposed_fraction:Option<f64>,chart_residual:Option<f64>,warp_max_error_px:Option<f64>,warp_p95_error_px:Option<f64>,warp_kind:WarpKind,warp_refusal_reason:Option<WarpRefusalReason>,presentation_ledger:PresentationLedger,held_frame_partition:Option<FramePartition>,held_since_scene_id:Option<u64>,status:PresentStatus}`. `palette` is the current presentation control, not retained-scene provenance: changing it reuses the retained values and the next present's sole shade pass applies it without a new scene. For `ReliefRedraw`, `warp_exposed_fraction` is the admitted planned fraction rather than a synchronous surface readback: the existing `census.rs` pass counts record status, while reading the presented surface in the submission path would delay or misattribute the moving frame. `PageFacts.warp_wall_ms` and `warp_fence_wait_ms` publish the latest value-reprojection-plus-shade submission wall and its polled-fence subset, with the callback-observation fields retained as explicit aliases; these include browser scheduling and are not GPU timestamps, so they do not isolate the shade pass. `PresentationLedger` retains the last eight successful presents in oldest-first order; each redraw entry converts destination chart units to the retained delivery lattice and back before deriving the requested centre and upper-left anchor, then rounds to hundredths. While Final owns those records, the presenter keeps the ledger's last displayed relief plan accepted so the loop leaves that surface image in place rather than replacing it with one stale hold turn.
...
`Warp::reproject(last_frame,from_pose,to_pose,precision_mode,validation)` is pure f64 planning. It refuses absent or mismatched source identity, incompatible sampled plane, more than half a source pixel of chart residual, edge-on state, and invalid matrix arithmetic. A finite compatible image plan is accepted only at or below `WARP_MAX_ERROR_PX=1.0`. Above that ceiling, or when a non-horizon corpus sample reaches a perspective refusal, the planner selects `ReliefRedraw` if its 65-by-65 source-footprint estimate combined with the measured 7.1952% hole-plus-residual-disagreement risk is at most 8%; otherwise `ReliefExposure` keeps it clear for the existing hold policy. The redraw keeps per-vertex perspective refusal and depth testing. Its compiled stretch guard would discard cells stretched across disocclusions and leave clear holes, but that guard is disabled while `RELIEF_STRETCH_GUARD` is `None`; the redraw neither delays nor displaces the render ladder.
...
`SavedView` carries six object angles, four origin coordinates, ten camera angles, five camera translations, yaw, pitch, height, distances, zoom, encoded centre, optional encoded target, and readout mirror; `SavedView::lerp` composes math's interpolators.

Applying a saved or morphed row is one app transaction. Every row field is compared bit-exactly with the requested state and unchanged fields perform no staging. A different slice retires the orbit metadata and navigates once with `INITIAL`; changed centre or scale is installed by one navigation configuration with one reason; a plane-preserving origin change stages its MAIN origin once and remains an exact retained-source pan without retiring the orbit. A pose-only row performs no reference request and does not rebuild MAIN, and each distinct transaction increments one requested-view revision used by static scheduling.

The app and present currently carry separate copies of `origins_share_slice`, and their inputs are not equivalent: row application classifies with the new row's `zoom_log2` and the requested grid width, while presentation classifies its retained frame with that frame's pose. A morph that changes origin and zoom together can therefore preserve the app orbit while present rejects the retained frame as a different slice. The classification is not a shared contract until the helper moves into math and both call sites agree which pose supplies zoom and width.

`SettledPromotion` defines the app-owned `PictureFast` static-quality state machine behind `SETTLED_DETERMINISTIC_PROMOTION_ENABLED=false`. A bit-distinct requested-view revision restarts the fast tier; after that revision has remained unchanged for `STATIC_SETTLE_WINDOW_MS=400` and its fast Final has completed and been presented, the state machine emits one deterministic start, keeps naming the fast Final until the deterministic scene's matching warp is presented, and then reports the deterministic tier without mutating the selected precision control. Explicit `Deterministic` mode bypasses promotion. Activation requires `stage_effective_precision_mode` to stage `MainState.precision_mode` without changing the requested control or calling `note_requested_change`; `BrowserFrameLoop` to own the state machine and report requested revision, fast-Final presentation, deterministic submission, completion, and presentation at the existing sites; and the arrival filter, receipt stamp, and `reference_submission_requires_worker` decision to read that one effective mode. Facts must publish `presented_precision_tier` from what is on the glass rather than the last completed scene. Because present includes precision policy in a frame partition, starting promotion moves the retained fast Final into its held slot for the deterministic window; activation must deliberately decide and document whether that stale-but-valid fast Final remains visible. The disabled flag prevents a partial runtime claim before those links and that presentation decision exist.

The app authorizes a coverage hold whenever a completed source is retained, and present independently proves that the retained or held source covers the requested destination before selecting `HoldStale`; neither decision reads refinement or manual scheduler state. Present keeps the held frame's scene id in `completed_scene_id` across a partition change and publishes `held_frame_partition` and `held_since_scene_id`; despite its name, `held_since_scene_id` is the held frame's own scene id. Once app facts forward those fields, the page uses the presence of `held_since_scene_id` to say that this exact scene is held from a previous partition while its replacement renders, instead of falsely describing the frozen canvas as a current-partition scene.

`PageFacts` follows source publication order and page-contract coverage. In addition to the existing transport, precision, ladder, timing, allocation, and delivery fields it publishes `glitch_pixel_count` for a delivered Final, `scene_mode`, `scene_update_pending`, `draft_skipped_count`, `sampled_reference_requests`, `sampled_reference_rounds`, `sampled_reference_discards`, `sampled_reference_refusal`, `scene_refusal_reason`, `last_draft_skip_reason`, `object_angles`, `plane_origin`, `camera_angles`, `camera_translation`, observer and distance controls, `horizon_pixels`, `horizon_fraction`, `uncertain_pixels`, `uncertain_fraction`, `edge_on`, `map_condition_number`, `warp_exposed_fraction`, `relief_redraw_count`, `warp_hold_count`, warp kind and error facts, `warp_refusal_reason`, `presentation_ledger`, `level_timings`, and discarded draft pixels/iterations. `scene_apron_scale` is the footprint's raster-selected requested span, `scene_backdrop_scale` and `scene_backdrop_extent` are absent until a coarse grid is allocated and then name its applied span and capacity-selected Final extent, `surface_uncovered_fraction` is the interior 63-by-63 footprint census remaining after the requested span, and `relief_clipped_fraction` is the fixed bounded record-domain census at the shared five-dimensional near clamp. The page iterates over every fact entry and JSON-renders object values, so these additive facts need no `main.js` renderer branch.

The page contributes four more facts of its own to the same overlay: `view_box_a_held` and `view_box_b_held`, `morph_t`, and `view_box_storage`. They are not `PageFacts` fields because none of them is app state — two boxes and a slider live in the document and in per-viewer storage, and pushing them into the app would invent a second owner for them. `morph_t` is `null` while the slider is disabled, and `view_box_storage` names the refusal when the storage accessor threw, so a page with no memory says so rather than silently holding nothing. Timer capability, timer probe, wasm and JavaScript bundle bytes, and the two-instance count are immutable boot facts; the page freezes them beside the changing app/page facts and merges them into every refresh snapshot, so a later refresh cannot replace measured boot values with absent app fields.
...
The controller retains accepted neutral-height `PoseMap` values in a three-slot cache keyed by every bit-exact input to their construction — all object angles, all VIEW controls, zoom and extent — so an unchanged Preview or Interactive HOT extent, the requested `ViewStamp` extent, and one additional prepared extent can stay resident; the ignored `measures_viewer_refresh_map_constructions_before_after` viewer-refresh harness measures 240 constructions before and two after across 120 alternating unchanged refreshes, the frame-loop pin drives distinct Preview and requested extents, and the controller refuses a HOT zoom that is not bit-identical to the requested zoom before constructing the pose.

The native viewer harness pins the manual refusal as `HoldStale` with the retained scene still presented, drives Update scene through Preview, Interactive, and Final, then requires the new Final to become the presented source and the hold fact to clear; companion arms require the same refusal in auto mode to hold while its Final fill remains pending, a retired and resumed non-Final round to keep holding, and a bounded manual homography to keep presenting the retained source. A manual Final followed by a pure height change must remain an accepted `ReliefRedraw` with the loop running, while a source-order assertion places both a paused manual schedule and a deep-reference wait before retained-record invalidation in `browser/submit.rs` without executing those guards; behavioural retention during a real wait remains unmeasured. The 90-input height-drag harness runs three seconds at 30 Hz on measured `d₅=8` and `d₅=2` fixtures. The generalized planner now admits their pure-height changes as relief redraws: both the fixed and moving retained-source probes report zero clear plans, the presentation trace reports zero clears and zero stale holds plus accepted redraws, scene identities stay monotone, and Final settles within one three-frame ladder round.
...
`requires visible replay`: GL identity, RGBA32F usages, scratch-copy visibility, height-zero and relief orientation, pose-only same-origin morph continuity without a reference restart or whole clear, one transition on a genuine slice crossing, immutable boot facts after repeated refreshes, corrected hybrid planes, scaled deep zoom beyond the former f32-underflow boundary, orange glitch diagnostics distinct from magenta contract violations, numeric Final glitch census, clear disocclusion, and a clean console. Once their capability flags are enabled with the named data and presentation hooks, the same replay must show fast Final during the 400 ms settle and deterministic work, swap only after deterministic presentation, cancel immediately on another edit, and hold an honestly labelled previous-partition frame through automatic and manual slice replacement.
...
`settle({ level = "Final", timeoutMs = 120000, requireNewScene = false })` resolves with the facts when the named level is the delivered one **and** `picture_finished` is true.

`picture_finished` is the app's own composed answer, published as one fact so that everybody asking "is it done" asks the same question. It is **six** readings, and each of the last three was paid for by a measurement that came out wrong without it.

The first three are the obvious ones: nothing pending, no manual scene update outstanding, no scene in flight. The fourth is that **the presented image is not stale** — the picture on the canvas belongs to the view being asked for now. A delivered refinement level cannot supply that: a level is a property of the last completed scene and survives a control move, so the instant a row is applied on top of a delivered Final the first three are all true of the *previous* picture, a settle asking only those resolves immediately, and a driver applying rows in a loop measures every row after the first one row late.

The fifth and sixth say the pixels on screen are the finished scene's. **A scene completes one turn before its warp is presented**, so there is a window in which the ladder is done, the pose agrees, and the eye is still on the previous picture; a caller copying the frame in that window copies the wrong row. Measured on the WebGL2 floor: a row applied at `height_scale` zero settled after 1,076 ms reporting `completed_scene_id` 5 and Final, and the copy taken immediately came back as scene 3 carrying the previous row's census exactly. So `picture_finished` also requires that the scene the presented image was **warped from** is the completed scene, and that the warp is not a `HoldStale` — a warp the presenter refused to move is an older picture standing in for this one. `presented_scene_id` is published beside `warp_source_scene_id` for the same reason the two differ: the submitted source says what is being drawn, and only the presented one says what is being looked at.

Every waiter in the module completes on a turn of the loop and none of them waits on an animation frame. A page the browser is not painting is never given one — `requestAnimationFrame` does not fire late in a hidden tab, it does not fire at all — so a waiter built on it hangs forever on a page whose loop is meanwhile turning perfectly well on the 250 ms fallback timer, which is the ordinary condition of a driver page. Measured before the fix: a hidden driver sat in `frame()` for over forty seconds while the copy had been ready almost immediately. `settle` and `frame` both wait on the loop's own turn, and exactly one `requestAnimationFrame` exists in the module: the scheduler's.

Two further rules keep that answer honest. There is no synchronous verdict: a row applied a moment ago has changed the controls and has not yet reached the loop that marks the picture stale, so `settle` always waits for at least one loop turn before deciding. And `requireNewScene` is there for a caller who knows its change must produce a new scene and would rather wait for that scene than for the pose to agree; with it, a `completed_scene_id` unchanged from the one read at the call is never settled. A settle on a picture that is already finished and current resolves on the first turn, which is the right answer to "is this finished" asked of something that is.

It rejects with a `SettleError` carrying the facts it saw in three cases: the app published a `loop_stopped_reason`; the loop went quiet without reaching the level, which is a state nothing will leave on its own; or the timeout expired. The bound exists because an unbounded wait is a hang with a nicer name. A hidden tab clamps its timers to about one turn a second and the animation callback stops entirely, so a driver running with the page not painting should raise `timeoutMs` well above the default rather than treat a timeout as a verdict about the picture; the measured hidden-pane settle of the owner's zoom row took under fourteen seconds and 158 turns, all of them from the fallback timer.

### 10.3 `frame()`, its two routes, and what it reads

`frame()` resolves with `{ width, height, route, scene_id, rgba }`, where `rgba` is packed RGBA, four bytes a pixel, rows top-down, `width * height * 4` bytes long. `scene_id` is the scene the copy was taken from, and a scene that has completed since is a refusal naming both numbers rather than quietly older bytes; `allowOlderScene: true` is for a caller who wants them anyway and will say so in what it reports. `drive.html` asserts the two agree and prints both. The bytes are the eight-bit values the surface format holds, which is what a screenshot of the canvas shows; a blue-first surface is returned red-first so a caller counting colours need not know which of the two eight-bit formats the browser handed the device. `route` names which of the two routes below produced them.

The copy is made inside the renderer, on request only and never per frame. It costs one full surface transfer — 2,073,600 bytes at 960 by 540 — plus the readback buffer while the copy is in flight, and on the fallback route one extra draw of the shade pass on the armed frame alone. The app arms one copy, the renderer takes it, `needs_refresh` stays true until the bytes have been read, and `app_take_frame_rgba` consumes the copy so two readers cannot both believe they hold the frame.

**The direct route — `copied from the surface`.** `Presenter::request_frame_readback(texture)` copies the surface image itself, called from `BrowserRuntime::complete_warp_capturing` between the acquisition of that image and its presentation, which is the only moment the frame exists as a texture the renderer can read. It answers the question outright, and **it is unreachable on WebGL2 today**: wgpu-hal 24.0.4's GLES adapter reports the surface usage as colour-target only, wgpu-core derives `COPY_SRC` from the hal usage alone, and this lab requests `Backends::GL` and nothing else, so `frame_readback_route` never selects it on the device class the lab targets. It was measured refusing on ANGLE over Mesa Intel before the fallback existed. It is kept as the path a future device or backend takes, not as a description of what runs.

**The fallback route — `copied from an offscreen re-render of the shade pass`.** When the surface is not a copy source, the shade pass is drawn a second time into an offscreen colour target of the surface's own extent and format, created with `COPY_SRC | RENDER_ATTACHMENT`, and the copy is taken from that.

It is pixel-identical by construction rather than by hope, and the construction is the argument. The second encode is the same call — the same pipeline, reprojected value texture, nearest sampler, present-time palette uniform and surface clear — appended to the same command encoder as the first, before that encoder is submitted. The pass is deterministic over those inputs, and there is no gap between the two encodes for anything to write to them: no loop turn, no queue write, no scene promotion and no control move. The shade pass has no depth or stencil attachment, so neither draw inherits geometry state.

What could make the two differ is therefore a short list, and it is guarded rather than assumed. A target whose extent is not the presented extent would be a second draw of a different picture, and that is refused with `copy a frame whose target extent is not the presented extent`. A surface format that is not eight-bit four-channel is refused before anything is drawn. A copy already in flight is refused rather than replaced. A capture that cannot be encoded for any of those reasons leaves its typed reason behind, which the loop publishes as `frame_capture_refusal`; the frame it was armed on is presented normally, because the picture is not a casualty of a measurement of it.

Neither route ever answers an empty picture. A map that fails is a typed refusal; bytes short of their own extent are a typed refusal; a map that never completes at all is abandoned after `FRAME_CAPTURE_DEADLINE_MS` (five seconds) with a reason, because holding it costs a surface-sized buffer, refuses every later request and keeps the loop turning for a caller who would otherwise wait out the whole timeout learning nothing; and the module itself refuses a byte count the reported extent does not justify.

`app_frame_capture_json` publishes `frame_capture_supported`, `frame_capture_status` (the selected route's sentence), `frame_capture_route`, `frame_capture_copy_route` (the route of the copy in hand), `frame_capture_scene_id`, `frame_capture_pending`, `frame_capture_ready`, the extent, and `frame_capture_refusal`. A copy is only worth reading when the picture is finished: call `frame()` after `settle` resolves.

Two claims in this section are pinned by source text and by predicate tests rather than by a running device, because the lane has no browser and the native suite has no GPU. That an armed copy is taken on the present it was armed for and on no other is pinned by `CaptureArming::surface_due` and `offscreen_due`, whose truth tables are tested, plus assertions that the loop asks those functions and does not restate them. That the two encodes are adjacent in one encoder is pinned by an ordering assertion over the submission's source. Neither is a substitute for the browser proof, which is what says the bytes are the picture.

### 10.4 The driver page

`web/labs/julibrot/drive.html` is shipped by the deploy alongside `index.html`, `main.js`, `lab.js`, `worker.js` and the stylesheet, and stamped with the same cache key: a pixel proof taken on a locally built copy is a measurement of a build nobody is serving. It is a canvas, a `pre` and one module script. It reads a row from `?row=` in the query string or from a `{ kind: "JulibrotRow", row }` message, calls `applyRow`, `settle` and `frame()`, and writes into the `pre` the settled level and extent, the frame's byte count, the four-class pixel census, and the facts JSON. It publishes the lab object as `globalThis.lab`, so a driver that wants to keep moving controls after the first settle can.
