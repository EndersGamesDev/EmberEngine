...
`SceneUniform` is exactly 160 bytes: grid/span at 0/16, sampled basis at 32/48, sampled `M` at 64/80/96 with the applied apron in byte 108, and zeroed reserved lanes at 112/128/144; present owns one buffer for the main layer and one for the optional backdrop. Palette data lives only in the separate presentation shade uniform.
...
`WarpPlan` carries rows, validity, `AnchorHomography|ClearOnly|HoldStale|ReliefRedraw`, chart residual, measured image max/p95 error, the source scene and texture identities, the destination pose and planned exposed fraction for a relief redraw, and an optional `WarpRefusalReason`: `PoseMismatch`, `ObjectSamples`, `Matrix`, `ChartResidual{px}`, `ErrorCeiling{max_px,p95_px}`, `ErrorCorpus{refused_samples}`, `ReliefExposure{predicted_fraction,limit}`, or `EdgeOn`. `warp_refusal_reason` serializes that reason as one string and is null for an accepted warp; an accepted relief redraw also publishes null `warp_max_error_px` and `warp_p95_error_px`, because the rejected image homography's error is not the mesh algorithm's error. `warp_kind=HoldStale` is the page-visible fact that distinguishes a refused manual or pending-replacement hold from an accepted moving warp. A hold draws the last completed picture unmoved, so the view that reaches the canvas is the one already there and not the one being asked for: the loop stamps the requested view as presented only for a warp that draws for it, and `presented_view_stale` therefore stays true under a hold of another view's picture. That reading is what makes a hold auditable, because a hold is defensible only while the replacement work it waits for is pending.

Both readings come from the warp plan's source, which decides what that comparison can say. A hold names the retained frame, and the completed reading names the same frame, so the two stay equal through a hold of any length and the rule keeps working exactly as it did before the stamp changed. A clear names no source, so a completed scene beside a blank canvas reads as the one-present gap and the loop waits: that is the state every page load passes through, where the opening warps clear and the first scene completes a turn before its own warp runs. `completed_scene_id` survives a clear — the clear is a presentation decision, and only the retained records leaving the heap retires the fact — so the picture is still there to reproject from. The arm is safe as long as a clear cannot strand a completed scene at a stale view, and it cannot while a clear replaces a hold only when nothing is pending, which is the state this rule restarts from: the turn that would produce the stranded blank is the turn the ladder restarts on.

Manual refinement is exempt from the comparison. A manual page holds unconditionally, and the rule starts no work there: it records that the pose moved, and the page's own update decides when a scene is drawn. Withholding the record would lose the pending-update flag the page shows without saving any repaint, so a manual page marks every stale turn as it did before the rule existed, and what the observation then does is unchanged in both modes.
...
`Warp::reproject(last_frame,from_pose,to_pose,precision_mode,validation)` is pure f64 planning. It refuses absent or mismatched source identity, incompatible sampled plane, more than half a source pixel of chart residual, edge-on state, and invalid matrix arithmetic. A finite compatible image plan is accepted only at or below `WARP_MAX_ERROR_PX=1.0`. Above that ceiling, or when a non-horizon corpus sample reaches a perspective refusal, the planner selects `ReliefRedraw` if its 65-by-65 source-footprint estimate combined with the measured 7.1952% stale-resolution risk is at most 8%; otherwise `ReliefExposure` keeps it clear for the existing hold policy. The redraw keeps per-vertex perspective refusal, depth testing and clear holes, and neither delays nor displaces the render ladder.
...
The original app hold predicate remains sufficient after the retained-frame change: present keeps the held frame's scene id in `completed_scene_id` across a partition change and selects its retained-or-held source, so automatic replacement work continues to request a stale hold while refinement is pending and manual mode continues to hold unconditionally. Present publishes `held_frame_partition` and `held_since_scene_id`; despite its name, `held_since_scene_id` is the held frame's own scene id. Once app facts forward those fields, the page uses the presence of `held_since_scene_id` to say that this exact scene is held from a previous partition while its replacement renders, instead of falsely describing the frozen canvas as a current-partition scene.
...
The copy is made inside the renderer, on request only and never per frame. It costs one full surface transfer — 2,073,600 bytes at 960 by 540 — plus the readback buffer while the copy is in flight, and on the fallback route one extra draw of the presentation pass on the armed frame alone. The app arms one copy, the renderer takes it, `needs_refresh` stays true until the bytes have been read, and `app_take_frame_rgba` consumes the copy so two readers cannot both believe they hold the frame.
...
**The fallback route — `copied from an offscreen re-render of the presented pass`.** When the surface is not a copy source, the presentation pass is drawn a second time into an offscreen colour target of the surface's own extent and format, created with `COPY_SRC | RENDER_ATTACHMENT`, and the copy is taken from that.

It is pixel-identical by construction rather than by hope, and the construction is the argument. The second encode is the same call — the same pipeline, the same bind groups, the same uniform buffers, the same clear colour, the same HOT slot asked for its own dynamic offset — appended to the same command encoder as the first, before that encoder is submitted. The pass is deterministic over its inputs, and there is no gap between the two encodes for anything to write to those inputs in: no loop turn, no queue write, no scene promotion, no control move. The depth and stencil attachments clear on load in both, so the second draw does not inherit the first's buffers.
