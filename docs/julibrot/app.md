...
A draft never displaces a better picture. After HOT planning, any accepted warp from a compatible higher-level retained scene sends the main schedule directly to Final, skipping Preview in PictureFast and Preview plus Interactive in Deterministic; `ReliefRedraw` covers pure height or `d₅` changes with all other sampling inputs fixed and neutral-5D-camera motions only when zoom, plane origin, centre displacement, extent, and main map also preserve the sampling lattice. Each accepted Final keeps the DATA span and dimensions it was sampled from, and the next ladder uses one alternate Final-capacity span, so Preview or Interactive cannot erase the records the redraw is presenting; promotion exchanges the two roles. The presenter may redraw records from the grid currently named as main while that grid is idle; immediately before a later kernel dispatch overwrites that span, app drops its retained-record binding but keeps the completed texture available for an honest image hold. The redraw retains and draws the complete main record mesh; a changed backdrop is re-requested first, so ground reachable only by that wider grid remains distinct sky until the coarse request lands. If the paired main grids and reference orbit leave no heap descriptor for that optional backdrop, backdrop planning degrades to a main-only frame instead of stopping the loop. An exposed accepted image warp retains the sharp source over its covered region and leaves only permitted disocclusion temporarily at the distinct clear colour; an over-ceiling non-neutral cross term is refused. The first scene and any refused, slice-incompatible, or MAIN-incompatible warp keep the ordinary Preview-first ladder. The same rule applies after the manual update button starts a ladder, while manual mode without an update presents and holds the redraw; at a pure height change that manual presentation legitimately reports `ReliefRedraw`, because the plan is accepted rather than a refused `HoldStale`. `draft_skipped_count` counts omitted levels, and `last_draft_skip_reason` distinguishes covering from exposed acceptance without becoming a never-hang work term.
...
`WarpPlan` carries rows, validity, `AnchorHomography|ClearOnly|HoldStale|ReliefRedraw`, chart residual, measured max/p95 error, the source scene and texture identities it was solved against, and an optional `WarpRefusalReason`: `PoseMismatch`, `ObjectSamples`, `Matrix`, `ChartResidual{px}`, `ErrorCeiling{max_px,p95_px}`, `ErrorCorpus{refused_samples}`, or `EdgeOn`. `warp_refusal_reason` serializes that reason as one string and is null for an accepted warp; `warp_kind=HoldStale` is the page-visible fact that distinguishes a refused manual or pending-replacement hold from an accepted moving warp. A hold draws the last completed picture unmoved, so the view that reaches the canvas is the one already there and not the one being asked for: the loop stamps the requested view as presented only for a warp that draws for it, and `presented_view_stale` therefore stays true under a hold of another view's picture. That reading is what makes a hold auditable, because a hold is defensible only while the replacement work it waits for is pending. The rule that starts a fresh ladder for a stale view then has to tell the last turn of a hold from a view that has genuinely moved on: a scene that has completed and not yet reached the canvas is one present away, not missing, so the restart waits until the scene on screen is the completed one.
...
`Warp::reproject(last_frame,from_pose,to_pose,precision_mode,validation)` is pure f64 planning. It refuses absent or mismatched source identity, cap/precision mismatch, object-angle incompatibility, more than half a source pixel of out-of-plane origin or chart residual, edge-on/invalid arithmetic, any unprojectable non-horizon sample in the full 9-by-9-by-5 corpus, and measured maximum above `WARP_MAX_ERROR_PX=1.0`. A projection refusal still fails closed, but it no longer erases the finite subset: max and p95 describe every sample both poses resolve, while `ErrorCorpus` states that unresolved points prevented acceptance unless the resolved subset already establishes the stronger `ErrorCeiling` refusal.
