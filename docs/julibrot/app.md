...
`WarpPlan` carries rows, validity, `AnchorHomography|ClearOnly|HoldStale|ReliefRedraw`, chart residual, measured max/p95 error, and the source scene and texture identities it was solved against; `warp_kind=HoldStale` is the page-visible fact that distinguishes a refused manual or pending-replacement hold from an accepted moving warp.
