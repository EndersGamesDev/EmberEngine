...
`SettledPromotion` defines the app-owned `PictureFast` static-quality state machine behind `SETTLED_DETERMINISTIC_PROMOTION_ENABLED=false`. A bit-distinct requested-view revision restarts the fast tier; after that revision has remained unchanged for `STATIC_SETTLE_WINDOW_MS=400` and its fast Final has completed and been presented, the state machine emits one deterministic start, keeps naming the fast Final until the deterministic scene's matching warp is presented, and then reports the deterministic tier without mutating the selected precision control. Explicit `Deterministic` mode bypasses promotion. Activation depends on one additive browser-data seam: `BrowserFrameLoop` retains this state, observes the viewer revision and a genuinely presented matching fast Final, feeds `effective_precision_mode` through the existing owner/viewer/loop/plan precision synchronization, constructs reference requests from that effective mode rather than the selected control, names deterministic scene submission/completion/presentation back to the state machine, and publishes `presented_precision_tier` from `presented_tier`; the disabled flag prevents a partial runtime claim before those links exist.

The app-side previous-partition policy is likewise capability-gated by `PREVIOUS_PARTITION_HOLD_ENABLED=false`. Given an actually retained previous-partition frame, `hold_refused_warp_with_partition_capability` keeps it visible across automatic replacement work and a pending manual update, while `holding_previous_partition_with_capability` identifies the stale-but-honest interval for facts. Activation depends on presentation retaining the invalidated completed `SceneFrame` in a separate previous-partition slot, allowing only `HoldStale` from that slot until the first current-partition scene completes, then releasing it, and publishing `held_previous_partition` plus `held_previous_partition_scene_id`; the page already turns those future facts into an explicit held-scene status.
