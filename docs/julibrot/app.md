...
Aggregate rebase totals remain `unavailable` during normal gather-only rendering. Every delivered Final publishes numeric `glitch_pixel_count` from present's ordered status census; Preview and Interactive report it as unavailable, and the census shares the scene completion ledger while adding its own mapped payload and callback.
...
Normal rendering leaves both rebase aggregate fields unavailable and publishes the numeric status-one count once a Final's census and scene fence have completed; all genuinely absent browser facts render `requires visible replay` rather than zero.
