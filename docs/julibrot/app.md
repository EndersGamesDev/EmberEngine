...
The native viewer harness pins the manual refusal as `HoldStale` with the retained scene still presented, drives Update scene through Preview, Interactive, and Final, then requires the new Final to become the presented source and the hold fact to clear; companion arms require the same refusal in auto mode to hold while its Final fill remains pending, a retired and resumed non-Final round to keep holding, and a bounded manual homography to keep presenting the retained source. A manual Final followed by a pure height change must remain an accepted `ReliefRedraw` with the loop running, while a source-order assertion places both a paused manual schedule and a deep-reference wait before retained-record invalidation in `browser/submit.rs` without executing those guards; behavioural retention during a real wait remains unmeasured. The 90-input height-drag harness runs three seconds at 30 Hz on the `d₅=8` and `d₅=2` owner rows, pins planner partitions `24/65/1` and `81/9/0` for clear/redraw/homography, checks per input that moving and flat retained sources agree on refusal because it is destination-driven at these rows, replaces the 24 and 81 refusals with the same number of stale holds and zero presented clears, preserves monotone scene identities, and settles a Final within one three-frame ladder round.
