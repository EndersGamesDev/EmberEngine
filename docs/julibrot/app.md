...
The fresh lab keeps the whole Mandelbrot set visually centred at centre `(0,0,0,0)` and `zoom_log2=0` but installs Seahorse Valley at the exact slice point `(0,0,−0.75,0.1)` as its visible zoom target. Seahorse Valley is the best first target because it lies on the boundary at the main-cardioid and period-two-bulb neck, where branching filaments and nested spirals remain legible through shallow `f32` zooms and continue into deeper structure, while the initial whole-set frame and its cost remain unchanged.
