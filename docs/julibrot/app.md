...
A kernel level is due when sampling state or the composed neutral-height map changes, when an edge-on stamp becomes mapped, or when refinement remains pending. A positive-relief backdrop is due ahead of that level until its pose-equivalent coarse grid is ready, subject to the alternation above; it shares the main reference and precision mode but owns its extent, capacity-selected plan, record grid, and map. Its dispatch sits behind the same outstanding-reference, pending-navigation and zoom guards as the main dispatch, so a stale orbit never drives the wide layer; and the Final-only status census counts the main grid alone, so a Final main publishes the same glitch count with a backdrop attached as without one. A scene is due when presentation changes, when a warp exposes source-free pixels, or when a compatible kernel level completes; a control edit that leaves both the slice and `F` unchanged to `1e−12` does not restart the ladder.
