...
`picture_finished` is the app's own composed answer, published as one fact so that everybody asking "is it done" asks the same question. It is four readings and not three: nothing pending, no manual scene update outstanding, no scene in flight, **and the presented image is not stale** — that is, the picture on the canvas belongs to the view being asked for now rather than to an older one. The fourth condition is the one a delivered refinement level cannot supply. A level is a property of the last completed scene and survives a control move, so the instant a row is applied on top of a delivered Final the first three are all true of the *previous* picture: a settle asking only those resolves immediately and a driver applying rows in a loop measures every row after the first one row late.
