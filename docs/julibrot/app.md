...
Applying a saved or morphed row is one app transaction. Every row field is compared bit-exactly with the requested state, unchanged fields perform no staging, and the changed fields produce one final navigation action: a different slice navigates once as a slice change, a plane-preserving origin change navigates once as a pan, and changed centre or scale is installed by one navigation configuration. A pose-only row performs no reference request and does not rebuild MAIN.
