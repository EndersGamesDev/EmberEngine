...
The native viewer harness pins the manual refusal as `HoldStale` with the retained scene still presented, drives Update scene through Preview, Interactive, and Final, then requires the new Final to become the presented source and the hold fact to clear; companion arms require the same refusal in auto mode to hold through one ladder round before its Final fill and a bounded manual homography to keep presenting the retained source. The 90-input height-drag harness runs three seconds at 30 Hz on the `d₅=8` and `d₅=2` owner rows, requires zero presented `ClearOnly` frames while an accepted Final exists, monotone scene identities, and a settled Final within one three-frame ladder round.
