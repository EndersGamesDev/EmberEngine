...
A kernel level is due when sampling state or the composed neutral-height map changes, when an edge-on stamp becomes mapped, or when refinement remains pending. A positive-relief backdrop is due ahead of that level until its pose-equivalent coarse grid is ready; it shares the main reference and precision mode but owns its extent, capacity-selected plan, record grid, and map. A scene is due when presentation changes, when a warp exposes source-free pixels, or when a compatible kernel level completes; a control edit that leaves both the slice and `F` unchanged to `1e−12` does not restart the ladder.
