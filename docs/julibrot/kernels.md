...
- A reference that escapes before the cap is still a reference. The delta iteration is defined only while the orbit it is expanded around exists, so every pixel whose own orbit leaves the bailout radius at an iteration the reference still covers reads the same escape count it would read from a full-cap reference, and a pixel that outruns the reference carries the reference-exhaustion kind rather than a number nothing supports. Rebasing extends that reach further, because a rebased pixel restarts at index zero. A picture drawn from such a reference is therefore partly exact and the rest named as uncertain, which is a picture; refusing to draw it is not.
