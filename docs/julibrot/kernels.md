...
- A reference that escapes before the cap is still a reference, and the reason is comparative rather than a claim of exactness. The delta iteration is defined only while the orbit it is expanded around exists, so truncating the reference at its own escape costs nothing below that point: a pixel reaching the bailout radius at an iteration the reference still covers consumes the same records it would consume from a full-cap reference at the same point, and gets the same answer. Whether that answer is itself exact is row 20's question rather than this one, and seven exits ahead of the escape test can return a glitch first: a non-finite iterate, the Pauldelbrot cancellation test, the rebase limit, an unrepresentable negation, the accumulated-error limit, and either renormalization failure. Rebasing widens the reach in the other direction, because a rebased pixel restarts at record zero and can outlive the reference by any amount. A pixel that does outrun it carries the reference-exhaustion kind rather than a number nothing supports, so a picture drawn from such a reference is partly answered and the rest named as uncertain, which is a picture; refusing to draw it is not. The kernel fixture covers the no-rebase case only, and its lengths are record counts, the same convention the app publishes as `orbit_length` and one higher than the escape index the orbit carries separately.
