...
Below the displayed switch POLICY `zoom_log2 < 14`, the shallow path uses `pixel_scale=f32(4.0/(2^zoom_log2·W))`, evaluated in `f64` before one `f32` conversion, and forms `o=(o_u·u+o_v·v)·pixel_scale`; at `zoom_log2≥14` the perturbation path forms the same basis combination in the scaled representation of §2.3 and never forms the tiny absolute scale in `f32`. The screen centre remains offset zero under every accepted map, so the reference orbit remains the existing centre and no absolute deep coordinate is introduced. Main dispatch retains this exact scale; an app-owned backdrop dispatch reuses the same map rows and supplies `zoom_log2−log₂(apron_scale)`, widening only its kernel pixel scale about that unchanged centre.
...
Each pixel also begins with the binary32 relative-error estimate `a = f32::EPSILON` and updates it only when that rebase is taken: `g = sqrt(|Zᵣ|²/|zₙ|²)` followed in source order by `a ← fl32(a·g)` and `a ← fl32(a+f32::EPSILON)`. The gain is the condition factor by which cancellation in the reconstructed `zₙ = Zᵣ+δₙ` amplifies inherited relative error, while the added epsilon accounts for the new rounded binary32 subtraction `δ ← zₙ−Z₀`; a non-finite estimate or `a > 10⁻³` returns numeric glitch `−2.0` before the subtraction is trusted.

The bound deliberately has no per-iteration doubling term: applying the quadratic map's worst-case factor two on every ordinary iteration would cross `10⁻³` from binary32 epsilon after about fourteen iterations, falsely rejecting essentially every legitimate long-lived pixel, and would turn a per-rebase guard into a per-iteration cost. Consequently the two tests still cannot certify error accumulated solely through ordinary advances between rebases, or a multi-rebase cancellation whose estimate remains below `10⁻³`; those cases remain inside the propagated-envelope and visible-readback evidence rather than being called proven orbit lengths.
...
The backdrop adds no refinement discriminant and changes no kernel ABI. App may allocate one second ordinary `RefinementPlan` whose requested extent is floor-half the delivered main Final dimensions, retain only its `Final` level, and dispatch it before main Preview with the same reference orbit, precision mode, and delivered iteration cap; that extent contains no more than one quarter of the main Final records, and the backdrop plan's existing capacity divisor may reduce it further.
...
The relative-precision conformance pin uses pixel `(696,0)` in the 960-by-540 zoom-14 seahorse row with a 41-record centre reference and cap 512: the detector-free CPU mirror reports a sampled cap after 27 rebases although the direct binary64 orbit escapes at index 250, while both production mirrors require numeric glitch after the accumulated detector crosses its bound on rebase 25. Native source conformance pins the WGSL escape, Pauldelbrot, accumulated-error, and rebase ordering; actual shader readback remains one of the visible-replay requirements below.
