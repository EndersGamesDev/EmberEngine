...
Each validation fails the run immediately: the tag shape and allowed series are checked; the tag is annotated; its signature verifies after the armored keys in `deploy/keys/*.asc` are imported into a temporary `GNUPGHOME`; its target is an ancestor of `origin/develop`; the Actions REST API selects the newest successful completed `push` run for `develop` at that SHA whose path is `.github/workflows/ci.yml` and proves that run contains exactly the three successful required jobs; the series manifest version equals the tag; `bash deploy/tests/test-changelog.sh --tag TAG` proves the parsed ledger has exactly one accepted entry for that candidate; and `origin/main` is either absent or an ancestor of the target.
