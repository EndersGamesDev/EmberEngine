...
1. Identify the merged release pull request's commit with `gh pr view NUMBER --json mergeCommit --jq .mergeCommit.oid`; record its result as `SHA`, and do not substitute a branch tip or a local pre-merge commit. The worked tag in the commands below is `julibrot-1.3.0`.
...
3. Before the first release only, switch Pages from the legacy branch build to Actions with `gh api --method PUT repos/OWNER/REPO/pages -f build_type=workflow`, and make the `github-pages` environment admit the ref its deployments will carry: list the policies with `gh api repos/OWNER/REPO/environments/github-pages/deployment-branch-policies` and, when no entry names `develop`, create one with `gh api --method POST repos/OWNER/REPO/environments/github-pages/deployment-branch-policies -f name=develop -f type=branch`. A `workflow_run` job evaluates on the repository's default branch whatever ref its steps check out, so `develop` is the ref `actions/deploy-pages` records, and an environment that does not admit it refuses the deployment awaited at step 8.

4. Confirm the signing key's armored public key is committed under `deploy/keys/`; for the current signing subkey recorded in `deploy/keys/wild-sky-maker.asc`, create the annotated signed tag with `git tag -s -u 309F3BF0ABAAC226494C8D9FFAB00BAD8B48D17C julibrot-1.3.0 SHA -m 'julibrot 1.3.0'`. A different signing key may be used only after its armored public key is committed there.

5. Verify the local tag and its signature with `git verify-tag julibrot-1.3.0`.

6. Put the hosts on `SHA` before the tag is pushed, and prove the book the release will actually publish. Run `EMBER_SHIP_REF=SHA bash deploy/ship-host.sh deploy` for each prebuilt host, then republish its mirror with `bash deploy/republish-host.sh <ssh alias> --repo <address-book repo> --branch hosts/book --per-host`. On the build server, install exact Node 24.20.0 and npm 11.19.0 and run `npm ci`, then assemble the release tree with `EMBER_PAGES_PREBUILT=1 EMBER_PAGES_ARCHIVE=<archive> bash deploy/deploy-pages.sh`, compile the Node project with `npx --no-install tsc -p tsconfig.node.json` and run `node target/web-generated/node-js/check-hosts.mjs --tree <assembled tree>` over it, requiring exit 0.
...
7. Push only the new tag ref with `git push origin refs/tags/julibrot-1.3.0`; do not push a branch or a broader tag refspec as part of this operation.
...
9. Confirm `main` moved to `SHA` with `gh api repos/OWNER/REPO/git/ref/heads/main --jq .object.sha`, then run `gh release view julibrot-1.3.0 --json isDraft,assets --jq '{isDraft, assets: [.assets[].name]}'` and require a published release (`isDraft` is false) with exactly one asset named `ember-pages.tar.gz`.

10. Download the published archive with `gh release download julibrot-1.3.0 --pattern ember-pages.tar.gz --dir DIR` and read its archive-root stamp with `tar -xOf DIR/ember-pages.tar.gz ./version.json`; require its `commit` to resolve to `SHA` and its `version` to equal `r$(git rev-list --count SHA)`, then complete the pending ledger row in a later documentation pull request by replacing its source with `SHA`, recording that version as its stamp, removing `(pending)` so the tag field is plain and leaving `published` absent because the GitHub release archive is the publication. The archive-root file was copied from source `web/version.json`.

11. After the first workflow deployment succeeds, list the environment policies again with `gh api repos/OWNER/REPO/environments/github-pages/deployment-branch-policies` and leave exactly one entry, the `develop` entry step 3 established. Remove the `gh-pages` entry left by the legacy build, and a `main` entry if one exists, since no deployment from this workflow can ever carry `main`; each removal is `gh api --method DELETE repos/OWNER/REPO/environments/github-pages/deployment-branch-policies/POLICY_ID`. Retain the frozen `gh-pages` branch itself as the historical record.
...
Between extracting that asset and configuring the deployment the workflow selects Node 24.20.0, installs locked npm dependencies, selects the pinned Rust toolchain, regenerates the compiler inputs and emits `target/web-generated/node-js/check-hosts.mjs` from its checkout. It runs that gate against the extracted asset and fails the deploy when any live server game has no host it can join. The gate is a checkout-provenance tool rather than a shipped archive sidecar; it reads the extracted tree's own `games.json`, `server.json` and emitted `hosts.js`, so it ranks exactly as the pages in that archive will, and it opens a real socket to each candidate rather than trusting what the book claims. A candidate that does not answer is asked again — three attempts at most, the second after a pause of 1 s and the third after a further 2 s — because one socket is a sample and this gate blocks a release on it: at the Julibrot 1.3.0 release every Four Kings host timed out inside a single window and every one of them answered on a rerun 1.3 s later, so a blip shorter than a probe reported that a site whose hosts were working had none. The pauses are longer than that blip on purpose, since a retry inside the window that failed would only sample it twice; a host that answered on another protocol is not asked again, because it has given its answer and the state where every host is up on the old build is exactly the one a release must hear about promptly; and the log names the attempt that carried a host, so one that needed a second or a third is visible rather than hidden behind a pass. The position and the blocking are both the contract: a step that ran after the deploy, or one marked `continue-on-error`, would be a log line rather than a gate, and `deploy/tests/test-workflows.sh` rejects a fixture of each shape. A site whose pages can find no host on their protocol is a site no player can play, and the moment to discover that is before it is served rather than after.

The environment accepts deployments only from `develop`. The workflow is triggered by `workflow_run`, and such a job evaluates on the repository's default branch whatever ref its steps check out, so `actions/deploy-pages` records the deployment against `develop` and no other ref can appear in the environment's history. The bytes are bound to `main` all the same, by the workflow rather than by the deployment record: it checks out `main`, requires a release tag at that `HEAD` whose published release carries exactly one `ember-pages.tar.gz`, and deploys that asset. Pages built from the frozen `gh-pages` branch until the first workflow deployment replaced it, and the branch remains afterward as the resolvable publication record. Reusing the release asset makes the published bytes identical to the reviewed release instead of producing a second build with merely equivalent source.
...
8. The `github-pages` environment accepts deployments only from `develop`, the one ref a `workflow_run` deployment can carry, while the content it serves is bound to `main` by the workflow's checkout and its release-tag requirement. The `gh-pages` entry from the legacy build was removed once the first workflow deployment (`6411560052`, run `34703926696`) replaced that build, and the `main` entry with it, because no deployment can ever carry `main` and an entry nothing can match describes a policy the environment does not have.
9. The first conforming release tag, `julibrot-1.3.0` at `a322c11f`, proved the end-to-end contract in the order the procedure sets out: the hosts were put on that source before the tag was pushed, release run `34703449716` then created the release asset and fast-forwarded `main`, and Pages run `34703926696` published those same bytes once the host gate found a joinable host for every live server game in the archive. One published version now traces from tag to release asset to served site and to the servers its pages join, which is what the migration existed to establish.
