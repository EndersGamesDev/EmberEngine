...
A release tag points at a commit reachable from `origin/develop`; the newest completed run for that exact SHA whose workflow path is `.github/workflows/ci.yml` has concluded `success`, and its jobs are exactly `cores + servers`, `deploy scripts` and `workspace`, each with conclusion `success`; the series manifest version equals the tag version; and the pending changelog contains the accepted entry. These provenance checks bind the name, source, tested result, workflow definition, manifest and release notes to one commit.
...
Each validation fails the run immediately: the tag shape and allowed series are checked; the tag is annotated; its signature verifies after the armored keys in `deploy/keys/*.asc` are imported into a temporary `GNUPGHOME`; its target is an ancestor of `origin/develop`; the Actions REST API selects the newest successful completed run at that SHA whose path is `.github/workflows/ci.yml` and proves that run contains exactly the three successful required jobs; the series manifest version equals the tag; `bash deploy/tests/test-changelog.sh --tag TAG` proves the parsed ledger has exactly one accepted entry for that candidate; and `origin/main` is either absent or an ancestor of the target.

After validation, the workflow builds the selected series' wasm bundle and the hub by the same path as `deploy/deploy-pages.sh`, stamps the version, and uploads the resulting site archive as the release asset. It runs `deploy/github-releases.sh --apply --tag TAG`, whose single-tag path refuses the historical annotation and series-path fallback when the exact changelog entry is absent, then uses the SSH key in `RELEASE_DEPLOY_KEY` for a plain push of the tag's peeled commit to `refs/heads/main`; git and the ruleset both reject a non-fast-forward promotion. Bulk backfill retains the fallback so historical annotated tags can still be reconciled deliberately.
