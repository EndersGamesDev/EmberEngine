...
10. Historical publication after step 9 used `PAGES_DIR=$(mktemp -d -t ember-pages-XXXX); git worktree add -q "$PAGES_DIR" gh-pages`, then `sdk "python3 \"\$(wslpath -u '$REPO_WIN')/deploy/merge-server-json.py\" \"\$(wslpath -u '$WIN')/server.json\" kings_ws '$WS_URL'"` (merge, never overwrite: kept `ws`, `fire_ws`, `proto`, `fire_proto`, `hosts`; set `v`), a commit and push on the host exactly as fire did ("Point kings_ws at …", Co-Authored-By trailer), and `git worktree remove --force`. This pre-Actions procedure is retained as design history; current publication uses validated tag assets and the Pages workflow triggered by successful release completion.
