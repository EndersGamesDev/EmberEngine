...
| Joker | Step: `steps(from, ALL8)` that are empty (never a capture; `JOKER_STEP = true` is a one-line playtest knob). Teleport: each of `mirrors(from)` that is empty; pieces in between are irrelevant, never a capture. Placement: only if `seats[owner].own_turns % JOKER_PLACE_EVERY == 0` (`JOKER_PLACE_EVERY = 5`, so the owner's own turns 5, 10, 15, ...), every empty tile of the board except `from`; never a capture; an unused placement is not banked. Capture: `from + f + l` (front-left in the OWNER's frame, wherever the joker stands) if on board and foreign. This is the joker's only capture. |
...
A seated player is eliminated the moment one of these happens: their King is captured by any foreign piece; they time out `TIMEOUTS_TO_ELIMINATE = 3` consecutive own turns; their connection drops during Playing (v1 has no reconnection, so a lost socket is a lost seat; while Waiting a leave merely frees the seat).
...
`timeout(state)`: as in section 1.7, then `end_turn`. `disconnect(state, seat)`: `present = false`; if alive, `eliminate(seat)`; if `seat == to_move`, `last = Pass { seat, eliminated: Some(seat) }` and `end_turn`; otherwise the seat is skipped when its turn would come. In both cases `end_turn`'s step (1) awards the win when one seat is left.
...
`ui.rs` is a pure `Selection` machine over `rules::targets`: Idle, click an own piece (or, while Waiting, an own Legend or Epic piece) to Selected, click a target to emit `Move { turn, from, to }` (or, while Waiting, a second own piece of the same class to emit `SetFormation` with the two tiles swapped), anything else clears; `pending` is set until the next `State` or `Rejected`. Keyboard: arrows and WASD move the cursor in the local seat's frame (Up is `+u`, Right is `+v` mirrored so that the corner sits bottom-left on screen), rising-edge latched like fire's boost; Enter or Space clicks, Esc clears.
