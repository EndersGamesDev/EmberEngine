...
- Work lands on a branch and enters `develop` through a pull request, never through a direct integration push; [`pull-requests.md`](pull-requests.md) defines the body contract and merge procedure, while `main` advances separately to validated release-tag commits.
