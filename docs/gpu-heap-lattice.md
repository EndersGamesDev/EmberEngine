...
The v8 replay paid that missing diagnostic: the borrow panic was gone, and the first hook line was `heap lattice panic at …/wgpu-24.0.5/src/backend/wgpu_core.rs:3468:38: wgpu error: Validation Error / Caused by: In Surface::get_current_texture_view / Surface image is already acquired`, because a measured frame retained its `SurfaceTexture` across the fence yield while the new selector acquired another; v9 gives the surface its own owner token, makes selectors wait for stale frames to drop unpresented, replaces the default uncaptured-error panic with recoverable generation failure, and corrects the rapid-click oracle's uninterrupted final rung from step 2 to step 3.
...
Executor decision — 2026-09-05: `write_span` retains the established square-padded upload as `SpanUploadMode::PaddedPages`, and a caller may explicitly select `ValidRows` to upload only complete logical rows plus one optional exact-width tail. `ValidRows` deliberately leaves texels after `logical_len` untouched, so a reused page may retain a previous tenant's records and callers must never sample beyond the span's logical bound; the browser proof must prefill a page with nonzero records, write a short valid-row prefix, read back the exact full rows and height-one tail, and prove the consuming shader cannot observe the residue. The native layout oracle sums byte counts from the same `valid_row_regions` production planner used by the real page loop and pins a 4,096-record orbit at 65,536 uploaded bytes instead of the padded 1,048,576 bytes, but WebGPU/WebGL2 validation and readback remain NEEDS BROWSER PROOF before enabling the capability in the app.

Executor decision — 2026-09-05: a selected resident header set is now resolved and compared with its `ExecutorDispatch` once, after which every page uses the already-validated offset slice. Exact pass/offset count equality is checked before iteration so `zip` cannot silently encode a prefix; the native mismatch fixture pins `HeaderPageSelection` at the first unpaired page. The one-page compatibility entry point remains additive, while Julibrot's multi-page path uses `encode_dispatch_selected_set`; the eight-page native fixture pins one admitted comparison exposing all eight aligned offsets.
