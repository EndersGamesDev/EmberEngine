...
**Mirrors.** A `host.json` at a mirror URL is one host entry, same keys. The pages fetch every mirror URL cache-busted, merge the entries into the list, and treat a mirror that does not answer as absent — a mirror that hangs, 404s or returns nonsense is an absence, never an error that reaches the player. What a mirror serves is read generously: one entry, a bare array of entries, or a whole book with its own `hosts` list. A mirror URL is added to the book once, by a writer; after that the mirror updates its own entry as often as it likes. GitHub Pages and raw GitHub URLs both send the CORS header the fetch needs.
...
1. **Load** the address book cache-busted, then every mirror in parallel with a short timeout (four seconds each), and merge the entries by name — later wins outright, because a publish REPLACES an entry rather than patching it. A book with no `hosts` list at all is promoted at this edge into one nameless entry built from its legacy top-level keys, so a page meeting a book nobody has republished yet still finds the server that is right there, and nothing downstream carries a legacy branch.
