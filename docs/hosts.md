...
The what-is-this v1 page includes `stage.julibrot-slide.v1`, a same-origin browser stage that loads `labs/julibrot/`, waits for a settled Final scene, drives the lab's existing DOM controls with ordinary `input` or `change` events, and reads the lab's rendered Facts list once per animation frame. Its fixed scenarios are a 12-step `o13` range drag at 50 ms, two 24-step height drags at `d5=8` and `d5=2` at 125 ms, a 10-step deep-scale drag at 80 ms, a 16-step A-to-B morph at 25 ms, a two-step iteration-cap change at 100 ms, one changed view held for at least 2,000 ms, and a 30-step two-slider alternation at 16 ms.

Each scenario record carries frames observed; clear-only, held, and warp-refused counts and fractions; capped uncovered-fraction samples; completed scene ids; game-side walls for each complete scripted step; and the worker-reference, credit-wait, dispatch, and scene/warp fence walls available in the lab's current `level_timings`. The report writes `null` for facts the lab does not publish today: presented and discarded scene ids, settle-to-paint walls, total reference requests, transfer, packing, and upload walls; it separately labels the sampled-reference request delta that does exist. At most 12 fact values are retained per field, at most 30 raw step walls are retained by a scenario, each observation note is capped at 1,280 bytes, and the complete stage's measurement-record JSON is capped at 10 KiB so the page can enforce the existing 52 KiB report ceiling with room for the other stages.

A browser below the lab's WebGL2 floor records the lab's own refusal reason as an unavailable stage rather than failing the diagnostic game. Run only this stage at `games/what-is-this/v1/?stage=julibrot-slide`; the page starts it immediately, so no button press is required. A full ordinary run uses either `Guess & Run` or `Skip & Run` and appends the Julibrot card after the existing stages.
