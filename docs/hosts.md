...
The writer takes `--name <host>`, a `--game <id> --url <wss> --proto <n>` triple repeated once per game, and `--version`, `--commit` and `--by` for the build stamp; `--remove` deletes an entry and `--recompute` touches nothing but the legacy address keys.
