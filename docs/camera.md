...
|4|`zoom_about`|mutable view, screen, anchor, exponent delta|Old and new fixed offsets share one exact pixel anchor.|At most `(2S + 4 + 4N)M + 4N A`, two scale divisions.|Combined with row 5: 28,149–40,990 ns.|
...
|9|`reference_displacement`|view, exact reference centre, screen|`N` fixed differences and the complete Gram solve remain fixed until two exact rational quotients round to binary64.|The same exact wide work as row 2.|355,220–385,949 ns.|

The release timing contract measures rows 4 and 5 together at `N = 5`, `LIMBS = 8`, with a fractional exponent selecting every table factor and all ten orientation planes active, then measures row 9 separately. Three fixed-evaluation samples measured 28,149–40,990 ns for the edit, 355,220–385,949 ns for reference displacement, and 383,369–423,589 ns combined. Earlier floating-evaluation samples measured 3,580–7,960 ns for reference displacement and 24,920–63,170 ns combined, making row 9 about 47 times slower after exact projection. The measured combined wall remains below the one-millisecond bound. Debug tests report the same quantities without enforcing the performance ceiling.
