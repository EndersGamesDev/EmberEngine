...
| An artist's static prop, already in named parts | **D** — imported: keep the parts, add pivots | the shipped revolver viewmodel (`tools/v15/build_viewmodel.py`) |
...
An artist's prop with **no bones and no animation curves**, split into named parts — a weapon, a door, a machine. The shipped v15 revolver pipeline above is the worked example: it preserves the named moving parts, gives each one the pivot its animation needs, and verifies the exported GLB rather than relying on an unmeasured predecessor.
